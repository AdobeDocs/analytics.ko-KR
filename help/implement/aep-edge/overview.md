...
Edge Network로 전송된 데이터와 AppMeasurement 데이터는 서로 다르게 작동하므로 Edge Network 페이로드는 Adobe Analytics가 히트를 처리하는 방식을 결정합니다. 자세한 내용은 [Adobe Analytics의 Edge Network 이벤트 유형](hit-types.md)을 참조하십시오.

Adobe Experience Platform Edge Network으로 전송된 데이터는 **XDM 오브젝트**, **데이터 오브젝트** 및 **컨텍스트 데이터**&#x200B;의 세 가지 포맷을 따를 수 있습니다. 데이터 스트림이 Adobe Analytics로 데이터를 전송하면 Adobe Analytics가 처리할 수 있는 포맷으로 변환됩니다.

## `xdm` 오브젝트

[XDM](https://experienceleague.adobe.com/kr/docs/experience-platform/xdm/home)&#x200B;(경험 데이터 모델)을 기반으로 만든 스키마를 따릅니다. XDM을 통해 이벤트의 일부로 정의된 필드를 유연하게 작업할 수 있습니다. Adobe Analytics에 특화된 사전 정의 스키마를 사용하는 경우 [Adobe Analytics ExperienceEvent 스키마 필드 그룹](https://experienceleague.adobe.com/kr/docs/experience-platform/xdm/field-groups/event/analytics-full-extension)을 스키마에 추가할 수 있습니다. 추가되면 웹 SDK의 `xdm` 오브젝트를 사용하여 이 스키마를 채우고 보고서 세트로 데이터를 보낼 수 있습니다. 데이터가 Edge Network에 도착하면 XDM 오브젝트가 Adobe Analytics에서 인식하는 포맷으로 변환됩니다.
...
`xdm` 오브젝트를 사용하는 대신 `data` 오브젝트를 사용할 수 있습니다. 데이터 오브젝트는 현재 AppMeasurement를 사용하는 구현에 맞춰져 있어 웹 SDK로의 업그레이드가 훨씬 쉬워집니다. Edge Network는 스키마를 준수할 필요 없이 Adobe Analytics에 특화된 필드의 존재를 감지합니다.

데이터 오브젝트 필드에 대한 전체 참조 및 Analytics 변수에 매핑하는 방법은 [Adobe Analytics에 데이터 오브젝트 변수 매핑](data-var-mapping.md)을 참조하십시오.

## 컨텍스트 데이터 변수

원하는 포맷으로 Edge Network에 데이터를 전송합니다. `xdm` 또는 `data` 오브젝트 필드에 자동으로 매핑되지 않는 모든 필드는 Adobe Analytics에 전달될 때 [컨텍스트 데이터 변수](/help/implement/vars/page-vars/contextdata.md)로 포함됩니다. 그런 다음 [처리 규칙](/help/admin/tools/manage-rs/edit-settings/general/processing-rules/pr-overview.md)을 사용하여 원하는 필드를 해당 Analytics 변수에 매핑해야 합니다.

예를 들어 다음과 같은 사용자 정의 XDM 스키마가 있는 경우:
...
이 필드들은 처리 규칙 인터페이스에서 사용할 수 있는 컨텍스트 데이터 키가 됩니다
...
주어진 컨텍스트 데이터 변수 페이로드(키 및 값 포함)의 최대 크기는 32KB입니다. [`xdm`](xdm-var-mapping.md) 또는 [`data`](data-var-mapping.md) 오브젝트에서 Adobe Analytics가 인식하도록 관련 필드를 조정하여 이 페이로드의 크기를 줄일 수 있습니다.
