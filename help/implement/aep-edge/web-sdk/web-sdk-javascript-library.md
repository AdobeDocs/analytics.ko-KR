...
* [Web SDK 태그 확장 기능](web-sdk-tag-extension.md): Web SDK 태그 확장 기능을 사용하여 새로 설치한 웹 SDK입니다. Adobe Experience Platform 데이터 수집의 태그를 사용하여 구현을 관리한다는 점을 제외하면 웹 SDK JavaScript 라이브러리 접근 방식(이 페이지)과 유사합니다. XDM 스키마에 포함할 일반적인 Analytics 변수를 포함하는 Adobe Analytics ExperienceEvent 필드 그룹이 필요합니다.
...
* [Web SDK JavaScript 라이브러리에 AppMeasurement](appmeasurement-to-web-sdk.md): 태그를 사용하지 않는 경우를 제외하고 Web SDK로 마이그레이션하는 유연하고 방법적인 방법입니다. 대신 수동으로 Adobe Analytics 데이터 수집 라이브러리(`AppMeasurement.js`)를 제거하고 Web SDK JavaScript 라이브러리(`alloy.js`)로 바꿀 수 있습니다.
