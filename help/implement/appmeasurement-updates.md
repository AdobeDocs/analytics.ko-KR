---
title: JavaScript용 AppMeasurement 릴리스 노트
...
# JavaScript용 AppMeasurement 릴리스 노트
...
* 캡처된 버그가 `s.Util.getQueryParam` 수정되었습니다. `#`
...
* 추가 `s.registerPreTrackCallback` 및 `s.registerPostTrackCallback` 후크. (AN-134567)
...
>[!IMPORTANT][!DNL Experience Cloud] ID 서비스 버전 1.6.0을 사용하려면 *용* 버전 1.6.3 이상이 [!DNL AppMeasurement]필요합니다[!DNL JavaScript]. If you want to upgrade to version 1.6.0 of the Experience Cloud ID service, please make sure you are using [!DNL AppMeasurement] code version 1.6.3 or higher.
...
* iOS SDK 버전 4.5부터 새로운 iOS 익스텐션을 사용하면 Apple Watch 앱, Today 위젯, 사진 편집 위젯 및 기타 모든 iOS 확장 앱에서 사용 데이터를 수집할 수 있습니다. Mobile [서비스 사용 안내서의 iOS 확장 구현을](https://docs.adobe.com/content/help/en/mobile-services/ios/ios-ext/ios-ext.html) 참조하십시오.
* Android SDK 버전 4.5부터 새로운 Android 확장 기능을 사용하면 Android 웨어러블 앱에서 데이터를 수집할 수 있습니다. Mobile [서비스](https://docs.adobe.com/content/help/en/mobile-services/android/wearables-android/android-wearable.html) 사용 안내서의 Android 웨어러블을 참조하십시오.
* 방문자 API 1.4 포함.
* DIL 버전 6.0을 사용하도록 AudienceManagement 모듈을 업데이트했습니다.

> [!NOTE] 알려진 **문제**:방문자 API/ [!DNL AppMeasurement] 모듈 통합에는 IE6- [!DNL Audience Manager] 9에서 수행한 두 개의 대상 게시 iFrame 요청이 있습니다. `//fast.<subdomain>.demdex.net/dest5.html` 및 `//fast.<subdomain>.demdex.net/dest4.html`Adobe 다른 브라우저에 표시될 때 올바른 동작은 `//fast.<subdomain>.demdex.net/dest5.html`.
...
* HTTP POST 지원.
   ...
   > [!IMPORTANT] 에서 GET 메서드(IE에서 [!DNL Analytics] 잘린 URL을 해결하는 방법) 대신 POST 메서드를 사용하기 위한 [!DNL AppMeasurement] [](https://helpx.adobe.com/analytics/kb/shortening-image-request-urls.html)호출은 Experience Cloud에 대해 최신 방문자 ID 서비스 구현을 사용해야 합니다.
...
* [Experience Cloud 방문자 ID 서비스](https://docs.adobe.com/content/help/en/id-service/using/home.html) 지원.
...
* 하트비트 비디오 측정에 사용되는 페이지 이벤트를 수정했습니다.
...
* [방문자 ID 서비스](https://docs.adobe.com/content/help/en/id-service/using/home.html)를 지원하기 위해 `VisitorAPI.js`가 추가되었습니다.
