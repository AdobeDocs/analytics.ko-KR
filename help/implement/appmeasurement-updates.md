...
* AppMeasurement은 이전에 특정 쿠키 도메인 구성이 필요했던 국가 코드 최상위 도메인에 대한 루트 도메인을 자동으로 인식하고 활용합니다. 이러한 자동 인식으로 인해 업데이트에 영향을 줄 수 있습니다.
* 배포에는 방문자 ID 서비스 라이브러리 5.5.0 및 Data Integration Library 9.6이 포함됩니다.
...
* [!UICONTROL AppMeasurement]에 종료 링크에 대한 `sendBeacon` 지원 기능을 구현했습니다. 히트가 `sendBeacon`을 사용하고 페이지가 언로드되는 경우 요청이 여전히 완료됩니다. 이 기능은 히트가 데이터 수집 서버에 도달할 가능성이 높으므로 종료 링크에 매우 유용합니다. (AN-175142)
...
* 방문자 ID 서비스가 옵트인에서 승인되지 않은 경우 옵트아웃을 허용하도록 AppMeasurement이 개선되었습니다. (AN-175259)
...
* 세션 저장소에 링크 추적 데이터를 저장하는 지원이 추가되었습니다. (AN-162272)
...
* AppMeasurement이 POST 중 쿠키 전송을 차단합니다. (AN-165538)
...
>[!NOTE]
>
>방문자 ID 서비스를 사용하는 고객은 방문자 API 3.0 이상이 필요합니다. 연관된 코드 라이브러리 (`at.js` 등)가 업데이트될 때마다 최신 방문자 API 버전으로 업그레이드할 수 있습니다.`AppMeasurement.js`

* ID 요청에 업데이트된 방문자 인터페이스를 사용하도록 AppMeasurement을 업데이트했습니다. (AN-151483)
* 링크 추적이 해제된 후에 링크 추적 쿠키가 계속 작성되는 문제를 해결했습니다. (AN-156332)
...
* ID 요청에 업데이트된 방문자 인터페이스를 사용하도록 AppMeasurement을 업데이트합니다. (AN-151483)
* 링크 추적 쿠키는 링크 추적이 해제되면 계속 작성됩니다. (AN-156332)
* AppMeasurement 기본 쿠키 수명을 5년에서 2년으로 줄였습니다. (AN-158572)
...
* VAPI v3.1을 AppMeasurement v2.8로 구성합니다. (AN-158353)
* 공유를 용이하게 하기 위해 데이터 수집 끝점 빌드를 리팩터링합니다. (AN-156647)
* AppMeasurement에 요청 라운드 트립 시간 지표를 추가합니다. (AN-158343)
...
s_gl이 호출될 때 AppMeasurement 라이브러리가 올바른 계정 조합을 항상 설정하지는 않는 문제가 해결되었습니다. (AN-152153)

## 버전 2.5.0

릴리스 일자: **2017년 9월 21일**

* `dil.js` 6.12 포함(Audience Manager 모듈)
* 방문자 API 2.5.0 포함.
...
* 여러 AppMeasurement 인스턴스화 순서에 대한 지원이 추가되었습니다. (AN-138237)
...
* 버전 번호를 2.0.0으로 업데이트해야 하는 새 빌드 프로세스로 이동되었습니다. (AN-137878)
* mboxMCSDID 처리를 추적 호출이 수행되는 올바른 섹션 위치로 이동했습니다. (AN-138483)
...
* VisitorAPI 2.0.0 포함
* 중단 검사가 완료된 후 SDID가 사용되도록 함수 호출 및 검사를 다시 시퀀싱했습니다. (AN-134364)
...
* 방문자 API 1.10.1 포함.
* Audience Manager 모듈이 Demdex Integration Library (DIL) 6.6으로 업데이트되었습니다. (AN-132065)
* 방문자 API 1.9.0 포함. (AN-132072)
* AppMeasurement Audience Manager 모듈을 DIL 6.5 및 추가 구성(AN-129411)으로 업데이트
...
* AMCV 쿠키를 읽고 쓸 수 있도록 AppMeasurement이 업데이트되었습니다. (AN-127098)
* 방문자 API 1.7.0 포함.

>[!NOTE]
>
>JavaScript 버전 1.6.3에 대한 릴리스 노트를 참조하십시오. 이 릴리스 노트에는 방문자 ID 서비스에 대해 업데이트된 요구 사항이 포함되어 있습니다.

## 버전 1.6.3

업데이트 날짜: **2016년 8월 4일**

* AppMeasurement이 요청 연결을 너무 빨리 종료한 문제가 수정되었습니다. (AN-126448)

>[!IMPORTANT]
>
>방문자 ID 서비스 *의 버전 1.6.0에는 AppMeasurement for JavaScript 버전 1.6.3 이상이 필요합니다*. 방문자 ID 서비스 버전 1.6.0으로 업그레이드하려면 AppMeasurement 1.6.3 이상을 사용해야 합니다.
...
* 방문자 API 1.6.0 포함
* AppMeasurement에서 방문자 API에서 잘못 난독화된 메서드를 호출하는 문제가 해결되었습니다. (AN-126006)
* JavaScript 오류: &quot;특성은 v:image에서만 유효합니다&quot;를 발생시키는 문제를 해결했습니다. (AN-124009)
...
* AppMeasurement Activity Map 모듈이 AppMeasurement 표준 모듈에 통합되었으므로 한 개의 `.js` 파일만 참조하면 됩니다. 또한 Activity Map 추적은 기본적으로 활성화됩니다. (AN-112689)
* AppMeasurement의 쿼리-문자열 변수 순서에서 발생하는 잘림 문제가 해결되었으므로 *`pageURLRest`*&#x200B;이(가) 마지막입니다. (AN-114647)
...
* POST가 추적 호출에 사용되는 경우 Audience Manager 모듈에 대한 처리가 수정되었습니다. (AN-115381)
...
* 방문자 API 1.5.2 포함
* VisitorAPI.js에서 Adobe Audience Manager DIL 6.2 - getCustomer ID를 사용하여 /event 호출로 Adobe Audience Manager에 전달하도록 Audience Manager 모듈이 업데이트되었습니다. (AN-104978)
...
* 방문자 API 1.4 포함.
* DIL 버전 6.0을 사용하도록 AudienceManagement 모듈이 업데이트되었습니다.

>[!NOTE]
>
>**알려진 문제**: 방문자 API/AppMeasurement Audience Manager 모듈 통합에서 IE6-9에 두 개의 대상 게시 iFrame 요청(`//fast.<subdomain>.demdex.net/dest5.html` 및 `//fast.<subdomain>.demdex.net/dest4.html`)이 있습니다. 다른 브라우저에 표시될 때 올바른 동작은 `//fast.<subdomain>.demdex.net/dest5.html`.
...
* 보이지 않는 사전 렌더링된 페이지를 추적하지 않도록 하는 WebKit 사전 렌더링 처리 문제를 수정했습니다.
...
* 구현 시 추가적인 대시 문자 구분 기호와 함께 버전 문자열에 추가되는 최대 4개의 문자를 지정할 수 있도록 해 주는 `tagContainerMarker` 변수를 추가했습니다. 이는 Dynamic Tag Management에서 사용됩니다.
  ...
  4자는 영숫자 및 마침표와 같은 URL 파일 경로에 사용할 수 있는 문자로 제한됩니다.

* H 코드로 이중 태그가 지정된 페이지에서, 강제 링크 추적이 활성화되면 (Webkit 브라우저에서 기본값) 자동 링크 추적 (다운로드 및 종료) 시 발생할 수 있는 루프를 수정했습니다. 또한, 유사한 루프를 방지하기 위해 자동 링크 추적에 대한 일반적인 안전 조치를 추가했습니다. 이 안전 조치는 *동일한* 오브젝트에 대한 보고된 클릭 수의 자동 링크 추적을 10초마다 한 번으로 제한합니다. 이 안전 조치는 자동 링크 추적에만 적용되므로, 수동 링크 추적 (s.tl) 호출은 제한되지 않습니다. 다른 오브젝트에 대한 클릭도 이 안전 조치의 영향을 받지 않으며, 추적됩니다.
* 지연이 필요할 때 클릭한 오브젝트에 대한 처리를 수정했습니다.
* 방문자 API에 아직 필요한 값이 없는 경우 링크 onclick 함수에서 s.t가 호출될 때 페이지 보기 수가 두 번 계산되는 문제를 해결했습니다.
* HTTP POST 지원.
...
* AppMeasurement for JavaScript `s_gi` 함수가 H 코드 `s_gi`을(를) 사용하여 만든 인스턴스를 올바르게 찾지 못했습니다. 이 문제는 JavaScript용 AppMeasurement 및 H 코드가 인스턴스가 여러 개인 동일한 페이지에 있는 일부 이중 태그 지정 구현에만 영향을 주었고, `s_gi`은(는) 보고서 세트별로 인스턴스를 찾는 데 사용되었습니다.

## 버전 1.3

릴리스 일자: **2014년 4월 17일**

* [방문자 ID 서비스](https://experienceleague.adobe.com/kr/docs/id-service/using/home)를 지원합니다.
...
* Audience Manager DIL 모듈과의 호환성 문제를 해결했습니다. Audience Manager 고객은 DIL 모듈 버전 4.8로도 업데이트해야 합니다.
...
* [하트비트 비디오 측정](https://experienceleague.adobe.com/ko/docs/media-analytics/using/media-overview)에 대한 지원을 추가했습니다.
* [방문자 ID 서비스](https://experienceleague.adobe.com/kr/docs/id-service/using/home)를 지원하기 위해 `VisitorAPI.js`가 추가되었습니다.
...
* AppMeasurement 오브젝트에서 계층 변수가 설정되지 않는 문제가 해결되었습니다.
