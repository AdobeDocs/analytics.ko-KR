...
4. JavaScript 코드가 1x1 픽셀 이미지 요청을 Adobe 데이터 수집 서버로 전송합니다. 구현에서 정의한 페이지 데이터는 이 이미지 요청에서 쿼리 문자열의 일부로 전송됩니다.
5. Adobe의 데이터 수집 서버가 요청된 이미지를 반환합니다.
6. Adobe 서버는 수집된 데이터를 보고서 세트에 구문 분석하고 저장합니다.
7. 보고서 세트 데이터는 웹 브라우저에서 액세스할 수 있는 보고서를 채웁니다.


Adobe은 플랫폼과 조직의 요구 사항에 따라 이 코드를 구현하는 몇 가지 방법을 제공합니다.

* **웹 SDK 확장**: Adobe Analytics 구현을 위한 현재, 표준화된 권장 방법입니다. Adobe Experience Platform 데이터 수집에 Web SDK 확장을 설치하고, 각 페이지에 로더 태그를 사용하고, 조직에 편리한 형식으로 Adobe Experience Platform Edge에 데이터를 보냅니다. Experience Edge는 들어오는 데이터를 올바른 형식으로 Adobe Analytics에 전달합니다.
* **웹 SDK**: 태그를 사용하지 않으려면 사이트에서 웹 SDK 라이브러리를 수동으로 로드할 수 있습니다. 각 페이지의 웹 SDK 라이브러리를 참조하고 원하는 추적 호출을 Adobe Experience Edge로 보냅니다.
...
* **기존 JavaScript**: Adobe Analytics를 구현하는 과거의 수동 방법입니다. 구현에서 사용되는 변수 및 설정을 간략하게 설명합니다. 이는 사용자 정의 코드가 있는 규칙을 사용하는 구현에 유용합니다.
