...
* **Web SDK 확장**: Adobe Analytics 구현을 위한 표준화된 권장 방법입니다. Adobe Experience Platform 데이터 수집에 Web SDK 확장을 설치하고, 각 페이지에 로더 태그를 사용하고, 조직에 편리한 형식으로 Adobe Experience Platform Edge에 데이터를 보냅니다. Experience Edge는 들어오는 데이터를 올바른 형식으로 Adobe Analytics에 전달합니다.
* **웹 SDK**: Adobe Experience Platform 데이터 수집을 사용하지 않으려면 사이트에서 웹 SDK 라이브러리를 수동으로 로드할 수 있습니다. 각 페이지의 웹 SDK 라이브러리를 참조하고 원하는 추적 호출을 Adobe Experience Edge로 보냅니다.
