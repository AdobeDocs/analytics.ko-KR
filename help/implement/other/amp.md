...
Adobe는 AMP를 사용하여 페이지에서 Adobe Analytics를 구현하는 두 가지 방법을 만들었습니다. 둘 다 `<amp-analytics>` HTML 태그를 사용합니다. 다음을 참조하십시오 [amp-analytics](https://amp.dev/documentation/components/amp-analytics) 자세한 내용은 AMP의 설명서 를 참조하십시오.

* **사용 `"adobeanalytics"` 템플릿**: 페이지에서 바로 Analytics 요청 구성
* **사용 `"analytics_nativeConfig"` 템플릿**: 일반 사이트에서 배포하는 것과 동일한 AppMeasurement 코드가 들어 있는 iframe 사용
...
## 방법 1: `<amp-analytics>` 을 사용하여 태그 지정 `"adobeanalytics"` 템플릿
...
다음의 코드 예제에는 두 개의 트리거 (`pageLoad`, `click`)가 정의되어 있습니다. `pageLoad` 트리거는 문서가 표시될 때 실행되며 `vars` 섹션에 정의되어 있는 대로 `pageName` 변수를 포함합니다. 두 번째 트리거인 `click`은 버튼을 클릭하면 실행됩니다. 다음 `eVar1` 변수가 값으로 이 이벤트에 대해 설정됩니다. `button clicked`.
...
다음 `<amp-analytics>` 태그는 AMP에서 인식하는 데이터 값을 제공할 수 있도록 변수 대체를 지원합니다. 다음을 참조하십시오 [에서 지원되는 변수 `amp-analytics`](https://github.com/ampproject/amphtml/blob/main/extensions/amp-analytics/analytics-vars.md) 자세한 내용은 GitHub에서 확인하십시오.

>[!NOTE]
>
>이 방법을 사용하여 Adobe에게 전송된 이미지 요청에는 많은 기본 보고서(예: 브라우저, 화면 크기 또는 레퍼러)에 대한 데이터가 포함되지 않습니다. 히트에 이 정보를 포함하려면 이 정보가 이미지 요청 쿼리 문자열의 일부로 포함되었는지 확인하십시오. 다음을 참조하십시오 [데이터 수집 쿼리 매개 변수](../validate/query-parameters.md) 이미지 요청의 전체 목록은 쿼리 매개 변수와 관련 변수를 참조하십시오.
...
## 방법 2: `<amp-analytics>` 을 사용하여 태그 지정 `"adobeanalytics_nativeConfig"` 템플릿
