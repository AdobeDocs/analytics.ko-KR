...
* **404 찾을 수 없음**: 이 응답은 이미지 요청을 찾을 수 없으며 데이터가 Adobe 데이터 수집 서버로 전송되지 않음을 의미합니다. 이 응답은 하드 코딩된 이미지 요청의 형식이 올바로 지정되지 않은 경우에도 가능합니다. Analytics을 구현한 개인 또는 팀과 협력하여 이 문제를 해결합니다.
