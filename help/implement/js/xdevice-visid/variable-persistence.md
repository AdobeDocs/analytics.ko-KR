...
* 프로필이 있으면, 그 때부터 이미 시스템에 있는 방문자 프로필이 사용되고 이전 방문자 프로필은 더 이상 사용되지 않습니다.
* 일치하는 방문자 ID가 없으면 새 프로필이 만들어집니다.

인증되지 않은 고객이 사이트에 처음 도달하면 해당 고객에게 Adobe Analytics에서 방문자 프로필이 할당됩니다. 새 프로필이 만들어지면, 한 방문이 종료되고 다른 방문이 시작됩니다.
...
* 지속되는 eVar의 경우 인스턴스 지표가 기록되지 않습니다. 즉, 장치 간 방문자 식별을 사용할 때 eVar 값에 대한 고유 방문 지표가 인스턴스 지표보다 큰 보고서를 볼 수 있습니다.
...
* 지리 특성 데이터는 방문의 첫 번째 히트를 기반으로 기록되며 사용된 장치에 관계없이 단일 방문에 대해서는 변경되지 않습니다. 즉, 새 장치에서 후속 데이터 연결에서 지리 특성 데이터는 일반적으로 포함되지 않습니다.
* 브라우저, 운영 체제 및 색상 심도와 같은 기술 열은 방문의 첫 번째 히트에 기록됩니다. 지리 특성 값처럼, 이 열은 봉합된 프로필에 복사되지 않습니다.
