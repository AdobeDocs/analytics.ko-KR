...
다음 `s.offlineHitLimit` 변수는 디바이스가 오프라인일 때 저장하는 최대 히트 수를 나타내는 정수입니다. 이 변수가 정의되지 않으면 기본값은 입니다. `10`. 어떤 정수 값으로든 설정할 수 있습니다. 높은 값을 설정할 때는 방문자 브라우저의 로컬 저장소 상한을 고려해야 합니다. 이 제한은 일반적으로 5~10MB입니다.
