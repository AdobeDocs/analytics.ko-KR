...
상거래를 사용하는 사이트의 경우 매출 및 통화는 Analytics의 중요한 부분입니다. 많은 사이트, 특히 여러 국가를 아우르는 사이트는 다양한 통화를 사용합니다. 를 사용하십시오 `currencyCode` 변수를 사용하십시오.

통화 전환에서는 각 히트에서 다음 논리를 사용합니다. 이러한 단계는 을 설정하는 수익 값에 적용됩니다. [`products`](../page-vars/products.md) 변수와 다음 &#39;통화&#39;로 나열된 모든 이벤트 [성공 이벤트](/help/admin/admin/c-success-events/success-event.md) 보고서 세트 설정에서 을 선택합니다.

* If `currencyCode` 가 정의되지 않은 경우 Adobe은 모든 통화 값이 보고서 세트의 통화라고 가정합니다. 자세한 내용은 [일반 계정 설정](/help/admin/admin/general-acct-settings-admin.md) 보고서 세트 설정에서 보고서 세트의 통화를 확인합니다.
* `currencyCode`가 정의되어 있고 보고서 세트의 통화와 일치하는 경우 통화 전환이 적용되지 않습니다.
* `currencyCode`가 정의되어 있고 보고서 세트의 통화와 다른 경우에는 Adobe가 현재 날짜의 환율을 기준으로 통화 변환을 적용합니다. Adobe는 [XE](https://xe.com)와 협력하여 매일 통화를 변환합니다. 보고서 세트에 저장된 모든 값은 보고서 세트의 통화로 표시됩니다.
* If `currencyCode` 이 잘못된 값으로 설정되어 있습니다. **전체 히트가 삭제되어 데이터 손실이 발생합니다.** 이 변수가 사용될 때마다 올바르게 정의되어 있는지 확인하십시오.

이 변수는 히트 간에 지속되지 않습니다. 이 변수가 보고서 세트의 기본 통화와 일치하지 않는 매출 또는 통화 이벤트를 포함하는 모든 페이지에서 정의되었는지 확인하십시오.

>[!NOTE]
>
>페이지 간에 통화 코드가 변경될 수 있지만 단일 히트에 있는 모든 통화 지표는 동일한 통화를 사용해야 합니다.

기간 **반드시** 이 변수를 구현할 때 모든 통화에 대한 통화 구분자로 사용됩니다. 예를 들어, 일반적으로 쉼표 구분 기호를 표시하는 스웨덴 크로나는 `products` 변수 및 모든 통화 이벤트를 포함합니다. Adobe은 보고에 올바른 통화 구분자를 표시합니다.
...
`s.currencyCode` 변수는 페이지의 통화를 나타내는 3자로 된 대문자 코드를 포함하는 문자열입니다. 값은 대/소문자를 구분합니다.
