...
* 카테고리가 `xdm.productListItems[].productCategories[].categoryID`에 매핑되어 있습니다. `productCategories[]` 배열의 첫 번째 항목을 사용합니다. `lineItemId`도 올바르게 매핑되지만 표준 XDM이므로 Adobe에서는 `categoryID`를 권장합니다. 두 XDM 필드가 모두 있으면 `lineItemId`가 우선합니다.
* 제품이 `xdm.productListItems[].SKU` 또는 `xdm.productListItems[].name`에 매핑되어 있습니다. 두 XDM 필드가 모두 있으면 `xdm.productListItems[].SKU`가 사용됩니다.
...
* 머천다이징 이벤트는 제품에 바인딩하려는 이벤트에 따라 `xdm.productListItems[]._experience.analytics.event1to100.event1.value`에 `xdm.productListItems._experience.analytics.event901to1000.event1000.value`에 매핑됩니다. 이러한 필드 중 하나에서 이벤트를 설정하면 Adobe Analytics으로 전송되는 [이벤트](events/events-overview.md) 문자열에 자동으로 포함됩니다.
...
[**데이터 오브젝트**](/help/implement/aep-edge/data-var-mapping.md)&#x200B;를 사용하는 경우 제품 변수는 AppMeasurement 구문 다음에 오는 `data.__adobe.analytics.products`를 사용합니다. 이 필드를 설정하면 XDM 오브젝트에 설정된 제품을 덮어쓰기하고 Adobe Analytics로 전송하지 않습니다.
...
## Adobe Analytics 확장 기능을 사용한 제품

Adobe Experience Platform 데이터 수집에는 이 변수를 사용할 전용 필드가 없습니다. 그러나 도움이 되는 여러 서드파티 확장이 존재합니다.

1. AdobeID 자격 증명을 사용하여 [Adobe Experience Platform 데이터 수집](https://experience.adobe.com/data-collection)에 로그인합니다.
2. 원하는 태그 속성을 클릭합니다.
3. [!UICONTROL 확장 기능] 탭으로 이동한 다음, [!UICONTROL 카탈로그]를 클릭하여 사용 가능한 모든 확장 기능을 확인합니다.
4. “product”라는 용어를 검색하면 이 변수를 설정하는 데 도움이 되는 사용 가능한 몇 가지 확장 기능이 표시됩니다.

이 확장 기능 중 하나를 사용하거나 아래의 AppMeasurement 구문에 따라 사용자 정의 코드 편집기를 사용할 수 있습니다.

## AppMeasurement 및 Analytics 확장 기능 사용자 정의 코드 편집기의 s.products

`s.products` 변수는 제품당 여러 개의 구분된 필드를 포함하는 문자열입니다. 각 필드는 문자열에서 세미콜론 (`;`)으로 구분하십시오.

* **카테고리** (선택 사항): 제품 카테고리. 이 필드의 최대 길이는 100바이트입니다.
...
이 변수는 동일한 히트에서 여러 제품을 지원합니다. 여러 제품이 포함된 장바구니 및 구매 시 유용합니다. 전체 `products` 문자열의 최대 길이는 64K 바이트입니다. 문자열에서 각 제품은 쉼표 (`,`)로 구분하십시오.
...
`digitalData` [데이터 레이어](../../prepare/data-layer.md)를 사용하는 경우 `digitalData.product` 오브젝트 배열을 반복할 수 있습니다.
