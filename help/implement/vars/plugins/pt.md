...
이 `pt` 플러그인은 Analytics 변수 목록에서 함수나 메서드를 실행합니다. 예를 들어, 매번 수동으로 메서드를 호출하지 않고 여러 변수에서 `clearVars` 메서드를 선택적으로 실행할 수 있습니다. 이 코드에 따라 다른 여러 플러그인이 올바르게 실행됩니다. 한 번에 두 개 이상의 Analytics 변수에서 특정 함수를 실행할 필요가 없거나 종속 플러그인을 사용하지 않는 경우에는 이 플러그인이 필요하지 않습니다.
