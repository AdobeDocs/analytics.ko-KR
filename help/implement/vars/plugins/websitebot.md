...
* **eVar 설정 구성**: 보고서 세트 설정의 [전환 변수](/help/admin/admin/conversion-var-admin/conversion-var-admin.md)에서 eVar를 설정합니다. 만료를 **Never** 또는 **Visit**&#x200B;로 설정하고 **&quot;Original Value(First)&quot;**&#x200B;로 할당하십시오. 이 eVar은 다음 두 상황에서 모두 설정해야 합니다.[!UICONTROL 직접 호출] 규칙 또는 `s.tl` 호출이 실행된 경우입니다.
...
1. **Mouse Move Listener** 이벤트를 `websiteBot` 규칙에 추가하고 이 사용자 지정 코드를 사용합니다.
...
1. `websiteBot` 를 식별자로 사용하여 Analytics 비콘을 실행하는 [!UICONTROL 직접 호출] 규칙을 추가합니다. 이 예제에서는 `s.tl` 호출을 사용합니다.

   ![websiteBot 식별자](assets/websitebot.png)

1. Adobe Analytics 실행 - 변수 및 Adobe Analytics 설정 - [!UICONTROL 직접 호출] 규칙에서 비콘 보내기 작업을 수행합니다.  이렇게 하는 한 가지 방법은 다음 예제에 나와 있습니다.

   ![비콘 보내기 작업](assets/websitebot2.png)
...
`websiteBot` 플러그인은 비보트 트래픽이 감지되면 `s.tl` 호출을 실행합니다.
