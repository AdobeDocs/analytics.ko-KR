...
1. 식별자로 `websiteBot` 을 사용하여 Analytics 비콘을 실행하는 직접 호출 규칙을 추가합니다. 이 예제에서는 `s.tl` 호출을 사용합니다.

   ![websiteBot 식별자](assets/websitebot.png)

1. 직접 호출 규칙에서 Adobe Analytics - 변수 및 Adobe Analytics 설정 - 비콘 보내기 작업을 실행합니다.  이렇게 하는 한 가지 방법은 다음 예제에 나와 있습니다.
