...
1. Adobe Experience Platform에서 태그를 사용하여 &#39;일반 Analytics 플러그인&#39; 확장 사용
...
1. 먼저 Analytics 추적 개체 ([`s_gi`](../functions/s-gi.md) 사용)를 인스턴스화합니다.
   * 태그가 활성화된 사이트는 Adobe Analytics이 로드될 때 추적 개체를 자동으로 인스턴스화합니다.
...
2. 두 번째로 플러그인 코드를 포함합니다.
   ...
   * 구현에서 Adobe Experience Platform에서 태그를 사용하지 않는 경우 추적 개체를 인스턴스화한 후 어디에나 플러그인 코드를 `AppMeasurement.js`에 붙여 넣을 수 있습니다.
3. 세 번째로 플러그인을 호출합니다.
   * 태그 활성화 사이트의 내부와 외부 모두에서 모든 구현은 JavaScript를 사용하여 플러그인을 호출합니다. 해당 플러그인 페이지에 설명된 형식을 사용하여 플러그인을 호출하십시오.
