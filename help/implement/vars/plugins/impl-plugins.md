...
2. 두 번째로 플러그인 코드를 포함합니다.
   ...
   * 익스텐션을 사용하지 않으려는 경우 Analytics 익스텐션을 구성할 때 사용자 지정 코드 편집기에 플러그인 코드를 붙여넣을 수 있습니다.
...
## 비표준 추적 개체와 함께 플러그인 사용

플러그인은 다른 추적 개체와 함께 기본적으로 작동하지 않습니다 `s`. 하지만 사용자 지정 추적 개체에 맞게 플러그인 코드를 수정할 수 있습니다. 지정된 플러그인 내에서 모든 참조를 원하는 추적 개체 `s` 로 바꿉니다.
