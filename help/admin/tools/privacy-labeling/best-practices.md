...
새 보고서 세트가 작성되거나 기존 보고서 세트 내에서 새 변수가 활성화될 때마다 레이블 지정을 검토해야 합니다. 또한 새로운 솔루션 통합이 활성화된 경우 레이블 지정이 필요할 수 있는 새로운 변수를 노출할 수 있으므로 레이블 지정을 검토해야 할 수 있습니다. 모바일 앱 또는 웹 사이트를 다시 구현하면 기존 변수가 사용되는 방식이 변경될 수 있으며, 이로 인해 레이블 업데이트가 필요할 수도 있습니다.

I1, I2, S1 및 S2 레이블은 Adobe Experience Platform의 동일한 이름을 가진 DULE 레이블과 동일한 의미를 가집니다. 그러나 이들 레이블은 전혀 다른 목적에 사용됩니다. Adobe Analytics 내에서 이들 레이블은 Privacy Service 요청의 결과로 익명화되어야 하는 필드를 식별하는 데 사용됩니다. Adobe Experience Platform 내에서는 액세스 제어, 동의 관리, 레이블이 지정된 필드에 대한 마케팅 제한 적용에 사용됩니다. Adobe Experience Platform은 Adobe Analytics에서 사용되지 않는 많은 추가 레이블을 지원합니다. Analytics 데이터 커넥터를 사용하여 Adobe Analytics 데이터를 Adobe Experience Platform으로 가져오는 경우 Adobe Analytics 내에서 적용한 I1, I2, S1 및 S2 레이블이 가져온 보고서 세트에서 사용되는 Adobe Experience Platform의 스키마에도 적용되는지 확인해야 합니다.
...
* **직접 식별 가능한 ID (I1)**: 사용자 이름을 지정하거나 사용자에게 연락하는 직접적인 방법을 제공합니다. 예를 들어 누군가의 이름(수백 명의 사용자가 사용할 수 있는 John Smith와 같은 일반적인 이름 포함), 이메일 주소 또는 전화번호 등이 있습니다. 이름이 없는 우편 주소는 해당 가구나 사업장 내의 특정 개인이 아닌 가구나 사업체만 식별할 수 있더라도 직접 식별할 수 있는 것으로 간주될 수 있습니다.
...
* 또 다른 예는 Analytics 쿠키 ID와 같은 디바이스 ID입니다. ID가 휴대폰 앱에서 발생하는 경우 해당 ID를 사용하는 모든 상호 작용을 휴대폰 소유자가 사용할 수 있도록 결정할 수 있습니다. 그러나 홈 컴퓨터나 도서실 또는 인터넷 카페의 컴퓨터와 같은 공유된 디바이스에서 ID가 발생하는 경우, 해당 디바이스의 사용자를 구분할 수 없으며 다른 사용자의 데이터를 반환할 위험이 너무 크므로 이 유형의 ID를 사용하도록 허용하지 않을 수 있습니다.

## Analytics에서 지원하는 ID 모범 사례 {#best-practices-an}

이 테이블을 사용하여 데이터 개인정보 보호 요청을 Analytics에 제출할 때 사용할 ID 유형을 확인합니다. 이 정보를 알고 나면 변수에 사용해야 하는 다른 레이블을 결정하는 것이 더 쉬워집니다.
...
<table id="table_E25612E32A03449A8E5DA00B88FCEB9E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>일부 고객은 ID를 <a href="/help/implement/vars/page-vars/evar.md">사용자 정의 트래픽 변수(props) 또는 사용자 정의 전환 변수(eVars)</a>에 배치합니다. 가장 일반적인 것은 CRM ID이지만 이메일 주소, 사용자 로그인 이름, 고객 충성도 번호 또는 이러한 값의 해시를 포함합니다. </p> 
    <ul id="ul_0B9492CF786046BB97E31CCF83A85FEA"> 
     ...
     <li id="li_8115B999E8DA46CAB359BCF1F4A4DCAE">이러한 필드에는 I1 또는 I2 레이블도 필요하며 DEL-PERSON 또는 DEL-DEVICE 레이블이 있어야 합니다. 일반적으로 DEL 레이블의 개인/장치 옵션은 ID 레이블의 개인/장치 옵션과 일치합니다. </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>이 방법은 널리 사용되지 않지만 Analytics에서는 사용자 정의 방문자 ID를 제공할 수 있는 구현을 지원하며, 이 ID가 있으면 기존 Analytics 추적 쿠키 대신 사용됩니다. 이 필드에는 I2, ID-PERSON 및 DEL-PERSON 레이블이 있습니다. </p> <p>많은 구현에서 이 ID는 CRM ID로부터 파생되므로 사용자가 이 사이트에 로그인하는 동안에만 존재합니다. 따라서 여러 디바이스에서 동일한 사용자 정의 방문자 ID를 사용할 수 있습니다. 한 가지 기술적 단점은 사용자가 로그인하기 전에 발생하는 추적이 로그인한 후 수집된 추적에 연결될 수 없다는 것입니다. 대신 사용자 정의 방문자 ID를 사용하여 디바이스를 식별하는 경우 ID-PERSON 및 DEL- PERSON 레이블을 각각 ID-DEVICE 및 DEL- DEVICE로 각각 변경해야 합니다. </p> </td> 
  </tr> 
 </tbody> 
</table>

## 삭제 레이블 설정 모범 사례 {#best-practices-delete}
...
* DEL- 레이블을 사용하면 이러한 변수가 [익명 처리됩니다](/help/admin/tools/privacy-labeling/labels.md#data-governance-labels) (ID가 &quot;데이터 개인정보 보호-&quot; 접두사가 있는 임의의 문자열로 대체됨). 동일한 익명화 값이 요청에 사용된 ID로 식별된 모든 히트에서 원래 값의 모든 인스턴스를 대체합니다. 이 필드의 원래 값이 그러한 ID 중 하나였다면 보고서 지표는 변경되지 않습니다.
* 일반적으로 필드에 레이블 ID-DEVICE가 있으면 레이블 DEL-DEVICE도 지정해야 합니다.
...
* 해당 요청(확장된 ID 포함)에 대한 ID로도 사용되지 않는 변수에 DEL-DEVICE 또는 DEL-PERSON 레이블이 지정된 경우 해당 변수의 고유 값은 지정된 (또는 확장된) ID가 발생하는 히트에 대해서만 익명으로 지정됩니다. 다른 히트에 동일한 값이 포함되어 있으면 다른 위치에서는 업데이트되지 않습니다. 이로 인해 카운트(지표)가 변경될 수 있습니다.
...
## 액세스 레이블 설정 모범 사례 {#best-practices-access}
...
<table id="table_A5B834CC08C641D99E2691A2361997E4"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>사용 중인 ID가 쿠키 ID이거나 ID-DEVICE 레이블이 있는 ID인 경우 ACC-ALL 레이블만 사용해야 합니다. </p> <p>각 액세스 요청에 대한 한 쌍의 파일을 가져옵니다. 한 파일에는 지정된 모든 ACC-ALL 필드와 일치하는 각 히트에 대한 행이 포함되고, 요약 파일에는 이 데이터에 대한 요약이 포함됩니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
