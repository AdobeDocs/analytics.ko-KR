...
>[!IMPORTANT]
>
>보고서 세트가 CX 엔터프라이즈 조직에 연결되어 있지 않으면 해당 사용량 데이터가 이 대시보드에 반영되지 않습니다. 또한 청구 ID를 여러 CX 엔터프라이즈 조직에 연결할 수 있습니다. 조직과 청구 ID 간에 항상 1:1 관계가 있는 것은 아닙니다.

보고서 세트 사용량 대시보드

* CX 엔터프라이즈 조직의 각 보고서 세트에 대한 현재 사용 기간의 서버 호출 사용량 (모든 호출, 기본, 보조, 모바일 기본, 모바일 보조)을 표시합니다.
* 서버 호출 카테고리당 전체 사용량 비율을 표시합니다.
* 매일 업데이트됩니다.
* 는 다운로드 가능합니다.
...
| 열 | 정의 |
|--- |--- |
| 보고서 세트 이름 | 보고서 세트에 대한 알기 쉬운 이름 |
| 모든 호출 수 (총 수의 %) | 현재 사용 기간에 발생한 모든 서버 호출입니다. |
...
| 포함된 보고서 세트 | 보고서 서버 사용량 페이지에서 선택한 모든 보고서 세트가 이 목록에 포함됩니다. |
