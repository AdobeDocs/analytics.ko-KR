---
description: 모든 서버 사용량 경고를 추가하거나 관리합니다. 경고를 설정하면 청구 회사의 모든 로그인 회사에 있는 모든 보고서 세트에 적용됩니다.
title: 서버 호출 사용량 경고
...
* 권한이 부여된 모든 서버 호출 유형에 대해 &quot;다음 이상&quot; 100%의 &quot;모든&quot; 서버 호출 사용량 또는
* 권한이 부여된 모든 서버 호출 유형에 대해 &quot;다음 이상&quot; 또는 &quot;90%&quot;인 &quot;모든&quot; 서버 호출 사용량
* 권한이 부여된 모든 서버 호출 유형에 대해 &quot;초과 또는 같음&quot; 75%의 &quot;임의&quot; 서버 호출 사용과 &quot;사용된 사용 기간&quot;이 사용 기간의 75% 미만이거나 같음&quot;
...
1. 경고를 정의합니다.
   ...
   * **시간 세부 기간**: 경고를 확인하는 빈도를 말합니다. *지금은 주별 세부 기간만 지원합니다.* 이는 경고가 주 단위로 확인되며, 현재 사용 기간의 데이터를 다시 확인한다는 의미입니다.
   * **수신자**: 경고가 지정된 임계 값을 트리거할 때 이메일을 받을 조직의 사용자를 지정합니다.
   * **만료 날짜**: 기본적으로 만료일은 경고 생성일로부터 1년입니다.
   * **다음의 경우에 경고 보내기**:

      * 다음 지표 중 하나 이상의 트리거
서버 호출/초 유형을 지표로 추가하고, 수정자 및 임계값을 선택하여 경고 임계값을 지정합니다.
         * 이(가) 다음 이상
         * 이(가) 다음 이하
      * 대상
체류 사용 기간에 대한 임계값 및 조건(다음 이상 또는 다음 이하)을 지정합니다.
...
1. 하나 이상의 경고 옆에 있는 확인란을 선택합니다. 경고 관리 작업이 맨 위에 표시됩니다.
1. 다음 작업 중 하나 이상을 완료합니다.

   | 작업 | 정의 |
   |--- |--- |
   | + 추가 | [+ 추가](/help/admin/tools/server-call-usage/scu-alerts.md)를 클릭하여 [!UICONTROL 경고 빌더]에 액세스합니다. |
   | 태그 | 경고에 태그를 지정하여 쉽게 사용할 수 있도록 구성합니다. |
   | 삭제 | 기본 경고를 제외한 모든 경고를 삭제할 수 있습니다. |
   | 이름 변경 | 기본 경고를 제외한 모든 경고의 이름을 변경할 수 있습니다. |
   | 승인 | 경고를 승인하여 &quot;공식&quot;으로 만듭니다. |
   | 활성화/비활성화 | 모든 경고(기본 경고 포함)를 활성화하거나 비활성화할 수 있습니다. |
   | 갱신 | 하나 이상의 경고를 선택한 경우 갱신할 수 있습니다. 이렇게 하면 원래 만료 날짜와 상관없이 [!UICONTROL 갱신]을 클릭한 날로부터 1년으로 만료 날짜가 연장됩니다. |
