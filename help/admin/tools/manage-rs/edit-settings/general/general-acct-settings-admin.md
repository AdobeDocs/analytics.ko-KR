...
| 시간대 | 보고서 데이터와 관련된 날짜와 시간을 결정합니다. 선택한 시간대가 일광 절약 시간제를 관찰하는 경우 시간별 데이터가 이를 반영하도록 자동으로 조정됩니다. 라이브 보고서 세트의 시간대를 변경하면 보고서 데이터에 스파이크나 갭이 만들어집니다. Adobe에서는 영향을 최소화하기 위해 피크 시간이 아닌 시간에 이 변경 작업을 수행할 것을 권장합니다. |
