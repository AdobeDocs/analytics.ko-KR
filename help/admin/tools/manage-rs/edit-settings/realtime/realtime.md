...
지속성 개념이 없으므로 eVar (전환 지표)는 지원되지 않는다는 것을 잊지 마십시오. 전환 지표를 선택할 수 있을 때 전환 지표는 차원과 동일한 페이지에서 설정되어 있는 경우에만 작동합니다. 자세한 내용은 [실시간 보고서 설정](/help/admin/tools/manage-rs/edit-settings/realtime/t-realtime-admin.md)에서 캡처한 경고 메시지를 참조하십시오.
