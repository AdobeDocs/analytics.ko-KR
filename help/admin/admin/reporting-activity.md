...
| 프로젝트 | 작업 공간 프로젝트, Report Builder 통합 문서 등 |
| 쿼리 | 현재 실행 중인 쿼리 수입니다. |
| 평균 대기 시간 | 실행 중인 모든 쿼리에 대한 평균 대기 시간입니다. |
...
| 열 | 설명 |
| --- | --- |
| 쿼리 ID | 문제 해결 용도로 사용할 수 있습니다. |
| 실행 시간 | 쿼리가 실행된 시간입니다. |
| 대기 시간 | 쿼리가 처리되기 전에 기다린 시간입니다. 일반적으로 용량이 충분하면 &quot;0&quot;에 있습니다. |
| 시작 시간 | 쿼리 처리가 시작된 시기(관리자의 현지 시간). |
| 애플리케이션 | Reporting Activity Manager에서 지원하는 응용 프로그램은 다음과 같습니다. <ul><li>Analysis Workspace UI</li><li>작업 공간 예약된 프로젝트</li><li>Report Builder</li><li>빌더 UI: 세그먼트, 계산된 지표, 주석, 대상 등</li><li>1.4 또는 2.0 API의 API 호출(5개의 동시 요청)</li><li>지능형 경고</li></ul> |
| 사용자 | 쿼리를 시작한 사용자입니다. |
| 프로젝트 | 작업 공간 프로젝트, Report Builder 통합 문서 등 |
| 월 경계 | 요청이 교차하는 월별 경계의 수입니다. 이로 인해 요청의 복잡성이 추가됩니다. |
| 열 | 요청의 복잡성을 측정하기 위한 Workspace의 지표 및 분류 수입니다. |
| 세그먼트 | 이 요청에 적용되는 세그먼트 수. 이로 인해 요청의 복잡성이 추가됩니다. |
| 상태 | 네 가지 가능한 상태 표시기는 다음과 같습니다. <ul><li>**빨간색 - [!UICONTROL 용량]**: 보고서 세트는 보고 용량 측면에서 제한을 받습니다. (95% 이상)</li><li>**노란색 - [!UICONTROL 거의 용량]**: 이 보고서 세트는 최대 용량(90% - 95%)에 도달할 위기에 있습니다.</li><li>**녹색 - [!UICONTROL 좋아요]**: 보고 용량은 충분합니다.</li><li>**[!UICONTROL 상태 보류 중]**: 상태를 사용할 수 없습니다.</li></ul> |
