...
| 매개 변수 | 설명 |
|--- |--- |
| d_rs | (레거시/추적 서버 기반 서버측 전달으로 설정) <br>히트와 함께 Analytics로 전달된 보고서 세트로 설정합니다. |
| d_dst_filter | (보고서 세트 기반 서버측 전달을 사용하여 설정) <br>히트와 함께 Analytics로 전달된 보고서 세트 ID로 설정합니다. |
...
| c_clientDateTime | dd/mm/yyy hh:mm:ss W TZ로 포맷된 타임스탬프    TZ는 분 단위이며 Date.getTimezoneOffset 메소드의 반환과 일치합니다. |
...
| c_latitude | 숫자 위도입니다. |
| c_linkClick | 옵션에는 다음이 포함됩니다.사용자 지정, 다운로드 종료 |
