...
이 상세 문서에서는 표준 eVar와 다르게 데이터를 처리하고 할당하는 머천다이징 eVar의 개념을 설명합니다. 또한 머천다이징 eVar가 제품 검색 방법과 어떻게 관련되는지를 설명합니다.
...
머천다이징 eVar의 만료 설정을 사용하면 제품/eVar 바인딩이 모두 만료되어야 하는 시점과 eVar이 이미지 요청에 전달된 후 post_evar 열을 더 이상 자동으로 채우지 말아야 하는 시기를 선택할 수 있습니다. eVar 만료은 성공 이벤트(선택한 항목)가 기록되거나 선택한 일정 기간(다시 선택한 기간)이 경과할 때 발생할 수 있습니다. Adobe Analytics에서는 eVar당 한 번에 하나의 만료 설정만 허용합니다.

제품 검색 방법 솔루션의 경우, 머천다이징 eVar의 만료를 설정하는 우수 사례는 장바구니에서 제품이 자동으로 제거되거나 구매 이벤트가 발생할 때 사이트의 장바구니에 담겨진 시간과 같아야 합니다. 두 만료 설정을 사용하면, 방문자가 구입하는 모든 제품은 해당 시점에 제품이 바인딩된 머천다이징 eVar 값에 주문/단위/매출 크레딧을 받게 됩니다.

### 유형

eVar 유형 설정은 eVar에 삽입되는 데이터 유형을 결정합니다. 대부분의 경우 - 전부는 아닐 경우 - 머천다이징 eVar을 설정하는 동안 이 값은 &quot;텍스트&quot;와 같아야 합니다. 머천다이징 eVar에 &quot;카운터&quot; 유형을 사용하는 것은 드물지만, 추적 요구 사항에 따라 효과적인 방식으로 성공 여부를 활용하여 제품별로 카운터 eVar 값에 할당할 수 있습니다.  &quot;카운터&quot; 유형을 사용하여 솔루션에 대해 논의하는 것은 이 문서의 범위를 벗어납니다.

### 머천다이징 바인딩 이벤트

머천다이징 바인딩 이벤트 설정을 사용하면 제품을 머천다이징 eVar의 값에 바인딩하는 조건을 지정할 수 있습니다. 이러한 조건은 특정 성공 이벤트 또는 eVar의 실행에만 제한됩니다. 트래픽 변수(예: prop)를 실행해도 머천다이징 바인딩에는 영향을 주지 않습니다.

머천다이징 바인딩 이벤트 설정에 대해 더 유용한 기능 중 하나는 두 개 이상의 이벤트를 통해 제품을 eVar 값에 바인딩하는 기능입니다. 예를 들어 이 설정을 사용하면 제품 보기 이벤트, 장바구니 추가 이벤트 또는 구매 이벤트를 통해 제품을 머천다이징 eVar 값에 바인딩할 수 있습니다. 이 설정은 제품과 동일한 이미지 요청에 다른 이벤트/eVar(머천다이징 또는 기타)가 포함될 때마다 및 기본적으로 수행되는 방식으로 머천다이징 eVar 값에 제품을 바인딩할 수도 있습니다.
...
사용 가능한 여러 설정 조합 중에서 &quot;우수 사례&quot;가 있는 설정이 궁금해질 수 있습니다.
...
예를 들어, 많은 의류 제품에는 크기, 색상, 스타일 및 기타 모든 속성을 지정하는 &quot;하위 SKU&quot;가 있습니다. 이러한 속성은 동일한 상위 제품에 속하는 다른 동위 제품과 단일 하위 제품을 구분합니다. 중간 크기의 파란색 티셔츠와 큰 빨간색 티셔츠를 구입하기로 결정한다고 가정해 보겠습니다. 두 셔츠 모두에 &quot;tshirt123&quot;의 상위 제품 ID가 있으며 eVar10이 하위 SKU를 캡처하도록 구성되어 있다고 가정합니다. 구매 확인 페이지에 설정된 변수는 다음과 같이 설정됩니다.
...
제품 상호 작용이 발생할 때마다 제품 검색 방법 eVar 값을 제품에 바인딩하려고 하므로 - 제품이 &quot;발견됨&quot;임을 의미하므로 발생할 수 있는 가장 일반적인 &quot;검색된 제품&quot; 상호 작용이 제품 보기(방문자가 제품 세부 사항 페이지로 이동할 때) 또는 장바구니 추가(방문자가 제품 검색 방법 페이지에서 직접 장바구니에 제품을 추가할 때)라고 가정하는 것이 안전합니다.  따라서 이러한 두 이벤트(prodView, scAdd)를 &quot;기본적인&quot; 머천다이징 바인딩 이벤트로 선택할 수 있습니다.
이러한 결합 이벤트가 이미지 요청 내에 포함될 때마다 동일한 요청(제품 변수 내)에 포함되어 있고 머천다이징 eVar에 아직 바인딩되지 않은 모든 제품 ID는 post_evar 열에 포함됨)에 머천다이징 eVar으로 전달된 가장 최근 값에 바인딩됩니다. 이 원래 바인딩이 수행된 후 이러한 제품을 다시 바인딩하려는 시도는 할당(바인딩) 설정이 &quot;원래 값(처음)&quot;으로 설정되어 있으면 무시됩니다.

### 우수 사례 설정

모범 사례 설정은 다음과 같습니다. 가장 강력한 결과 세트를 사용하여 가능한 한 쉽게 제품 검색 방법을 구현합니다. Adobe은 클라이언트가 각 제품 검색 방법 머천다이징 eVar(일반적으로)를 다음과 같이 구성할 것을 권장합니다.
...
post_evar 열을 고려할 때 Adobe 처리 서버는 이 히트를 다음과 같이 봅니다.
...
post_products 열에 포함된 값이 익숙할 수 있습니다. 이 문서에서 위로 스크롤하여 아래에 표시된 대로 이 post_products 값과 s.products 값을 비교합니다.  post_products 열이 제품 변수 구문을 사용하여 설정된다는 것을 알 수 있습니다. 즉, 바인딩은 전환 변수 구문 eVar 값을 제품 구문을 통해 제품 변수로 &quot;복사&quot;합니다. 이 복사 작업은 제품 변수와 바인딩 이벤트(eVar 구성을 통해 설정)가 동일한 요청에 포함되어 있는 경우에만 수행됩니다. 이 시점에서 post_eVar 열에 포함된 값은 제품에 바인딩됩니다. 이 바인딩은 post_products 열에 저장된 제품 구문을 통해 표시됩니다.
