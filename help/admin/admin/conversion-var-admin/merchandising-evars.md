...
머천다이징 eVar를 사용하면 *방문당/주문당* 수준 대신. *제품당* 수준에서 eVar가 캡처한 값에 성공적인 활동을 할당할 수 있습니다.
...
서로 관련하여 모든 제품 검색 방법의 성능을 측정하기 위해 추가 eVar를 사용할 수 있습니다. 위에서 설명한 검색 방법 외에도, eVar은 외부 웹 사이트의 제품 세부 사항 페이지에 대한 링크와 같이 비교에서 다른 검색 방법을 포함합니다.
...
앞에서 언급했듯이 전환 변수 구문이 있는 모든 머천다이징 eVar에는 &quot;가장 최근 (마지막)&quot; 할당만 있습니다. 따라서 머천다이징 eVar에 대한 할당 설정은 방문자가 사이트를 계속 사용할 때 post_evar 열에 삽입되는 값을 판별하지 않습니다. 오히려, 이 설정은 제품에 바인딩되는 eVar 값과 이러한 제품이 바인딩된 eVar 값에 성공 이벤트를 다시 할당하는 방법을 결정합니다.

머천다이징 eVar의 할당(즉, 바인딩) 설정이 &quot;원래 값(첫 번째)&quot;과 동일하게 설정되면 다음이 발생합니다. post_evar 열과 함께 설정되며 이전에 post_evar 열의 해당 &quot;사전 처리된&quot; eVar에 바인딩되지 않은 모든 제품은 post_evar 열에 포함된 값에 바인딩됩니다.  eVar 값과 제품 간의 이 바인딩은 보고서 세트 설정의 &quot;다음 시기 이후에 만료&quot; 설정에 따라 eVar이 만료될 때까지 변경되지 않습니다.
...
* 사이트에서 자동으로 장바구니에서 제품을 제거하기 전에 사이트의 장바구니에 제품을 보관하는 시간(예: 14일, 30일 등)
...
productID 12345과 동시에 캡처되는 모든 성공 이벤트(장바구니 추가, 구매)는 제품 ID 12345 및 &quot;internal 키워드 검색&quot;의 eVar1 값에 모두 반영됩니다. 다른 eVar1 값이 제품 ID 12345과 연결된 성공 이벤트에 대한 크레딧을 받는 유일한 방법은 나중에 eVar1이 products 변수 내에서 다른 값으로 설정된 경우(제품 ID 12345 함께) 입니다.
...
이 변수 설정은 제품 ID 12345의 바인딩을 &quot;internal 키워드 검색&quot;의 eVar1 값에서 &quot;internal campaign&quot;의 eVar1 값으로 변경합니다. 또한 eVar이 &quot;가장 최근(마지막)&quot;의 제품 구문 및 할당(바인딩) 설정을 사용하도록 구성된 경우 이러한 다시 바인딩 변경이 발생합니다. 할당(바인딩) 설정이 대신 &quot;원래 값(첫 번째)&quot;으로 설정된 경우 제품 ID 12345과 함께 eVar1을 &quot;내부 캠페인&quot;과 동일하게 설정하면 제품 ID 12345을 &quot;내부 캠페인&quot;의 eVar1 값에 다시 바인딩하지 않습니다. 대신 바인딩은 원래 바인딩된 값인 &quot;내부 키워드 검색&quot;으로 유지됩니다.
...
* 검색 방법 페이지에서 제품 검색 방법 세부 사항(위에 표시)을 제품 세부 사항 페이지로 전달하고
* 이전 페이지에서 방금 전달한 항목에서 동일한 제품 변수 값을 다시 만듭니다.
...
가장 최근 (마지막)&quot;의 할당(바인딩) 설정을 사용하여 추가 문제가 발생할 수 있습니다. 많은 웹 탐색 경험에서 방문자는 이미 보거나 장바구니에 추가한 제품을 &quot;다시 찾기&quot;합니다. 이러한 일은 일반적으로 후속 방문을 통해 또는 구매를 완료하기로 결정하기 직전에 발생합니다. 사이트를 방문하는 동안 방문자가 &quot;sandal123&quot; 제품을 &quot;sandal&quot;의 키워드 검색을 통해 찾는다고 가정해 봅시다. 키워드 검색 결과 페이지에서 장바구니에 바로 추가합니다. 장바구니 추가를 캡처하는 코드는 다음과 같이 설정됩니다.
...
이제 방문자가 이 방문 중에 제품을 구매하지 않지만 3일 후 장바구니에 여전히 있는 &quot;sandboys123&quot; 제품을 사용하여 사이트를 다시 방문한다고 가정해 보십시오. 방문자는 구매하기 전에 제품에 대해 자세히 알고 싶어합니다. 방문자는 키워드 검색을 사용하여 제품을 찾는 대신 사이트를 탐색합니다. &quot;제품을 다시 찾기&quot; 바로 전에 &quot;womens > shoes > sandboards&quot; 머천다이징 검색 섹션에 이 종말이 됩니다. &quot;sandal123&quot; 제품에 대한 제품 세부 사항 페이지를 &quot;다시 찾기&quot;할 때, 변수는 다음과 같이 설정됩니다(페이지 로드 시).
