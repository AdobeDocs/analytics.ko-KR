---
description: 검색 방법 페이지에서는 다양한 검색 방법 보고서가 사이트에서 전환 성공 이벤트에 대한 크레딧을 받는 방법을 식별합니다. 예를 들어 검색 엔진이 구매를 수행하는 방문자를 사이트에 참조하는 경우 검색 방법은 검색 엔진이 참조에 대한 크레딧을 받는 방법을 지정합니다.
title: 검색 방법
topic: Admin tools
...
검색 방법 페이지에서는 다양한 검색 방법 보고서가 사이트에서 전환 성공 이벤트에 대한 크레딧을 받는 방법을 식별합니다. 예를 들어 검색 엔진이 구매를 수행하는 방문자를 사이트에 참조하는 경우 검색 방법은 검색 엔진이 참조에 대한 크레딧을 받는 방법을 지정합니다.
...
>[!NOTE] 모든 검색 방법은 방문이 종료될 때 만료됩니다. 다른 이벤트(예: 장바구니 체크아웃) 후에 만료되도록 선택하면 방문 중에 장바구니 체크아웃이 발생할 때 검색 방법이 만료됩니다. 방문 중에 장바구니 체크아웃이 발생하지 않으면 방문이 종료될 때 검색 방법이 만료됩니다.
