...
* **데이터 수집 관리자:** 이 사용자는 데이터 수집 UI(Experience Platform Launch UI의 구 명칭)에서 퍼블리싱 권한, 컨테이너 만들기, 사용자 권한 등 모든 항목에 대해 완전한 액세스 권한을 보유합니다. 이러한 사용자가 반드시 프로그래머일 필요는 없지만 적어도 HTML, CSS 및 JavaScript에 대한 초보 지식이 있는 것이 유용합니다. 이러한 사용자는 조직의 웹 사이트 소유자와 협력하여 Experience Platform 태그가 사이트에 구현되도록 하는 책임을 담당합니다. 조직 구현을 담당할 최소한 한 명 이상의 사용자를 지정하십시오. 이 사용자가 다른 데이터 수집 관리자를 초대할 수 있습니다.
