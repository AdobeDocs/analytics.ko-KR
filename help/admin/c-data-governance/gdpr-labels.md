...
<table id="table_663EFF43A454498386F7F3E60875E0F8"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>이 레이블이 있는 필드는 모든 데이터 개인 정보 요청에 대해 반환됩니다. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_0972910DB2D7473588F23EA47988381D"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>ID-DEVICE/ID-PERSON </p> <p>DEL-DEVICE/DEL-PERSON </p> </td> 
   <td colname="col4"> <p>ID 또는 DEL 레이블(없음으로 설정)은 제거할 수 없지만, 사용자 지정 ID 구현에 따라 DEVICE 또는 PERSON 변형으로 변경할 수 있습니다. </p> <p>사용자 지정 방문자 ID 파섹 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" morerows="1"> 
...
데이터 개인 정보 삭제 요청에 대한 Adobe Analytics 지원은 보고에 대한 영향을 최소화하도록 설계되었습니다. 대부분의 경우 보고서에 표시된 지표는 변경되지 않아야 합니다. 데이터 개인 정보 삭제 전에 실행된 내역 보고서는 삭제 후 실행된 동일한 보고서와 일치합니다. 이 작업은 보고된 값이 일관되게 유지될 수 있도록 식별할 수 없는 데이터를 그대로 두면서 데이터 주체에서 삭제된 데이터를 완전히 분리하여 수행됩니다.

다음 표에서는 다양한 변수가 "삭제"되는 방식을 설명합니다. 이는 완전한 목록이 아닙니다.
...
<table id="table_A329C2E2645F4685BC208826D070A5F6"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>기존 값은 "Data Privacy-356396D55C4F9C7AB3FBB2F2FA223482" 형식의 새 값으로 대체됩니다. 여기서 "Data Privacy-" 접두어 뒤의 32자리 16진수 값은 암호로 강력한 12 8비트 의사 난수. 기본적으로 임의의 문자열로 대체되기 때문에 이 새 값에서 원래 값을 결정할 방법이 없으며 원래 값을 판별하는 새 값을 도출하는 방법이 없습니다. </p> <p>지정된 변수의 경우, 대체되는 값과 동일한 값이 동일한 데이터 개인 정보 보호 요청의 일부로 삭제되는 다른 히트 내에서 발생하는 경우 해당 값의 모든 인스턴스가 동일한 새 값으로 대체됩니다. </p> <p>값의 일부 인스턴스가 하나의 삭제 요청으로 대체되고 이후 요청이 원래 값의 다른(새) 인스턴스를 삭제하는 경우 새 대체 값은 원래 대체 값과 달라집니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>구매 ID </p> </td> 
   <td colname="col2"> <p>기존 값은 "G-7588FCD8642718EC50" 형식의 새 값으로 대체됩니다. 여기서 "G-" 접두어 뒤의 18자리 16진수는 암호 강력한 128비트 의사 난수 중 처음 18자리입니다. 트래픽 및 상거래 변수의 삭제에 적용되는 모든 주석도 여기에 적용됩니다. </p> <p>구매 ID는 트랜잭션 ID로, 누군가가 구매 확인 페이지를 새로 고침하는 경우와 같이 구매가 두 번 결제되지 않도록 하는 것이 주 목적입니다. ID 자체는 구매가 기록된 자신의 DB 행에 구매를 연결할 수 있습니다. 대부분의 경우 이 ID를 삭제할 필요가 없으므로 기본적으로 삭제되지 않습니다. 데이터 개인 정보 삭제 요청 이후 구매를 사용자에게 다시 연결할 수 있는 경우, 이 방문자에 대한 Analytics 데이터를 구매자에게 연결할 수 없도록 이 필드를 삭제해야 할 수 있습니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>값이 지워집니다(변수의 유형에 따라 빈 문자열 또는 0으로 설정). </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_6FECF3D654514862912D371E6BE4143B"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Adobe Advertising Cloud ID 파섹 방문자 ID 및 MCID와 마찬가지로 쿠키에서 채워집니다. 다른 ID가 삭제될 때마다 히트에서 삭제해야 합니다. 자세한 내용은 해당 변수에 대한 설명을 참조하십시오. </p> </td> 
  </tr> 
 </tbody> 
</table>
