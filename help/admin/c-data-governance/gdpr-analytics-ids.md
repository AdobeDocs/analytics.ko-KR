...
어떤 레이블에 어떤 변수/필드에 적용되어야 하는지 파악하려면 먼저 Analytics 데이터에서 캡처하는 ID를 이해하고 데이터 개인 정보 보호 요청에 사용할 레이블을 결정해야 합니다. 데이터 개인 정보는 ID로 간주될 수 있는 범위의 범위를 확장합니다. ID는 직접 식별 가능한 ID(ID 레이블: I1)와 간접 식별 가능한 ID(ID 레이블: I2)의 광범위한 두 클래스로 구분됩니다.
...
* 시스템 내에서 ID 중 하나(예: 이메일 주소)를 다른 ID(예: CRM ID)에 매핑할 수 있습니다. 그런 다음 일관성을 위해 데이터 개인 정보 처리 시 데이터 개인 정보 요청에 대해서만 CRM ID를 사용하기로 결정합니다.
...
이 표를 사용하여 데이터 개인 정보 요청을 Analytics에 제출할 때 사용할 ID 유형을 결정합니다. 이 정보를 알고 있으면 변수에 사용해야 하는 다른 레이블을 쉽게 확인할 수 있습니다.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>일부 고객은 ID를 <a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/props_eVars.html" format="html" scope="external">사용자 지정 트래픽 변수(props) 또는 사용자 지정 전환 변수(eVars)</a>에 배치합니다. 가장 일반적인 것은 CRM ID이지만 이메일 주소, 사용자 로그인 이름, 고객 충성도 번호 또는 이러한 값의 해시를 포함합니다. </p> 
    <ul id="ul_0B9492CF786046BB97E31CCF83A85FEA"> 
     <li id="li_D35B61CC6A8B485A8E09358A46D3F598">데이터 개인 정보 요청에 이러한 ID 중 하나를 사용하려면 ID-PERSON 레이블을 포함하는 필드에 지정해야 합니다. </li> 
    ...
    </ul> <p> 보고서 세트에 데이터 개인 정보 요청에 대한 데이터 대상을 식별하는 데 사용할 ID가 들어 있는 사용자 지정 변수가 하나 또는 두 개 이상 있는 경우는 드물다. I1 또는 I2 레이블이 지정된 변수가 여러 개 있을 수 있지만, 일반적으로 이 중 한두 개에만 ID-PERSON 또는 ID-DEVICE 레이블이 있습니다. </p> </td> 
  </tr> 
  <tr> 
...
다른 레이블이 들어 있는 필드는 거의 없지만, 일반적으로 많은 필드에 ACC 레이블이 있습니다. 적절한 액세스 레이블은 데이터 개인 정보 요청에 사용 중인 ID에 따라 달라집니다.
