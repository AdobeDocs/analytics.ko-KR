...
Adobe Cloud Platform(ACP)은 브랜드의 데이터 거버넌스 인프라를 소비자 경험을 만들고 관리하는 데 사용하는 Adobe 툴과 연결하는 통합 솔루션을 제공합니다. Adobe Cloud Platform의 데이터 거버넌스 기능을 사용하면 데이터 거버넌스 정책을 데이터 사용과 직접 연결할 수 있습니다.

Familiarize yourself with [how Adobe Analytics handles GDPR](https://www.adobe.com/kr/data-analytics-cloud/analytics/general-data-protection-regulation.html) which discusses steps for GDPR readiness and how to integrate with the Adobe Experience Cloud GDPR API.
...
이를 위해 Adobe Analytics는 사용자가 데이터 제어자로서 Analytics 보고서 세트와 그러한 보고서 세트의 모든 차원과 지표에 대한 [개인 정보 레이블](/help/admin/c-data-governance/gdpr-labels.md#data-governance-labels)을 설정할 수 있는 데이터 거버넌스 사용자 인터페이스를 제공합니다. 직접 식별 가능한 데이터 또는 간접적으로 식별 가능한 데이터가 포함된 데이터 세트의 열을 식별할 수 있으므로 해당 데이터를 처리하기 위해 액세스 및 삭제 요청을 제출할 수 있습니다. 각 요청에 대해 Analytics 데이터 거버넌스 사용자 인터페이스에 정의된 레이블은 해당 요청에 해당하는 특정 식별자에 대해 적용됩니다.
...
* GDPR 삭제 및 액세스 요청을 적용할 수 있도록 각 보고서 세트에 대한 데이터 유지 정책을 설정합니다.
   ...
   > [!NOTE] Adobe Analytics에 데이터 유지 기간을 설정하지 않은 경우 Adobe Analytics에서는 GDPR API에 대한 요청 처리, 즉 최종 사용자로부터 받은 액세스 또는 삭제 요청 처리를 지원할 수 없습니다. 데이터 유지 기간을 설정하려면 고객 성공 관리자에게 문의하십시오.

* 권한 확인:adobe Analytics에서 데이터 거버넌스 관리 인터페이스를 사용하려면 Adobe Analytics 관리자여야 합니다.
