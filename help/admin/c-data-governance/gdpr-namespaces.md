...
뿐만 아니라 다른 Adobe `“namespaceId”: 10` 제품이 해당 양식을 `“namespace”: “AAID”` 사용하는 것을 확인할 수도 있습니다.
...
다음을 사용할 수도 있습니다. `“namespaceId”: 4` 이 외에도 `“namespace”: “ECID”` 다른 Adobe 제품이 해당 양식을 사용할 수도 있습니다.
...
사용자 지정 트래픽 또는 전환 변수 (prop 또는 evar) 의 ID에 대해 변수에 ID-device 또는 id-person 레이블을 사용하여 레이블을 지정한 다음 해당 유형의 ID에 고유한 네임스페이스 이름을 지정합니다. [변수에 ID-DEVICE 또는 ID-PERSON으로 레이블을 지정할 때 네임스페이스 제공](gdpr-labels.md)을 참조하십시오.
...
> [!TIP] ID-장치 또는 ID-PERSON 레이블을 적용할 때 네임스페이스가 지정되지 않은 경우, 변수의 친숙한 이름 (보고 UI에 표시된 이름) 또는 변수 번호 (evar 12와 같이) 를 사용하여 GDPR API에 네임스페이스를 사용하지 마십시오. 친숙한 이름이 아닌 네임스페이스를 사용하면 동일한 사용자 ID 블록을 사용하여 여러 보고서 세트에 올바른 변수를 지정할 수 있습니다. 예를 들어, ID가 일부 보고서 세트의 다른 evar에 있거나 친숙한 이름이 일치하지 않는 경우 (예: 친숙한 이름이 특정 보고서 세트에 대해 현지화된 경우)
