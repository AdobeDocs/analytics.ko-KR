---
description: 검색할 수 있는 각 ID에는 네임스페이스가 할당되어 있습니다. 네임스페이스는 모든 보고서 세트에서 해당 ID가 사용되는 모든 변수에서 해당 ID를 식별하는 사용자 지정 문자열입니다.
title: 네임스페이스
...
검색할 수 있는 각 ID에는 네임스페이스가 할당되어 있습니다. 네임스페이스는 모든 보고서 세트에서 해당 ID가 사용되는 모든 변수에서 해당 ID를 식별하는 사용자 지정 문자열입니다.

네임스페이스 문자열은 ID를 데이터 개인 정보 보호 요청의 일부로 제공할 때 검색할 필드를 식별하는 데 사용됩니다. 데이터 개인 정보 보호 요청이 제출되면 이 요청에는 요청에 사용할 데이터 주체 ID를 지정하는 JSON 섹션이 포함됩니다. 데이터 주체의 단일 요청의 일부로 여러 ID를 포함할 수 있습니다. JSON에는 다음이 포함됩니다.

* 네임스페이스 문자열을 포함하는 &quot;namespace&quot; 필드.
* 대부분의 Adobe Analytics 요청에 대해 값 &quot;analytics&quot;가 포함된 &quot;type&quot; 필드입니다.
* Analytics가 각 보고서 세트의 연결된 네임스페이스 변수에서 검색해야 하는 ID를 포함하는 &quot;값&quot; 필드
...
값은 대시로 구분된 두 개의 16진수 숫자로 지정해야 합니다. 영문자인 모든 16진수 숫자는 대문자를 사용하여 지정해야 합니다. 16진수 값에는 선행 0이 없어야 합니다(선행 0이 필요한 더 이상 사용되지 않는 양식에 지정된 것과 동일한 값의 차이).
...
이 값은 16자리 16진수 숫자 2개 또는 19자리 10진수로 지정해야 합니다. 숫자는 대시, 밑줄 또는 콜론으로 구분해야 합니다. 숫자에 충분한 자리가 없는 경우 앞에 0을 추가해야 합니다.
...
네임스페이스는 사용자 지정 방문자 ID에 대해서도 미리 정의됩니다.
...
이전에 다른 변수 또는 보고서 세트에 대해 정의한 네임스페이스를 보고 이러한 네임스페이스 중 하나를 재사용할 수 있으므로 해당 유형의 ID를 저장하는 모든 보고서 세트에 동일한 네임스페이스를 쉽게 사용할 수 있습니다. 보고서 세트 내의 여러 변수에 동일한 네임스페이스를 할당할 수도 있습니다. 예를 들어 일부 고객은 트래픽 변수 및 전환 변수(페이지에 따라, 경우에 따라 하나 또는 둘 다에 있음)에 CRM ID를 저장하고, 두 변수 모두에 &quot;CRM ID&quot; 네임스페이스를 할당할 수 있습니다.
