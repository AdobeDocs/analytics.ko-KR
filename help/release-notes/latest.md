...
| 공지 | 추가 또는 업데이트 일자 | 설명 |
| ----------- | ---------- | ---------- |
| **구매 ID 및 이벤트 ID(이벤트 일련화)의 37개월 만료** | 25,2023년 5월 | 릴리스가 타깃팅된 Analytics 히트 처리 엔진의 향후 릴리스 **2023년 6월 말 또는 2023년 7월 초**&#x200B;는 구매 ID 및 이벤트 ID(이벤트 일련화)의 37개월 만료를 적용합니다. 현재 구매 ID 및 이벤트 ID는 Adobe Analytics에서 만료되지 않습니다. 구매 ID 또는 이벤트 ID가 확인/사용되면, 그 이후 히트는 언제가 되었는지에 관계없이 해당 구매 또는 이벤트를 중복으로 표시합니다. 새로운 처리 엔진 릴리스를 통해<ul><li>구매 ID 및 이벤트 ID는 항상 37개월 후에 만료됩니다.</li><li>구매 ID 또는 이벤트 ID를 본 지 37개월이 경과하면 더 이상 중복 구매 또는 이벤트로 간주되지 않습니다.</li><li> 37개월 이상 전의 구매 ID 또는 이벤트 ID를 &quot;재사용&quot;하는 경우, 더 이상 중복으로 간주되지 않습니다.</li></ul> |
