...
| **대/소문자를 구분하지 않는 다중 값 변수** | 대/소문자를 구분하지 않는 다중 값 변수의 경우 다음 변수에 저장된 값 `mvvar1` - `mvvar3` 데이터 피드에서 더 이상 자동으로 소문자가 지정되지 않습니다. 대신 데이터 피드(및 Analytics 소스 커넥터를 통해 Adobe Experience Platform 및 CJA로 전달된 데이터)는 페이지에서 전달된 원래 사례를 반영합니다. | 2022년 10월 24일 |
