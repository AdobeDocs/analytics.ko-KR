...
| **웹 SDK 마이그레이션을 위한 워크플로우 개선** | 이제 Adobe Experience Platform 데이터 수집은 데이터 개체의 많은 필드를 Adobe Analytics에 직접 자동으로 매핑합니다. 이렇게 개선된 워크플로우는 다음과 같은 이점을 제공합니다.<ul><li>따라서 조직에서는 웹 SDK를 만들 염려가 없고 [!UICONTROL XDM 스키마]. 다음을 참조하십시오 [데이터 개체 변수 매핑](https://experienceleague.adobe.com/en/docs/analytics/implementation/aep-edge/data-var-mapping) 추가 정보. (참조할 추가 설명서 링크)</li><li>Web SDK로 마이그레이션한 후 조직은 Adobe Analytics에서 Customer Journey Analytics으로 마이그레이션하는 것이 더 좋습니다. 이는 웹 SDK를 통해 보다 원활하게 Customer Journey Analytics으로 마이그레이션할 수 있기 때문입니다.</li></ul> (이에 대한 자세한 정보와 기타 마이그레이션 옵션이 곧 제공될 예정입니다.) |  | 2024년 4월 |
