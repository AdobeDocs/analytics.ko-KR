...
* 사용자 지정 이벤트가 로드되지 않는 문제가 해결되었습니다. (AN-324163)
...
| 공지 | 추가 또는 업데이트 일자 | 설명 |
| ----------- | ---------- | ---------- |
| **구매 ID 및 이벤트 ID의 37개월 만료(이벤트 일련화)** | 2023년 7월 10일 | 에 릴리스된 Analytics 히트 처리 엔진의 최신 릴리스 **2023년 7월 13일**&#x200B;에서 구매 ID 및 이벤트 ID(이벤트 일련화)의 37개월 만료 적용을 시작했습니다. 이전에는 Adobe Analytics에서 구매 ID 및 이벤트 ID가 만료되지 않았습니다. 구매 ID 또는 이벤트 ID가 확인/사용되면 그 이후에 발생할 모든 히트에 해당 구매 또는 이벤트가 중복으로 표시되었습니다. 새로운 처리 엔진 릴리스:<ul><li>구매 ID 및 이벤트 ID는 항상 37개월 후에 만료됩니다.</li><li>구매 ID 또는 이벤트 ID를 조회한 후 37개월이 경과한 경우에는 더 이상 중복 구매 또는 이벤트로 간주되지 않습니다.</li><li> 37개월 이전의 구매 ID 또는 이벤트 ID를 “재사용”하는 경우, 더 이상 중복으로 간주되지 않습니다.</li></ul> |
