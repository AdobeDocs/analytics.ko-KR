...
| **DMA용 Google에 필요한 동의 플래그에 대한 분석 지원** | 새로운 유럽 개인 정보 보호 규정으로 인해 Google은 유럽에서 수집된 데이터가 전송되면 두 가지 특정 유형의 동의 여부를 표시해야 합니다. **3월 6일 시작**, Google은 관련 동의가 부여되었음을 표시하지 않는 이벤트 데이터를 더 이상 수락하지 않습니다. Adobe Analytics은 3월 중순에 이 데이터 캡처를 위한 지원을 출시할 예정입니다. 자세한 내용은 곧 알려 드리겠습니다. |  | 2024년 3월 중순 |
...
| **Activity Map은 웹 SDK에 대해 더 적은 서버 호출을 사용합니다** | 현재 Activity Map 링크 이벤트는 자체 이벤트로 계산되며 추가 비용이 발생합니다. <p>이 향상된 기능을 통해 일부 링크 이벤트를 가져와 다음 히트로 패키지화합니다. 이는 AppMeasurement에서 이벤트를 처리하는 방법과 유사합니다.</p> |  | 2024년 4월 3일 목요일 |
...
| 공지 | 추가 또는 업데이트 일자 | 설명 |
| ----------- | ---------- | ---------- |
| **Adobe API 개체 멤버 추가** | 2024년 1월 17일 | 언제든지 Adobe는 버전 관리에 대한 공지나 변경 없이 기존 API 오브젝트에 선택적 요청 및 응답 멤버(이름/값 쌍)를 추가할 수 있습니다. Adobe에서는 이러한 추가 사항이 이해되지 않는 경우 처리 시 무시되도록 Adobe API와 통합하는 서드파티 도구의 API 설명서를 참조할 것을 권장합니다. 제대로 구현된 경우 이들 추가 사항은 구현을 위해 중단하는 변경 사항입니다. Adobe는 릴리스 정보를 통해 제공하는 표준 알림 없이 매개변수를 제거하거나 필수 매개변수를 추가하지 않습니다. |
