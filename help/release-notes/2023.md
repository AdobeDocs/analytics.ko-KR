...
| **에 대한 이름 지정 규칙이 업데이트되었습니다. [브라우저](../components/dimensions/browser.md) 차원** | 2023년 1월 17일 | Chrome, Edge 및 Firefox 브라우저용 버전 110부터 회사 이름이 더 이상 브라우저 앞에 표시되지 않습니다. 이 경우 브라우저 이름을 기반으로 하는 세그먼트 정의가 있는 경우 보고서에 영향을 줄 수 있습니다. 예를 들어 &quot;브라우저에 Google&quot;가 포함되어 있다는 정의를 포함하는 세그먼트는 버전 110부터 Chrome 브라우저를 더 이상 식별하지 않습니다. 또한 Chrome 110부터 모바일 버전과 비모바일 버전 모두 동일한 레이블이 지정됩니다. 이전에는 모바일과 모바일이 아닌 버전에 개별적으로 레이블이 지정되었습니다. 모바일 세분화 및 분류 를 사용하여 모바일과 비모바일 트래픽을 비교할 수 있습니다. |
