...
트래픽 소스에서는 덮어쓰기 또는 방문 종료 중 먼저 발생하는 시점까지 모든 보고서 값이 지속되고 크레딧을 받습니다. 이전에는 키워드와 참조 도메인만 지속되었습니다. 예를 들어 방문자가 Google 검색을 수행하여 $100 구매를 위해 사이트를 방문하게 *`DVD`**`DVD`* 되면 보고서는 키워드와 Google 검색 엔진에도 $100 크레디트를 할당합니다. 이 기능은 관리자 설정에 관계 없이 변경되지 않습니다.
