...
크로스 디바이스 분석(CDA)은 Analytics를 디바이스 중심 보기에서 사람 중심 보기로 변환하는 기능입니다. 그 결과 분석가는 브라우저, 디바이스 또는 앱을 넘나드는 사용자 행동을 이해할 수 있습니다. Adobe에서는 결정론적 일치만 사용하여 장치를 함께 연결하는 [필드 기반 결합](field-based-stitching.md)을 지원합니다.
...
디바이스가 결합되면 변수 지속성이 디바이스 간에 전달됩니다. 예를 들어 사용자가 먼저 데스크탑 컴퓨터에서 광고를 통해 사이트를 방문합니다. 해당 사용자는 모바일 앱을 찾아 설치한 다음 결국 모바일 디바이스에서 구매합니다. Cross-Device Analytics를 사용하면 모바일 디바이스의 매출을 데스크탑 컴퓨터에서 클릭한 광고에 연결할 수 있습니다.
...
교차 장치 분석을 사용하려면 [필드 기반 결합](field-based-stitching.md)이 필요합니다.
...
크로스 디바이스 분석은 획기적이고 강력한 기능이지만 사용 방법에 대한 제한이 있습니다. [필드 기반 결합](field-based-stitching.md)에도 고유한 제한 사항이 있습니다.
...
* 크로스 디바이스 분석은 가상 보고서 세트 및 보고 시점 처리를 사용하는데, 여기에는 자체 제한 사항도 있습니다. 예를 들어 현재 마케팅 채널 변수는 지원되지 않습니다. 이러한 제한 사항에 대한 자세한 내용은 [가상 보고서 세트](/help/components/vrs/vrs-about.md) 및 [보고서 처리 시간](/help/components/vrs/vrs-report-time-processing.md)을 참조하십시오.
* 비공개 그래프에서는 CX Enterprise 및 Adobe Analytics에서 발견된 [고객 특성](https://experienceleague.adobe.com/ko/docs/core-services/interface/services/customer-attributes/attributes) 기능에서 사용하는 ID 동기화와 동일한 ID 동기화를 사용합니다. 그러나 크로스 디바이스 분석 가상 보고서 세트(비공개 그래프나 필드 기반 결합에 따름)는 고객 특성 기능과 호환되지 않습니다. 즉, 고객 속성 기반 차원은 크로스 디바이스 분석 가상 보고서 세트와 함께 사용할 수 없습니다.
...
* 가상 보고서 세트의 내역 데이터는 Adobe가 디바이스를 인식하고 함께 결합하는 방식에 따라 변경됩니다. 소스 보고서 세트의 데이터는 변경되지 않습니다.
* 결합된 데이터에는 8~12시간의 지연이 있습니다.
* 특정 디바이스에 대한 내역 데이터 매핑은 최대 1년 동안 저장됩니다.
* 1년 이내에 디바이스가 매우 높은 매핑 내역 항목 수에 도달하면 매핑 내역은 잘립니다. 정확한 제한은 사용한 결합 옵션에 따라 다릅니다.
