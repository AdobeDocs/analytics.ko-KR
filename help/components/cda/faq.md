...
CDA의 장치 간 스티칭은 두 개의 동시 프로세스에서 발생합니다.

* 첫 번째 프로세스를 &quot;라이브 스티칭&quot;이라고 합니다. 이 프로세스는 데이터가 Adobe Analytics으로 스트리밍될 때 발생합니다. 라이브 스티칭하는 동안 CDA는 데이터를 개인 수준에서 재설정하는 데 최선을 다합니다. 하지만 라이브 스티칭할 때 개인을 알 수 없는 경우 CDA는 방문자 ID로 다시 폴링되어 방문자를 나타냅니다.

* 두 번째 프로세스를 &quot;다시 보기&quot;라고 합니다. 재생하는 동안 CDA는 시간을 거꾸로 하여 가능한 경우 지정된 전환 창 내에서 내역 데이터를 재설정합니다. 이 조회 창은 CDA를 구성하도록 요청한 방법에 따라 1일 또는 7일입니다. 재생하는 동안 CDA는 이전에 알려지지 않은 히트 수를 재구성하려고 시도합니다.
