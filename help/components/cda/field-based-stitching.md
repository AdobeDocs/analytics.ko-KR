...
* 사용자가 로그인하거나 이메일을 여는 경우와 같이 가능할 때마다 개인을 고유하게 식별하는 prop이나 eVar을 구현이 설정해야 합니다. 이 요구 사항은 모바일 앱 (사용하는 경우)을 비롯한 모든 플랫폼에 적용됩니다. 필드 기반 결합에 대해 프로비저닝되면 원하는 식별 변수를 계정 관리자에게 알려 줍니다.
