...
| 쿼리 문자열 매개 변수 | 사이트의 페이지 URL이 `https://example.com/?page=12345&cat=1`과 같은 모습이라면 “page”와 “cat”은 모두 검색어 문자열 매개 변수입니다.  (참조 `https://en.wikipedia.org/wiki/Query_string`.)  규칙 세트당 하나의 쿼리 문자열 매개 변수만 지정할 수 있습니다. 쿼리 문자열 매개 변수를 더 추가하려면 연산자로 `ANY`를 사용한 다음, 새 쿼리 문자열 매개 변수를 규칙에 추가하십시오. 쿼리 문자열 매개 변수는 대소문자를 구분하지 않습니다. 예를 들어 “cat”와 “CAT”는 동일한 방식으로 평가됩니다. |
