...
>[!NOTE] 최적의 보고 성능을 보장하기 위해 20개의 최신 달력 이벤트가 추세 및 횟수 초과 그래프에 표시됩니다.

시간에 따른 트렌드 보고서의 경우 이벤트를 그래픽으로 표시하고 캠페인 또는 기타 이벤트가 사이트 트래픽, 매출액 또는 기타 지표에 영향을 미치는지 여부를 확인할 수 있습니다. 달력 이벤트를 설정하는 프로세스에는 이벤트 이름 지정, 날짜 설정, 메모 추가 및 아이콘 설정이 포함됩니다. 예를 들어, Social을 사용하는 경우 달력 이벤트를 만들어 사이트 트래픽의 증가와 Twitter를 사용하는 유명인의 제품 언급 간에 상관 관계가 있음을 보여줄 수 있습니다.
...
1. 클릭 **[!UICONTROL Add New]**.
   ...
   1. 를 **[!UICONTROL Event Date]**&#x200B;지정합니다.
   1. Choose the **[!UICONTROL Report Suite]** to which this event applies.
   1. (선택 사항) 이 이벤트에 **[!UICONTROL Note Text]** 대한 항목을 추가합니다.
...
1. (선택 사항) 캘린더 이벤트를 사용자에게 적용합니다. 이제 공유됨으로 표시된 달력 이벤트에는 다른 사용자에게 &quot;전달&quot;하는 옵션이 있습니다. 달력 이벤트를 적용하면 이 이벤트가 받는 사용자의 트렌드 및 시간별 그래프에 나타납니다.
   1. 클릭 **[!UICONTROL Push To Users]**.
   1. Click **[!UICONTROL Check All]** or check the box next to the user name/s.
   1. 클릭 **[!UICONTROL Save]**.
   이렇게 하면 달력 이벤트를 훨씬 더 유용하게 사용할 수 있습니다. 이제 애널리스트는 오버레이를 사용자의 보고서에 강제로 적용하여 더 많은 컨텍스트를 제공할 수 있습니다.
1. Click **[!UICONTROL Analytics]** > **[!UICONTROL Components]** > **[!UICONTROL Calendar Events]**.
1. 아래에서 다음 옵션 **[!UICONTROL Shared Calendar Events]**&#x200B;중 하나를 지정합니다.
