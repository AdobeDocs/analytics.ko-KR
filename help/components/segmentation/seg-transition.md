...
세그먼트를 삭제할 때 이 세그먼트가 적용된 예약된 보고서 및 대시보드는 정상적으로 계속 작동합니다(즉, 세그먼트 또는 대시보드는 삭제된 세그먼트를 계속 사용함).
...
이제 세그먼트가 여러 보고서 세트에서 작동하므로 이름이 같은 세그먼트가 여러 개 있을 수 있습니다. 다음을 수행해야 합니다.
...
* &quot;새 차원 및 이름이 변경된 차원&quot;을 참조하십시오(아래 참조).
...
| 모바일 운영 체제 (더 이상 사용되지 않음) | 모바일 OS | 대신 운영 체제 차원을 사용하고 모바일 디바이스 방문 세그먼트를 적용합니다. |
