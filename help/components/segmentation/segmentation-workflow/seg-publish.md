...
Adobe Analytics 세그먼트를 CX Enterprise에 게시할 수 있습니다. 따라서 Audience Manager 및 Advertising, Target, Campaign을 포함한 기타 활성화 채널에서 마케팅 활동에 세그먼트를 사용할 수 있습니다.

Analytics 세그먼트를 8시간 이내에 CX Enterprise에 게시할 수 있습니다. 이러한 세그먼트를 사용하여 Audience Manager의 대상자를 모든 다운스트림 대상으로 활성화합니다.
...
* 이 세그먼트를 저장할 보고서 세트가 [CX Enterprise에서 사용](/help/components/segmentation/segmentation-workflow/seg-publish.md)되도록 설정되어 있는지 확인하십시오. 그렇지 않으면 CX Enterprise에 게시할 수 없습니다.
...
* **멤버십 제한**: Adobe Analytics에서 CX Enterprise로 공유하는 대상은 2천만 명의 고유 구성원을 초과할 수 없습니다.
* **데이터 개인정보**: 대상자는 방문자의 인증 상태에 따라 필터링되지 않습니다. 방문자는 인증되지 않음 및 인증됨 상태의 사이트를 검색할 수 있습니다. 방문자가 인증되지 않은 경우 발생하는 작업으로 인해 여전히 방문자가 대상에 포함될 수 있습니다. 대상을 공유할 때 파생되는 전반적인 개인 정보 문제를 이해하려면 [Adobe CX Enterprise 개인 정보 보호](https://www.adobe.com/kr/privacy/experience-cloud.html)를 검토하십시오.
* [!DNL Adobe Analytics]과(와) Audience Manager **의 세그먼트 간**&#x200B;차이점에 대한 논의는 [Analytics 및 Audience Manager의 세그먼트 이해](/help/integrate/c-audience-analytics/aam-analytics-segments.md)를 참조하십시오.

## 세그먼트 게시 타임라인

| 사용 가능한 사항 | 사용 가능한 시기 | 사용 가능한 위치 |
|---|---|---|
| 메타데이터 (세그먼트 제목 및 정의) | 게시 후 즉시 | Audience Manager, CX Enterprise Audience Library, Target |
| 멤버십이 있는 사용 가능한 세그먼트 | 게시 후 8시간까지 | Audience Manager의 방문자 프로필 뷰어 |
| 트레이트 및 멤버십 인구 | 24-48시간 이내 | Audience Manager |
...
| 요소 | 설명 |
|---|---|
| **[!UICONTROL 이 세그먼트를 Experience Cloud에 게시(*보고서 세트*용)]** | 이 옵션이 활성화되면 세그먼트 제목 및 정의가 즉시 CX Enterprise와 공유되는 반면 세그먼트 멤버십은 4시간마다 평가되고 공유됩니다. <br> 예를 들어 해당 대상이 Target의 활동과 연결되면 [!DNL Analytics]은 해당 CX 엔터프라이즈 및 Target 대상의 자격을 규정하는 방문자용 ID를 전송하기 시작합니다. 이때 대상 이름 및 해당 데이터가 CX Enterprise의 [!DNL Audience Library] 페이지에 표시되기 시작합니다. </br> |
...
| **[!UICONTROL 대상자 라이브러리에서 만들기]** | 만들고 게시하는 세그먼트는 CX Enterprise의 [!DNL Audience Library] 페이지에서 지연 없이 사용할 수 있도록 할 수 있습니다. 이러한 세그먼트는 Analytics 업데이트에 종속되지 않으며, 75개의 게시된 세그먼트라는 제한 계산에 포함되지 않습니다. |
| **[!UICONTROL 75개 중 x개가 게시됨]** | CX Enterprise에 게시한 세그먼트 수입니다. 게시된 세그먼트 목록과 관련 보고서 세트 및 소유자를 보려면 링크를 클릭하십시오. |
...
1. **[!UICONTROL 게시됨]** 열을 봅니다. 이 열의 **[!UICONTROL 예]**&#x200B;는 세그먼트가 CX Enterprise에 게시되었음을 나타냅니다. **[!UICONTROL 아니요]**&#x200B;는 세그먼트가 게시되지 않았음을 나타냅니다.

## Audience Manager UUID 검색
...
### 방법 1: Adobe CX 엔터프라이즈 디버거 사용

1. Chrome 웹 스토어에서 [Adobe CX Enterprise Debugger](/help/implement/validate/debugger.md)를 다운로드하여 설치합니다.
...
1. **[!UICONTROL 대상]** 페이지에서 CX Enterprise에서 가져온 대상을 찾습니다. 이러한 대상은 Target 활동에서 사용할 수 있습니다.
