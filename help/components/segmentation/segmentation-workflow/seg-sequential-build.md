...
예를 들어, 한 식당에서는 기본 랜딩 페이지를 피하고 주문 페이지로 직접 이동하는 단골 사용자를 확인하려고 합니다. 이 예는 방문자가 페이지 A를 방문하지 않고 페이지 B로 바로 이동하는 것을 방지하기 위해 단순화할 수 있습니다.
...
예를 들어, 의류점은 제품 페이지를 열람했지만 이후에 장바구니를 방문하지 않은 모든 방문자를 확인하려고 합니다. 이 예는 페이지 A로 이동한 다음 현재 또는 다음 방문에서 페이지 B로 이동하지 않도록 방문자에게 단순화할 수 있습니다.
...
예, 가능합니다. 이 방문자 세그먼트를 **페이지 A를 본 다음 B, C 및 D라는 각 페이지를 본 방문자**&#x200B;로 식별하려고 한다고 가정합니다.

이 세그먼트를 논리 그룹 컨테이너 없이 만들 수는 있지만 그렇게 하는 것은 복잡하고 힘든 일입니다. 방문자가 볼 수 있는 모든 페이지 시퀀스를 지정해야 합니다.
