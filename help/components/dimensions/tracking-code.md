...
추적 코드 차원은 사이트에 있는 추적 코드의 이름을 나열합니다. 인터넷의 서로 다른 위치에 서로 다른 쿼리 문자열 매개 변수 값이 있는 링크를 배치할 수 있습니다. 이 차원은 트래픽을 사이트로 유도하는 데 가장 성공한 링크를 이해하는 데 도움이 됩니다.

추적 코드 쿼리 문자열 추가는 조직에서 사용하는 이메일, 광고, 소셜 미디어 게시물 및 기타 마케팅 활동에 공통으로 사용됩니다.
...
차원 항목에는 사이트의 추적 코드 이름이 포함됩니다. 조직은 사용자가 사용하려는 특정 차원 항목을 파악합니다. 자세한 내용은 [캠페인 추적](/help/implement/use-cases/campaign-tracking.md) 추가 정보.
