...
[!UICONTROL 단일 페이지 방문] [지표](overview.md)은(는) 전체 방문에 대해 [페이지](../dimensions/page.md) 차원 항목에 단일 고유 값만 포함된 방문 횟수를 보여줍니다. 이 지표는 짧은 방문을 보고 싶지만 [[!UICONTROL 바운스]](bounces.md)만큼 엄격한 규칙이 없는 차원의 컨텍스트에서 유용합니다.
