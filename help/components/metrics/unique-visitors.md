...
Analysis Workspace는 보고서의 세부 기간을 기준으로 고유 방문자를 처리합니다. 예를 들어 [일](../dimensions/day.md) 차원을 사용하는 경우 각 차원 항목에 대한 일별 고유 방문자 수를 보게 됩니다. 하지만 보고서 합계의 경우에는 자유 형식 테이블의 날짜 범위에 대해 중복이 제거된 고유 방문자 수입니다.

## 이 지표의 계산 방법

이 지표는 특정 차원 항목에 대한 고유 방문자 ID의 수를 계산합니다. Adobe Analytics가 고유 방문자를 식별하는 방법에 대한 자세한 내용은 [방문자 식별 개요](/help/implement/id/overview.md)를 참조하십시오.
