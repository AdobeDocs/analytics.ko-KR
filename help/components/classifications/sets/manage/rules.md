...
## 규칙 집합 인터페이스 {#rule-set-interface}

>[!CONTEXTUALHELP]
>id="classificationsets_rules_samplekeys"
>title="샘플 키"
>abstract="규칙 세트를 테스트하려면 테스트 키를 입력하거나 붙여넣습니다. 각 행은 별도의 키 값입니다. 결과가 포함된 대화 상자를 표시하려면 **[!UICONTROL 규칙 세트 테스트]**&#x200B;를 선택합니다."
...
키 차원 내부 캠페인의 값이 `em`(예: `em:FY2025:Summer Sale`)으로 시작하는 경우 `Email`을(를) **[!UICONTROL 채널]** 분류에 대한 값으로 할당하는 규칙을 정의하려고 합니다.
...
키 차원 내부 캠페인에 대한 값에 `2025`이(가) 포함된 경우 `2025`을(를) **[!UICONTROL 년]** 분류에 값으로 할당하는 규칙을 정의하려고 합니다(예: `em:Summer Sale:FY2025`).
...
키 차원 내부 캠페인에 대한 값에 `Winter`이(가) 포함된 경우 `Winter Sale`을(를) **[!UICONTROL Type]** 분류에 값으로 할당하는 규칙을 정의하려고 합니다(예: `fb:Winter:FY2024`).
...
`:` **[!UICONTROL 토큰]**&#x200B;을(를) 기반으로 주요 차원 내부 캠페인의 값을 **[!UICONTROL 채널]**, **[!UICONTROL 유형]** 및 **[!UICONTROL 연도]** 분류로 분할하는 규칙을 정의하려고 합니다.
...
* 사용자가 `Cowboys Fantasy Tony Romeo`을(를) 검색할 때 `Fantasy`이(가) **[!UICONTROL Type]**(으)로 분류됩니다.
* 사용자가 `Cowboys vs. Broncos`을(를) 검색할 때 `Team`이(가) **[!UICONTROL Type]**(으)로 분류됩니다.
