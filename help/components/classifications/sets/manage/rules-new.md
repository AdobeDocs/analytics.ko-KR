...
분류 세트의 컨텍스트 내에서 규칙을 정의합니다. 이는 분류 세트를 구독한 모든 보고서 세트 및 키 차원 조합에 규칙이 적용(활성화 시)됨을 의미합니다. 이 구현은 기존 분류 규칙 빌더가 작동하는 방식과는 다소 다릅니다. 분류 규칙 빌더에서 하나 이상의 규칙을 규칙 세트의 일부로 별도로 정의한 다음 규칙 세트를 하나 이상의 보고서 세트와 연결합니다. 새 인터페이스에서는 분류 세트 내의 규칙을 규칙 세트라고도 하지만 다른 분류 세트 속성을 구성하는 동일한 인터페이스 내에서 정의됩니다.
...
1. **[!UICONTROL 분류 집합: _분류 집합 이름_]**대화 상자에서&#x200B;**[!UICONTROL 규칙]**탭을 선택합니다.
   ...
   * 분류 세트에 대해 처음으로 **[!UICONTROL 규칙]** 인터페이스에 액세스하거나 지금까지 기존 규칙 빌더 인터페이스를 계속 사용하기로 결정했다면 시작하는 방법을 선택할 수 있는 대화 상자가 표시됩니다. 옵션은 다음과 같습니다.
      ...
      * **기존 규칙을 마이그레이션**. 현재 분류 규칙을 가져오고 새 인터페이스에서 이러한 규칙을 계속 사용합니다. 기존 규칙은 유지되고 새 형식으로 변환됩니다.
...
규칙을 만들거나 편집할 때 규칙 세트 인터페이스를 사용합니다.
...
| 5 | **[!UICONTROL 테스트 옵션]** | 샘플 키 차원 값을 사용하여 분류를 테스트합니다. <ul><li>**[!UICONTROL 샘플 키]** 텍스트 영역에 값을 추가하거나 붙여 넣으십시오.<br/>규칙 집합 인터페이스의 다양한 사용에서 샘플 키를 유지하려면 **[!UICONTROL 샘플 키 기억]**&#x200B;을 선택하십시오.</li><li>**[!UICONTROL 규칙 집합 테스트]**&#x200B;를 선택하여 규칙 집합을 테스트합니다.</li></ul> |
...
| 2 | 선택한 함수에 대한 입력입니다. 입력은 선택한 함수에 따라 다릅니다. 예를 들어 정규 표현식 함수의 경우 입력은 정규 표현식이고 분할 함수의 경우 입력은 토큰입니다. 특정 기능에 대한 적절한 입력을 입력합니다. 예를 들어, 내부 캠페인 코드에서 세 개의 분류를 식별하는 정규 표현식의 경우 `^(.+)\:(.+)\:(.+)$`입니다. |
...
키 차원이 끝나는 특정 값을 기준으로 분류를 설정합니다.
...
키 차원이 일치하는 특정 값을 기준으로 분류를 설정합니다.

+++ 세부 사항 

#### 필수 입력

**[!UICONTROL 일치]**&#x200B;에 대한 값을 입력하십시오. 예: `em:FY2025:Summer`.
...
키 차원 값이 여러 규칙과 일치하고 규칙 세트에 분류 설정 작업이 동일한 규칙이 포함되어 있는 경우 마지막 규칙이 분류에 대한 값을 결정합니다. 따라서 가장 중요한 분류 설정 작업의 등급을 규칙 세트의 마지막 규칙의 일부로 지정해야 합니다.

동일한 분류 설정 작업을 공유하지 않는 규칙을 여러 개 만드는 경우 처리 순서는 문제가 되지 않습니다.
...
* 사용자가 `Cowboys vs. Broncos`을(를) 검색하는 경우 `Team`이(가) **[!UICONTROL Type]**(으)로 분류됩니다.
