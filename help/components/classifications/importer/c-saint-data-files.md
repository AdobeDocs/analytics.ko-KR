...
* v2.1 파일 형식이 지정되고 셀이 적절히 [이스케이프되는](/help/components/classifications/importer/t-classifications-escape-data.md) 경우 탭, 줄바꿈, 따옴표와 같은 특수 문자가 셀 내부에 임베드될 수 있습니다. 특수 문자에는 다음이 포함됩니다.
...
* 보고서 데이터를 분류하는 데에는 문자 제한이 적용됩니다. 예를 들어 제품 이름이 100자(바이트)를 초과하는 제품(*`s.products`*)에 대한 분류 텍스트 파일을 업로드하는 경우, 해당 제품은 보고에 표시되지 않습니다. 추적 코드와 모든 사용자 정의 전환 변수(eVars)는 255바이트를 허용합니다. 또한 이 정책은 동일한 255바이트 제한이 적용되는 분류 및 하위 분류 열 값으로 확장됩니다.
...
**EVENT:** 보고서의 라인 항목당 지정된 사용자 정의 이벤트가 발생한 횟수에 값을 곱합니다.
...
>[!IMPORTANT]
>새 분류 아키텍처에 대해 활성화된 보고서 세트에는 이 옵션을 사용할 수 없습니다.

1. [!DNL Analytics]에 로그인하고 관리 > 분류로 이동합니다.
