...
초기 설정은 몇 개의 행을 다시 분류하거나 행을 추가하는 대신 큰 원본 데이터 세트로 분류 데이터베이스를 채우거나 분류를 다시 구성합니다.

보고서 세트(주어진 변수 또는 보고서의 경우)에서 초기 업로드에 이어 후속 가져오기에서 새 행과 업데이트된 행만 업로드하는 것이 좋습니다. 변경되지 않는 행은 이후 업로드에서 생략해야 합니다.

업로드하는 각 새 키 값은 해당 월의 해당 변수에 대한 고유값에 대해 계산됩니다.

해당 월의 고유 값을 초과한 경우 보고에서 고유 값 초과에 대한 해당 분류 데이터가 표시되지 않습니다. Data Warehouse에서 이러한 분류를 확인할 수 있습니다.

>[!NOTE]
>
>분류 데이터 파일을 처리하는 데 필요한 시간은 파일 크기와 Adobe 서버에서 이미 처리 중인 파일의 현재 수에 따라 다릅니다. 데이터 파일 처리에 걸리는 시간은 보통 72시간 이내입니다.
...
1. FTP 계정 세부 사항을 지정합니다.
   ...
   | **보고서 세트 선택** | 선택한 데이터 세트를 분류할 보고서 세트를 선택합니다. 여러 개의 보고서 세트를 선택하려면 선택한 보고서 세트의 각 분류가 일치해야 합니다. |
   | **충돌 시 데이터 덮어쓰기** | 중복 데이터를 덮어쓰려면 이 옵션을 선택합니다. 이 옵션은 기존 분류를 업데이트하는 경우 유용합니다. [최신 분류 아키텍처](../sets/overview.md)를 사용하는 경우 이 설정은 항상 활성화되어 있습니다. |
   ...
   | **승인** | (필수) 새 FTP 계정으로 전송된 모든 데이터 파일을 Adobe에서 자동으로 가져오도록 승인합니다. |
...
>[!NOTE]
>
>가져오기로 인해 분류가 변경되지 않으면 알림이 전송되지 않습니다. 이메일은 가져오기가 성공하고 분류가 변경된 경우에만 전송됩니다.
...
1. FTP 액세스 정보(호스트, 로그인, 암호)를 사용하여 원하는 FTP 클라이언트로 FTP 서버에 액세스합니다.
...
Adobe은 정기적으로 FIN 파일이 연결된 업로드된 데이터 파일을 검색합니다. Adobe은 보고서 세트를 FTP 계정 구성에 지정된 보고서 세트 및 데이터 세트로 가져옵니다.
