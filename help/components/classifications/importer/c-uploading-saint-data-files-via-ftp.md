...
다음의 권장 제한 사항이 중요합니다. 너무 많음

>[!IMPORTANT]
>
>너무 많은 작은 파일 또는 단일 큰 파일이 있으면 처리 서버에서 불필요한 처리 로드가 발생할 수 있습니다. Adobe은 큰 파일을 50MB 청크로 분할하고 작은 파일을 함께 결합하는 것을 권장합니다.
...
1. FTP 계정 세부 사항을 지정합니다.
   ...
   | **충돌 시 데이터 덮어쓰기** | 이 옵션을 선택하여 중복 데이터를 덮어씁니다. 이 옵션은 기존의 분류를 업로드하려는 경우에 유용합니다. 을 사용하는 경우 [최신 분류 아키텍처](../sets/overview.md)이면 이 설정은 항상 활성화되어 있습니다. |
   | **가져오기가 완료된 이후** | 가져오기가 완료된 이후 이 FTP 계정에 관한 알림을 받을 수 있도록 이메일 주소를 지정한 경우 이 옵션을 선택하면 업데이트된 데이터 세트를 동일한 FTP 계정으로 자동으로 내보냅니다. 을 사용하는 경우 [최신 분류 아키텍처](../sets/overview.md)로 지정하는 경우 이 옵션을 사용할 수 없습니다. |
...
Adobe Analytics이 FTP 폴더에 업로드된 읽기 및 처리 파일을 가지고 있으면 FTP 사이트에서 파일이 자동으로 제거됩니다.
