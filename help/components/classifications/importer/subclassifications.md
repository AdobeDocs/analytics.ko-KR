...
이 모델의 각 분류는 독립적이며 선택한 보고 변수에 대한 새 하위 보고서에 해당합니다. 또한 각 분류는 데이터 파일에서 분류 이름을 열 머리글로 하여 하나의 데이터 열을 구성합니다. 예:
...
**하위 분류:**&#x200B;하위 분류는 변수 대신 상위로 다른 분류를 가지는 모든 분류입니다. 하위 분류는 상위 분류에 대한 추가 정보를 제공합니다. 예를 들어 [!UICONTROL 캠페인] 분류에는 캠페인 소유자 하위 분류가 있을 수 있습니다. [!UICONTROL 숫자] 분류도 분류 보고서에서 지표로 작동합니다.

각 분류(상위 또는 하위)는 데이터 파일에서 하나의 데이터 열을 구성합니다. 다음 이름 지정 형식을 사용하는 하위 분류에 대한 열 머리글:
...
>[!NOTE]
>
>제품 분류 데이터는 제품과 직접적으로 관련된 데이터 속성으로 제한됩니다. 데이터는 웹 사이트에서 제품이 분류되거나 판매되는 방식에 제한되지 않습니다. 판매 카테고리, 사이트 검색 노드 또는 판매 항목과 같은 데이터 요소는 제품 분류 데이터가 아닙니다. 오히려 이러한 요소는 보고서 전환 변수에 캡처됩니다.
...
|---|---|---|---|---|---|---|
| 410390013 | 폴로 | 남성용 폴로 셔츠, 반팔 (M,01) | M | M | 01 | 돌 |
| 410390014 | 폴로 | 남성용 폴로 셔츠, 반팔 (L,03) | M | L | 03 | 헤더 |
| 410390015 | 폴로 | 여성용 폴로 셔츠, 긴팔 (S,23) | F | S | 23 | 아쿠아 |

### 제품 분류 - 다중 파일 (파일 1) {#section_A99F7D0F145540069BA4EEC0597FF13F}

| 키 | 제품 이름 | 제품 세부 사항 | 성별 | 크기 | 코드 |
|---|---|---|---|---|---|
| 410390013 | 폴로 | 남성용 폴로 셔츠, 반팔 (M,01) | M | M | 01 |
| 410390014 | 폴로 | 남성용 폴로 셔츠, 반팔 (L,03) | M | L | 03 |
| 410390015 | 폴로 | 여성용 폴로 셔츠, 긴팔 (S,23) | F | S | 23 |
