...
**분류 세트**&#x200B;는 데이터를 분류하는 기본 방법입니다. **분류 규칙** 및 **분류 가져오기 도구**&#x200B;는 데이터를 분류하는 이전 방법입니다. 이 방법은 더 이상 사용되지 않으며 2026년 8월 31일 이후 더 이상 액세스할 수 없습니다.

* **분류 세트**: 간소화된 단일 인터페이스로 분류를 만들고 관리합니다.
* **분류 규칙**(이전): 분류 차원 항목에 주어진 차원 항목을 할당하는 규칙을 만듭니다. 이러한 데이터 분류 방법은 차원에 새 고유 값이 자주 발생하거나 빈번한 수동 분류가 부담이 되는 경우 가장 적합합니다.
* **분류 가져오기 도구**(이전): 각 행의 차원 항목이 포함된 템플릿 스프레드시트를 내보냅니다. 열은 차원에 대한 각 분류를 나타냅니다. 이러한 데이터 분류 방법은 모든 차원 항목이 알려진 경우 가장 적합하며, 자주 업데이트할 필요가 없습니다.
