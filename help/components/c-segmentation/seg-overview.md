...
* 특성을 기반으로 하는 방문자: 브라우저 유형, 장치, 방문 수, 국가, 성별.
* 상호 작용을 기반으로 하는 방문자: 캠페인, 키워드 검색, 검색 엔진.
* 종료 및 입장 기준 방문자: Facebook, 정의된 랜딩 페이지, 참조 도메인의 방문자.
...
세그먼트는 속성 또는 사이트와의 상호 작용에 따라 방문자를 필터링하는 조건을 설정합니다. 세그먼트에서 조건을 설정하려면 방문자 특성 및/또는 탐색 트레이트에 따라 방문자를 필터링하기 위한 규칙을 설정합니다. 방문자 데이터를 더 자세히 분류하려면 각 방문자에 대한 특정 방문 및/또는 페이지 보기 히트별로 필터링할 수 있습니다. 세그먼트 빌더는 이러한 하위 집합을 만들고 중첩된 계층적 방문자, 방문 또는 히트 컨테이너로 규칙을 적용하는 간단한 아키텍처를 제공합니다.
...
세그먼트 데이터의 모든 분류에는 적용되는 범위가 있습니다. Most breakdowns are based on *Page Views*, however, many valuable segments are based on the *Visit* container, and to a lesser degree the *Visitor* container. 컨테이너 범위를 기반으로 보고를 이해하는 것이 중요합니다.
