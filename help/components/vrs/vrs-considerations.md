...
요약 데이터 소스를 사용하면 집계된 지표를 보고서 세트 수준의 Adobe Analytics로 가져올 수 있습니다. 요약 데이터 소스 업로드는 *방문자 ID 없이*&#x200B;집계된 지표를 포함하므로 [!UICONTROL 방문] and [!UICONTROL 방문자] 컨테이너로 세그먼트화할 수 없습니다. VRS는 세그먼테이션을 사용하여 작동하므로 세그먼트가 방문 또는 방문자 컨테이너를 사용하여 빌드된 경우 요약 데이터 소스를 사용하여 가져온 데이터는 가상 보고서 세트에서 사용할 수 없습니다.
