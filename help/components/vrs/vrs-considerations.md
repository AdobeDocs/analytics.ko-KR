...
다중 세트 태깅을 글로벌 보고서 세트 및 VRS로 대체하면 Adobe Analytics 구현을 단순화하고 서버 호출을 줄일 수 있으므로 모범 사례로 권장됩니다. 그러나 VRS에는 몇 가지 고려해야 할 중요한 제한 사항이 있습니다. 다음 지침은 글로벌 보고서 세트를 기반으로 구축된 가상 보고서 세트를 구현하는 것이 적합한 방법인지 결정하는 데 도움이 됩니다.
