...
요약 데이터 소스를 사용하면 집계된 지표를 보고서 세트 수준의 Adobe Analytics로 가져올 수 있습니다. 요약 데이터 소스 업로드는 방문자 ID *없이 집계된 지표*&#x200B;을 포함하므로 [!UICONTROL 방문] 및 [!UICONTROL 방문자] 컨테이너에서 세그먼트화할 수 없습니다. VRS는 세그먼테이션을 사용하여 작동하므로 세그먼트가 방문 또는 방문자 컨테이너를 사용하여 작성된 경우 요약 데이터 소스를 사용하여 가져온 데이터는 가상 보고서 세트에서 사용할 수 없습니다.

요약 데이터 소스는 히트 컨테이너를 사용하고 해당 히트 컨테이너에 데이터 소스 정보를 포함하도록 설정된 규칙이 있는 경우 가상 보고서 세트에 표시됩니다.
...
2. 가상 보고서 세트가 올바로 설정되었는지 확인한 후 구현에서 보조 보고서 세트 ID를 제거합니다. 보조 보고서 세트를 제거하려면 다음을 수행하십시오.
   * Adobe Experience Platform의 데이터 수집 UI에서 더 이상 사용하지 않을 보고서 세트 옆에 있는 &#39;x&#39;를 클릭합니다.
