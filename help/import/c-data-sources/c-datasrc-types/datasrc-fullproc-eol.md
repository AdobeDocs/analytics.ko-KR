...
몇 년 동안 전체 처리 데이터 소스를 사용하면 히트 수준 데이터를 Adobe Analytics에 제출할 수 있습니다. 이 데이터는 JavaScript 라이브러리 및 모바일 앱 SDK를 통해 수집된 데이터와 동일한 방식으로 처리되었습니다. 2020년에 Adobe은 [대량 데이터 삽입 API](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md)는 전체 처리 데이터 소스와 동일한 기능을 수행하지만 추가 기능을 사용하여 수행합니다. 이 항목에서는 대량 데이터 삽입 API에서 제공하는 추가 기능에 대한 세부 사항을 제공하고 파일 형식의 차이점을 간략하게 설명합니다.

2021년 3월 25일부터 Adobe에 새 전체 처리 데이터 소스 연결이 생성되지 않았습니다. 기존 연결은 서비스가 2022년 1월 31일에 완전히 종료될 때까지 지원되었습니다. 표준 설명서 외에 Adobe에서는 [대량 데이터 삽입 API를 통해 데이터를 제출하는 데 필요한 절차](https://adobe.ly/aabdia).
...
* 대량 데이터 삽입은 방문자 ID에 대한 여러 옵션을 지원합니다. Analytics ID와 Marketing Cloud ID를 모두 제출할 수 있습니다( [ID 서비스](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=ko-KR) 추가 정보). 또한 고유한 ID를 [ECID 생성을 위한 시드](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md#customer-id-and-experience-cloud-visitor-id-seeds).
* 대량 데이터 삽입은 목록 및 컨텍스트 데이터 변수를 지원합니다.
* 대량 데이터 삽입은 Activity Map 데이터를 지원하지 않습니다.

## 파일 형식 및 콘텐츠의 주요 차이점

* 대량 데이터 삽입에는 몇 가지 추가 필수 필드가 있습니다. 자세한 내용은 [설명서](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md) 자세한 내용
* 방문자 연속성 및 속성을 보장하기 위해 대량 데이터 삽입을 위해서는 파일 내의 행을 시간 순서대로 정렬해야 합니다. 자세한 내용은 [방문자 그룹](https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/bdia.md#visitor-groups) 를 추가하여 파일 간 방문자 활동 순서 지정에 대해 알아봅니다.
...
| cookiesEnabled | cookiesEnabled | `Y` 또는 `N` 방문자가 자사 세션 쿠키를 지원하는 경우 |
...
| events | events | [이벤트 문자열](https://experienceleague.adobe.com/docs/analytics/implementation/vars/page-vars/events/event-serialization.html?lang=en#vars)를 채울 수 있습니다. 예: scAdd,event1,event7 |
| hierN | hierN, 즉 `<hier2>`...`</hier2>` | 계층 이름. 최대 5개의 계층을 가질 수 있습니다( hier1 - hier5 ). 기본 계층 이름을 지정할 수 있습니다 `hier2` 또는 친숙한 이름(Yankees)입니다. |
...
| marketingCloudVisitorID | 지원되지 않음 | Marketing Cloud ID. 자세한 내용은 [방문자 식별](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=en#id-service-api) 및 Marketing Cloud 방문자 ID 서비스 |
...
| timestamp | 날짜 | YYYY-MM-DDThh의 ISO 8601 날짜 형식 사용:mm:ss UTC_offset(예: 2021-09-01T12):00:00-07:00 ) 또는 Unix 시간 형식(1970년 1월 1일 이후 경과한 시간(초)입니다. |
| trackingServer | 지원되지 않음 | 열 헤더를 통해서만 제공할 수 있습니다. |
| transactionID | 지원되지 않음 | 보고 목적으로 다중 채널 사용자 활동을 연결하는 데 사용되는 공통된 값입니다. 자세한 내용은 [Data Sources 사용 안내서](https://experienceleague.adobe.com/docs/analytics/import/data-sources/datasrc-home.html?lang=en#data-sources). |
| userAgent | 지원되지 않음 | 사용자 에이전트 문자열 |
| visitorID | visitorID | 방문자의 Analytics ID. 자세한 내용은 [방문자 식별](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=en). |
| zip | zip | 전환 우편번호. |
