...
전체 처리는 이벤트 목록에서 직접 숫자/통화/카운터(1개 이상) 이벤트 값을 제외한 이전 이벤트 목록 형식만 지원하며, 이것은 `"eventNN=#.##"`이 아닌 `"eventNN,eventKK"`입니다. 즉, 카운터 이벤트는 데이터 소스 파일의 이벤트 열로 전달되고 1씩 증가하는 경우에만 지원됩니다.
