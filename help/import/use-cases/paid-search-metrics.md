...
지침을 보려면 데이터 소스 마법사로 돌아가 FTP를 통해 파일을 업로드합니다.
...
| 평균 주문 가격 | 매출/주문 | 통화 | 주문당 평균 매출을 표시합니다. |
...
| 노출 횟수당 이익 (PPI) | (매출 - 비용)/노출수 | 통화 | 광고가 표시될 때마다 비용과 견주어 얼마나 많은 매출이 발생했는지 보여 줍니다. |
| 광고 투자 수익률(ROAS) | 판매액/광고비 | 통화 | (ROI) 해당 광고에 지출된 달러당 벌어들인 달러를 나타냅니다. |

## 보고서 구성 및 실행

마지막 단계는 데이터 소스 지표와 계산된 지표를 추적 코드 보고서에 추가하고 캠페인을 드릴다운하여 각 광고 그룹의 실적을 즉시 확인하는 것입니다.
