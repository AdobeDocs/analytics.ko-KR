...
이 용어집을 사용하여 Adobe Analytics에서 사용하는 많은 용어의 컨텍스트를 파악합니다.
...
* **** ASI 슬롯:더 이상 존재하지 않습니다. 이전 버전의 Adobe Analytics에서 ASI 슬롯은 세그먼트화된 데이터를 보기 위한 임시 보고서 세트 컨테이너를 제공했습니다. 현재 버전의 Adobe Analytics에서 세그먼트를 모든 보고서에 즉시 적용할 수 있습니다.
* **** 분류:다른 차원의 컨텍스트 내에서 차원을 볼 수 있도록 해줍니다. 분석 [사용 안내서에서 차원](/help/analyze/analysis-workspace/components/dimensions/t-breakdown-fa.md) 분류를 참조하십시오.
...
* **** 채널:참조:
   * 사이트 섹션 차원을 채우는 채널 변수. 구현 [사용 안내서의 페이지 변수를](/help/implement/vars/page-vars/page-variables.md) 참조하십시오.
   * 마케팅 채널 - 사용자가 사이트에 도착하는 방법을 이해하는 데 도움이 되는 구성 요소입니다. 구성 [요소](/help/components/c-marketing-channels/c-getting-started-mchannel.md) 사용 안내서의 마케팅 채널을 참조하십시오.
...
* **** 만료:전환 변수 컨텍스트에서 백엔드 값이 지속되는 기간 이 지속성을 사용하면 이벤트가 이벤트의 히트 전에 변수 값과 연결할 수 있습니다. See [Conversion variables](/help/admin/admin/conversion-var-admin/conversion-var-admin.md) in the Admin user guide.
...
* **** 발생 횟수:차원 값이 설정되거나 지속된 히트 수를 보여주는 지표 유형입니다. 구성 [요소](/help/components/c-variables/c-metrics/metrics-occurrences.md) 사용 안내서의 발생을 참조하십시오.
* **** Omniture:더 이상 용어로 사용되지 않습니다. 2009년 Adobe에 인수되기 전에 Adobe Analytics를 소유한 조직
* **** 경로 지정:흐름을 참조하십시오.
* **** 페이지 보기:페이지 보기를 증가시키는 히트 유형. 구성 [요소 사용 안내서의 페이지 보기를](/help/components/c-variables/c-metrics/metrics-page-view.md) 참조하십시오. 히트를 참조하십시오.
* **** 지속성:변수 값과 개별 히트에서 발생하는 이벤트 간의 연결을 허용하는 전환 변수에 대한 추상 개념입니다. 만료도 참조하십시오.
* **** 주 서버 호출:이미지 요청 또는 히트에 대한 대체 이름. 대부분 다중 세트 태그 지정 및 청구 컨텍스트에서 사용됩니다. 동일한 히트가 여러 보고서 세트로 전송되면 첫 번째 보고서 세트는 주 서버 호출이고 나머지는 보조 서버 호출입니다. 이 규칙은 페이지 보기 및 링크 추적을 포함하여 모든 히트 유형에 적용됩니다. 보조 서버 호출을 참조하십시오.
* **** 처리 규칙:참조:
   * 처리 규칙, 관리 콘솔에서 특정 규칙을 사용하여 데이터 수집을 변경하는 방법입니다. See [Processing rules](/help/admin/admin/c-processing-rules/processing-rules.md) in the Admin user guide.
   * 히트가 속한 마케팅 채널을 결정하는 규칙 세트인 마케팅 채널 처리 규칙. 관리 [사용 안내서의 마케팅 채널 처리 규칙을](/help/admin/admin/marketing-channels-admin.md) 참조하십시오.
* **** Prop:트래픽 변수를 참조하십시오.
* **** 등급 보고서:일반적으로 지표를 사용하여 차원을 따르는 보고서 형식입니다. 이 유형의 보고서에서는 사이트에서 가장 많이 본 페이지와 같은 상위 항목을 볼 수 있습니다. 트렌드 보고서를 참조하십시오.
* **** 실시간:대기 시간이 거의 없는 상태로 구성된 변수를 수집하는 즉시 표시합니다. 관리 [사용 안내서의 실시간 보고서를](/help/admin/admin/realtime/realtime.md) 참조하십시오.
...
* **** 세그먼트:데이터의 특정 하위 세트에 집중할 수 있습니다. 구성 [요소](/help/components/c-segmentation/seg-overview.md) 사용 안내서의 세그멘테이션을 참조하십시오.
* **** 세그먼트 컨테이너:가져올 데이터의 양을 결정하는 세그먼트의 일부입니다. 컨테이너는 페이지 보기, 방문 또는 방문자를 기반으로 할 수 있습니다. 구성 [요소](/help/components/c-segmentation/seg-overview.md) 사용 안내서의 세그멘테이션을 참조하십시오.
* **** 직렬화:이벤트 정리를 참조하십시오.
* **** 서버 호출:이미지 요청 또는 히트에 대한 대체 이름(주로 청구 컨텍스트에서 사용)
* **** 단일 액세스:차원에 하나의 고유 값만 있는 방문. 여러 개의 고유 값이 없는 한 방문에 여러 개의 히트가 있을 수 있습니다. 구성 [요소 사용 안내서의 단일 액세스를](/help/components/c-variables/c-metrics/metrics-single-access.md) 참조하십시오. 바운스를 참조하십시오.
* **** SiteCatalyst:더 이상 용어로 사용되지 않습니다. Adobe Analytics의 이전 제품 이름입니다.
* **** 솔루션 디자인 문서:솔루션 디자인 참조 또는 SDR이라고도 합니다. 사용자 지정 변수의 사용 방법과 이 변수를 채우는 데 사용된 로직에 대해 대략적으로 설명하는 조직에서 유지 관리하는 내부 문서입니다. 구현 [사용 안내서에서 솔루션 디자인 문서](/help/implement/prepare/solution-design.md) 만들기를 참조하십시오.
* **** 하위 관계:더 이상 용어로 사용되지 않습니다.차원 분류로 대체되었습니다. 이전 버전의 Adobe Analytics에서 하위 관계는 전환 변수를 분류하는 기능을 부여했습니다. 분석 [사용 안내서에서 차원](/help/analyze/analysis-workspace/components/dimensions/t-breakdown-fa.md) 분류를 참조하십시오.
* **** 성공 이벤트:사용자가 수행한 추적된 작업입니다. 조직은 추적할 이벤트와 이를 추적하는 데 사용하는 성공 이벤트 변수를 결정합니다. 구성 [요소 사용 안내서의 사용자 지정 이벤트를](/help/components/c-variables/c-metrics/metrics-custom.md) 참조하십시오.
* **** 지원되는 사용자:고객 지원 담당자를 참조하십시오.
* **** 트래픽 변수:구어체는 prop로 알려져 있다. 단일 히트에 대한 사용자 지정 값을 저장합니다. 이전 버전의 Adobe Analytics에서는 Prop에 고유한 값을 제공했지만, 플랫폼에 대한 개선 사항은 사용자 지정 트래픽 변수를 크게 불필요하게 만듭니다. 대부분의 경우 사용자 지정 전환 변수(eVar)를 사용하는 것이 좋습니다. 구성 [요소 사용 안내서의 사용자 지정 트래픽 변수를](/help/components/c-variables/dimensionslist/reports-custom-traffic.md) 참조하십시오.
* **** 트렌드 보고서:일반적으로 지표가 있는 여러 날짜 범위를 표시하는 보고서 형식입니다. 이 유형의 보고서에서는 시간이 지남에 따라 지표가 수행되는 방식을 볼 수 있습니다. 등급 보고서를 참조하십시오.
* **고유 방문자**:사이트를 방문한 고유 개인의 수를 나타냅니다. 단일 고유 방문자는 여러 방문을 가질 수 있습니다. 구성 [요소](/help/components/c-variables/c-metrics/metrics-unique-visitors.md) 사용 안내서에서 고유 방문자를 참조하십시오.
* **** 가상 보고서 세트:일반 보고서 세트를 참조하고 데이터를 구체화할 수 있는 데이터의 가상 컨테이너입니다. 데이터는 가상 보고서 세트로 전송되지 않습니다.대신 데이터는 일반 보고서 세트로 전송되고 가상 보고서 세트는 수집된 데이터를 기반으로 만들어집니다. 구성 [요소](/help/components/vrs/vrs-about.md) 사용 안내서의 가상 보고서 세트를 참조하십시오.
* **** 방문:사이트에서 발생한 고유 세션 수를 나타냅니다. 구성 [요소](/help/components/c-variables/c-metrics/metrics-visit.md) 사용 안내서의 방문 횟수를 참조하십시오.
* **** VISTA 규칙:Adobe가 데이터 서버측을 복사, 구문 분석 또는 필터링하기 위해 요청한 사용자 지정 로직 VISTA 규칙은 일반적으로 추가 비용을 발생시킵니다. 처리 규칙을 참조하십시오.
* **** 웹 비콘:이미지 요청을 참조하십시오.
