...
각 데이터 수집 서버는 원시 분석 데이터를 캡처 및 처리한 다음 보고를 위해 일괄 데이터를 시간별로 업로드합니다. 전환 프로세스에는 보통 30분이 걸립니다. 따라서 이전 업로드가 완료된 직후 발생하는 트래픽의 일반 지연 시간은 약 90분입니다(다음 배치 업로드까지 60분, 이후 파일 전송 및 표시까지 30분). 업로드하기 직전에 발생하는 트래픽의 경우 데이터 지연이 30분 (다음 배치 업로드가 발생할 때까지 0분, 그런 다음 파일 전송 및 표시에 30분)으로 하락할 수 있습니다.
...
지연이 발생하는 경우 Adobe는 처리 파이프라인을 적극적으로 모니터링하고 가능한 한 빨리 처리 시간을 정상으로 되돌리기 위해 가능한 모든 조치를 수행합니다. 지연 문제 대부분은 1시간 이내에 해결됩니다. 특정 보고서 세트에 관심이 있는 경우 조직의 지원 사용자 중 한 명이 지연이 발생하는 보고서 세트 ID를 사용하여 고객 지원 센터에 문의할 수 있습니다. Adobe 담당자는 지연 시간을 확인하고 문제가 개선되고 해결되면 알려 줄 수 있습니다.
