...
각 데이터 수집 서버는 원시 분석 데이터를 캡처 및 처리한 다음 보고를 위해 일괄 데이터를 시간별로 업로드합니다. 전환 프로세스에는 보통 30분이 걸립니다. 따라서 이전 업로드가 완료된 직후 발생하는 트래픽의 일반 지연 시간은 약 90분입니다(다음 배치 업로드까지 60분, 이후 파일 전송 및 표시까지 30분). 업로드 직전에 발생하는 트래픽의 경우 데이터 지연 시간은 30분까지 짧아질 수 있습니다(다음 배치 업로드가 발생할 때까지 0분, 그런 다음 파일 전송 및 표시에 30분).

필요한 경우 Adobe 고객 지원 센터에서 자주 사용하는 보고서 세트에 대해 30분 단위 배치 데이터 업로드(시간별 업로드 대신)를 활성화할 수 있습니다.
...
* IAB 보트 필터링 활성화: [보트 필터링](/help/admin/tools/manage-rs/edit-settings/general/bot-removal/bot-removal.md)은 보고서 세트에 보트나 크롤러가 많이 등장하는 경우 지연을 줄이는 데 효과적입니다. IAB 보트 목록은 [Interactive Advertising Bureau](https://www.iab.net/about_the_iab)에서 업데이트 및 유지 관리되므로 이 목록을 사용하는 것이 좋습니다. 사용자가 봇 규칙을 사용자 지정하여 IAB의 규칙을 보완할 수도 있습니다.

## 지연 대응

지연이 발생하는 경우 Adobe는 처리 파이프라인을 적극적으로 모니터링하고 가능한 한 빨리 처리 시간을 정상으로 되돌리기 위해 가능한 모든 조치를 수행합니다. 많은 지연 문제는 몇 시간 내에 해결됩니다. 특정 보고서 세트에 관심이 있는 경우 조직의 지원 사용자 중 한 명이 지연이 발생하는 보고서 세트 ID를 사용하여 고객 지원 센터에 문의할 수 있습니다. Adobe 담당자는 지연 시간을 확인하고 문제가 개선되고 해결되면 알려 줄 수 있습니다.
