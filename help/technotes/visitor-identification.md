...
Adobe Analytics에서는 방문자를 식별하는 몇 가지 메커니즘을 제공합니다. 다음 표에는 Analytics에서 방문자를 식별할 수 있는 다양한 방법(기본 설정 순서)이 나와 있습니다.

| 사용한 주문 | 쿼리 매개 변수(컬렉션 메서드) | 제공 시기 |
|---|---|---|
...
많은 시나리오에서 한 번의 호출에서 2개 또는 3개의 서로 다른 ID가 표시될 수 있지만 Analytics에서는 이전 표에서 나타난 첫 번째 ID를 공식 방문자 ID로 사용합니다. 예를 들어, 사용자 지정 방문자 ID(&quot;vid&quot; 쿼리 매개 변수에 포함됨)를 설정하는 경우, 이 ID는 동일한 히트에 있을 수 있는 다른 ID보다 먼저 사용됩니다.
...
사용자 지정 방문자 ID는 방문자를 식별하는 고유한 방법이 있는 사이트에서 사용할 수 있습니다. 예를 들어 사용자가 사용자 이름과 암호를 사용하여 웹 사이트에 로그인할 때 생성되는 ID입니다.

사용자 ID를 파생하고 관리할 수 [!UICONTROL visitor IDs] 있는 경우 다음 방법을 사용하여 ID를 설정할 수 있습니다.

| 메서드 | 설명 |
|---|---|
| [s.visitorID](../implement/vars/config-vars/visitorid.md) 변수 | 브라우저에서 JavaScript를 사용하거나 다른 AppMeasurement 라이브러리를 사용하는 경우 데이터 수집 변수에 방문자 ID를 설정할 수 있습니다. |
| 이미지 요청의 쿼리 문자열 매개 변수 | 이렇게 하면 하드 코딩된 이미지 요청에 [!UICONTROL visitor ID] 대한 [!UICONTROL vid query string] 매개 변수를 통해 Adobe로 전송할 수 있습니다. |
| 데이터 삽입 API | JavaScript를 허용하지 않는 무선 프로토콜을 사용하는 장치에서는 `<visitorid/>` XML 요소가 들어 있는 XML 게시물을 해당 서버로부터 Adobe 수집 서버로 보낼 수 있습니다. |
| URL 다시 작성 및 VISTA | 일부 배포 아키텍처는 쿠키를 설정할 수 없을 때 세션 상태를 유지하기 위해 URL 다시 쓰기를 지원합니다. 이와 같은 경우, Adobe 엔지니어링 서비스에서는 페이지의 URL에서 세션 값을 찾는 [!DNL VISTA] 규칙을 구현한 다음 형식을 지정하여 [!UICONTROL visid] 값에 삽입할 수 있습니다. |
>[!CAUTION]
...
사용자가 사이트를 방문하면 Adobe 웹 서버에서 브라우저에 대한 HTTP 응답에 영구적 쿠키를 포함하여 영구적 쿠키를 설정합니다. 이 쿠키는 지정된 데이터 수집 도메인에 설정됩니다.

Adobe 데이터 수집 서버로 요청이 전송되면 헤더에 방문자 ID 쿠키(`s_vi`)가 있는지 확인이 이루어집니다. 이 쿠키가 요청에 있으면 방문자 식별에 사용됩니다. 쿠키가 요청에 없는 경우 서버는 고유 방문자 ID를 생성하여 HTTP 응답 헤더에서 쿠키로 설정한 후 다시 요청과 함께 전송합니다. 쿠키는 브라우저에 저장되고 이후 사이트를 방문하는 동안 데이터 수집 서버로 다시 전송되어 방문자 간을 식별할 수 있습니다.
...
이를 방지하기 위해 많은 고객들이 [자사 쿠키 구현](https://marketing.adobe.com/resources/help/en_US/whitepapers/first_party_cookies/)의 일부로서 데이터 수집 서버에 대한 CNAME 레코드를 구현했습니다. CNAME 레코드가 고객 도메인을 데이터 수집 서버로 호스트 이름을 매핑하도록 구성된 경우(예: `metrics.mysite.com`을 `mysite.omtrdc.net`으로 매핑) 데이터 수집 도메인이 웹 사이트 도메인과 일치하므로 방문자 ID 쿠키가 저장됩니다. 이렇게 하면 방문자 ID 쿠키가 저장될 가능성이 높아지지만 CNAME 레코드를 구성하고 데이터 수집 서버에 대한 SSL 인증서를 유지해야 하므로 약간의 오버헤드가 발생합니다.

### 모바일 장치의 쿠키 {#section_7D05AE259E024F73A95C48BD1E419851}

쿠키를 사용하여 모바일 장치를 추적하는 경우 몇 가지 설정을 사용하여 측정 방식을 수정할 수 있습니다. 쿠키의 기본 수명은 5년이지만 CL 쿼리 매개 변수(`s.cookieLifetime`)를 사용하여 이 기본값을 바꿀 수 있습니다. cname 구현을 위해 쿠키 위치를 설정하려면 CDP 쿼리 문자열 `s.cookieDomainPeriods`를 사용하십시오. 값이 지정되지 않은 경우 기본값은 2입니다. 기본 위치는 domain.com입니다. CNAME을 사용하지 않는 구현의 경우 방문자 ID 파섹 쿠키 위치는 207.NET 도메인입니다.
...
대부분의 모바일 장치는 브라우저 쿠키를 허용합니다. 그러나 장치가 쿠키를 허용하지 않는 경우 무선 장치를 고유하게 식별하는 다른 방법이 사용됩니다.

Adobe는 다수의 모바일 장치를 고유하게 식별하는 많은 HTTP 가입자 ID 헤더를 확인해 왔습니다. 이러한 헤더에는 종종 장치 전화 번호(또는 번호에 해시된 버전) 또는 기타 식별자가 포함됩니다. 현재 장치의 대부분에는 장치를 고유하게 식별하는 헤더가 하나 이상 있으며, 모든 Adobe 데이터 수집 서버는 방문자 ID 대신 이러한 헤더를 자동으로 사용합니다.

In a typical image request, a &#39;1&#39; in the path ( `/b/ss/rsid/1`) causes Adobe servers to return a gif image and to attempt to set a persistent [!UICONTROL visitor ID] cookie ( `AMCV_` or `s_vi`). 그러나 장치가 HTTP 헤더를 기반으로 하는 모바일 장치로 인식되는 경우 &#39;5&#39;가 &#39;1&#39; 대신 전달되고, 이는 wbmp 형식 이미지를 반환해야 하고, 인식된 무선 헤더(쿠키 아님) 목록을 사용하여 장치를 식별해야 함을 나타냅니다.

다음 표에는 경로에 있는 반환 이미지 유형 값(&#39;1&#39; 또는 &#39;5&#39;)을 기준으로 사용되는 ID 메서드의 순서가 나와 있습니다.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>기본값: </p> 
    <ul id="ul_E37E9919658A492C92187BAA18D33AB6"> 
     ...
     <li id="li_52706792FAD14F459266E3A672F92EA1">구독자 ID 헤더 </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>장치가 무선 장치로 확인되었거나, <code> /5/</code>가 이미지 요청에서 수동으로 전송되었습니다. </p> 
    <ul id="ul_624BEDFA3E1243CF9B42081D8B8EFFFB"> 
     ...
     <li id="li_ADBA806B74CA43EFA8612301E06106C6">구독자 ID 헤더 </li> 
...
수동 이미지 요청에서 &#39;1&#39; 또는 &#39;5&#39;를 전달할 수도 있지만, 이러한 코드는 상호 배타적이므로 항상 &#39;5&#39;를 전달하면 쿠키가 지원될 때 쿠키가 사용되지 않습니다. 장치가 쿠키를 지원하는지 확인하고 지원하는 경우 이미지에서 &#39;5&#39;가 아닌 &#39;1&#39;을 전달하도록 자체 메커니즘을 통합할 수 있습니다. 이 상황에서 정확도 개선은 쿠키를 지원하는 모바일 장치의 수로 제한됩니다.

### 가입자 ID 헤더 {#section_60D6EAC0D16945A89DD5A7ADF3B8298D}

가입자 ID 방법은 일반적으로 쿠키 삭제, 쿠키 승인 문제 및 게이트웨이 쿠키 관리 문제로 인해 사용자 식별을 위한 쿠키보다 안정적입니다.

모바일 방문자가 사용하는 통신사의 허용 목록에 추가되어 방문자 식별의 변경 사항을 개선할 수 있습니다. 통신사의 방문자 ID에 액세스하려면 통신사에 연락하여 도메인을 허용 목록에 추가합니다. 통신사의 허용 목록에 있는 경우 액세스할 수 없는 가입자 ID 헤더에도 액세스할 수 있습니다.

다음 헤더 목록은 무선 장치를 식별하는 데 사용됩니다. 헤더를 처리하기 위한 알고리즘은

1. http 헤더 키 추출(예: &quot;X-Up-Calling-Line-ID&quot;)
1. 알파벳(A-Z 및 a-z)이 아닌 모든 문자 트리밍
1. 헤더 키를 소문자로 변환
1. 키의 끝을 다음 표의 항목과 비교하여 일치하는 항목을 찾습니다.
...
| 화보 | IP 주소 | x-huawei-NASIP:211.139.172.70 |
| userip | IP 주소 | UserIP:70.214.81.241 |
| ipaddress | IP 주소 | X-Nokia-ipaddress:212.97.227.125 |
| 가입자 정보 | IP 주소 | X-SUBSCRIBER-INFO:IP=10.103.132.128 |

예를 들어 &quot;callinglineid&quot;는 &quot;X-Up-Calling-Line-ID&quot; 및 &quot;nokia-callinglineid&quot;와 일치합니다. 헤더 유형은 헤더에서 예상할 사항을 알려줍니다. 헤더 우선 순위의 순서는 여기에 나열되어 있습니다(&quot;callinglineid&quot; 헤더가 있는 경우 이 헤더는 &quot;subno&quot; 대신 사용됩니다).
