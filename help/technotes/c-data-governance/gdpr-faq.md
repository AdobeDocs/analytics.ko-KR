...
데이터 주체가 식별자(사용자 정의 변수, 예: prop 또는 eVar)와 함께 요청을 제출하면 Adobe Analytics에서 지정된 식별자에 대해 수집된 데이터의 전체 보유 기록을 스캔합니다. Analytics Prop 또는 eVar에 저장된 사용자 정의 ID를 구성하는 방법에 대한 자세한 내용은 [네임스페이스의 Analytics 설명서](/help/technotes/c-data-governance/data-labeling/gdpr-namespaces.md)를 참조하십시오.
