...
방문자가 동일한 세션에서 여러 채널을 통해 사이트에 도달하는 경우가 있습니다. 예를 들어 방문자가 소셜 미디어 페이지를 클릭한 다음 몇 분 후에 검색 엔진을 방문하여 사이트에 도달합니다. 이러한 경우 비소셜 도메인이 이 보고서에 표시될 수 있습니다. 비소셜 도메인을 제외하려면 방문 횟수로 보고서를 정렬하거나 방문 횟수 대신 히트 수를 기준으로 세그먼트 사본을 만듭니다. 자세한 내용은 구성 요소 사용 안내서의 [세그멘테이션 컨테이너](/help/components/segmentation/seg-overview.md)를 참조하십시오.
...
3. **페이지** 차원을 찾은 다음 화살표 아이콘을 클릭하여 페이지 값을 표시합니다. Dimension 항목은 노란색입니다.
