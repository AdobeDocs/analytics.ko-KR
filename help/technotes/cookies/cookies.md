...
>[!NOTE]
>[크로스 장치 분석](https://experienceleague.adobe.com/docs/analytics/components/cda/overview.html?lang=en#cda) 및 [Customer Journey Analytics](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-overview/cja-overview.html?lang=en#comparing-cja-to-traditional-adobe-analytics)은 해시된 로그인 ID와 같은 개인 ID를 사용하여 쿠키 전체를 스티칭할 수 있습니다.
...
방문자 ID 서비스는 &quot;[demdex.net](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html)&quot; 쿠키를 사용하여 여러 고객 도메인에서 방문자에 대한 영구 식별자를 제공합니다. 이전 Analytics ID 서비스, &quot;s_vi&quot; 쿠키는 사용자 지정 CNAME 수집 도메인을 사용하지 않는 구현에 대해 타사 쿠키로 설정됩니다.

타사 쿠키가 차단된 브라우저에서는 도메인 간 추적을 사용할 수 없습니다.

### 자사 쿠키 제한 사항 {#limitations-first-party-cookies}

자사 쿠키는 모든 주요 브라우저에서 허용됩니다. 하지만, Apple은 ITP(Intelligent Tracking Program)를 통해 Adobe이 설정하는 자사 쿠키의 수명을 제한합니다. 이는 iOS 및 iPadOS의 모든 브라우저뿐만 아니라 Safari에도 영향을 줍니다.

Adobe의 자사 쿠키는 7일 만료 또는 Apple이 판단하기에 클릭스루 횟수에 대해 추적기(24시간 만료)에서 오는 것으로 제한됩니다. 7일 만료와 함께, 사용자가 사이트를 방문하고 7일 이내에 돌아오는 경우, 쿠키의 만료 날짜는 다른 7일까지 연장됩니다. 그러나 사용자가 사이트를 방문하고 8일 후에 돌아오면 두 번째 방문에서 새 사용자로 처리됩니다.
...
#### ITP 정책 {#ITP-timeline}에 대한 주요 변경 내용의 타임라인
...
ITP 정책은 자주 진화하고 있습니다. 최신 정책은 Apple의 [Webkit의 추적 방지](https://webkit.org/tracking-prevention)를 참조하십시오.

#### 영향을 받는 Adobe 퍼스트 파티 쿠키는 무엇입니까?

Adobe에 의해 설정된 모든 자사 쿠키 및 관련 JavaScript 라이브러리는 ITP 정책의 영향을 받습니다.

* [ECID(Adobe Experience Cloud ](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html) 방문자 ID) 서비스 라이브러리가 설정한 &quot;AMCV&quot; 쿠키
...
* &quot;s_vi&quot;를 설정할 수 없을 때 사용되는 폴백 쿠키인 기존 [&quot;s_fid&quot; 쿠키](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-analytics.html)

#### Analytics용 Safari에 ITP가 미치는 영향은 무엇입니까?

ITP 제한 사항의 영향은 사용자의 행동에 따라 상당히 다를 수 있습니다. ITP로 영향을 받는 브라우저(예: Safari)를 사용하고 7일간의 부재 후 돌아온 방문자만 영향을 받습니다. 방문자가 ITP 브라우저를 사용하지 않거나 7일 이내에 재방문하면 영향을 받지 않습니다. Analytics에서 자신의 데이터를 검토하여 이 제한이 미치는 영향을 이해해야 합니다. 사이트에 미치는 영향을 측정하는 방법에 대한 팁은 &quot;[Safari 변경 사항이 비즈니스에 영향을 미치는지 어떻게 확인할 수 있습니까?](#measure-itp-effect)&quot;를 참조하십시오.
...
2. 속성 변경 사항. 속성은 동일한 방문자가 전환 이벤트를 이전 활동과 연결하는 데 의존합니다. 쿠키가 만료되면 후속 이벤트가 새 방문자와 연결됩니다. 새 방문자의 활동은 이전 방문자의 활동과 연결할 수 없습니다.
...
2020년 2월에 Chrome 80 브라우저가 릴리스되고 Firefox 및 Edge 브라우저의 후속 버전이 있을 경우, SameSite 쿠키 속성은 쿠키를 제3자 컨텍스트에서 사용할 수 있는지 여부를 제어하는 3개의 다른 값에 대한 사양을 적용합니다.
...
방문자 ID 서비스를 사용하는 고객의 경우 쿠키에는 기본적으로 `SameSite=None` 및 `secure` 속성이 설정되어 있어 이러한 쿠키가 제3자의 사용 사례를 지원할 수 있습니다.

Analytics 이전 식별자(&quot;s_vi&quot; 및 &quot;s_fid&quot; 쿠키)를 사용하는 고객의 경우, 표준 수집 도메인의 제3자 사용 사례를 사용하도록 쿠키가 설정됩니다.adobedc.net, 2o7.net 및 omtrdc.net CNAME 구현을 사용하는 고객의 경우 Analytics는 `SameSite=Lax`을(를) 설정합니다.

>[!NOTE]
>
>여러 도메인을 소유하고 있고 모든 도메인에서 데이터 수집을 위해 동일한 CNAME을 사용하는 경우, 쿠키는 다른 도메인에서 타사 쿠키로 처리됩니다. 기존 Analytics 식별자를 사용하는 경우 이러한 쿠키를 사이트 전체에서 공유할 수 있도록 설정을 `SameSite=None`으로 업데이트할 수 있습니다. 자세한 내용은 다음 섹션의 &quot;[여러 도메인에 대해 하나의 CNAME을 사용할 때 동일한 사이트 값 변경](#samesite-one-cname)&quot;을 참조하십시오.
...
* ITP 관리 브라우저에서 트래픽의 비율을 측정합니다.
   ...
   1. 세그먼트를 만들어 ITP 플랫폼을 사용하는 방문자 수를 확인합니다.
      ...
      >[!NOTE]
      >
      >ITP의 영향을 받는 특정 브라우저는 CNAME 구현을 사용하는지에 따라 달라집니다. 자세한 내용은 &quot;[ITP 정책](#ITP-timeline)&quot;에 대한 주요 변경 사항의 타임라인을 참조하십시오.
