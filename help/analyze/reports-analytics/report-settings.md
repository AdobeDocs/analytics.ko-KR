...
| 통화 | 데이터를 변환할 통화입니다. 이 설정에서 값을 선택하면 데이터베이스에 저장된 데이터가 영향을 받지 않지만 현재 날짜의 통화 변환 비율에 따라 변환된 값으로 표시됩니다. 통화 옵션을 구성하지 않으면(기본값으로 설정) 통화 변환이 수행되지 않고 모든 값이 미국 달러(USD)로 저장되고 표시됩니다. 데이터를 처리할 때(표시 전에) 통화를 변환하려면 Adobe 계정 팀에 문의하십시오. |
