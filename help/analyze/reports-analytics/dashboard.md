---
description: 대시보드는 reportlet이라고 하는 축소판 보고서 모음입니다. 대시보드는 검색 방법, 방문자 프로필 등과 같은 사이트의 특정 측면에 대한 전체 개요를 제공하는 관련 reportlet을 포함하는 경우에 가장 유용합니다.
subtopic: Dashboards
...
대시보드는 reportlet이라고 하는 축소판 보고서 모음입니다. 대시보드는 검색 방법, 방문자 프로필 등과 같은 사이트의 특정 측면에 대한 전체 개요를 제공하는 관련 reportlet을 포함하는 경우에 가장 유용합니다.

## 대시보드 및 Reportlet {#concept_8CD3ACA2830A4994A68A31D8773B57E0}

대시보드는 *`reportlets`*&#x200B;이라고 하는 썸네일 보고서의 컬렉션입니다. 대시보드는 검색 방법, 방문자 프로필 등과 같은 사이트의 특정 측면에 대한 전체 개요를 제공하는 관련 reportlet을 포함하는 경우에 가장 유용합니다.
...
대시보드를 랜딩 페이지로 설정하고, 다른 사용자와 대시보드를 공유하고, 배달 일정을 예약할 수도 있습니다. 대시보드(또는 책갈피)를 랜딩 페이지로 설정하지 않으면 [!UICONTROL My Recommended Reports] 대시보드가 표시됩니다. **[!UICONTROL My Recommended Reports]** 보고서는 보고서와 가장 자주 본 보고서 5개를 **[!UICONTROL Key Metrics]** 보여줍니다. 이 보고서는 동적으로 변하며 가장 많이 보는 실제 보고서를 기반으로 합니다.

자주 본 일부 보고서는 대시보드가 될 수 없으며 나타나지 않습니다. 다음과 같습니다.
...
>[!NOTE] 대시보드가 더 이상 보고 및 분석에 표시되지 않습니다. **[!UICONTROL Site Overview]** 하지만 여전히 일부 또는 전체 reportlet을 볼 수 있는 몇 가지 상황이 있습니다.

* 자주 본 보고서가 3개만 있는 경우 보고 및 분석은 사이트 개요 대시보드에서 두 개의 보고서를 가져와 **[!UICONTROL My Recommended Reports]** 대시보드를 완료합니다.
* 새 보고서 세트는 자주 보는 보고서로 대체될 때까지 처음에 사이트 개요 reportlet을 계속 제공합니다. 그렇다 하더라도 이제 대시보드가 **[!UICONTROL My Recommended Reports]**&#x200B;호출됩니다.
...
이 사용자 지정 가능한 대시보드에서는 reportlet을 제공된 템플릿에 놓을 수 있습니다.

**[!UICONTROL Components]>[!UICONTROL Dashboards]>[!UICONTROL Shared Dashboards]>[!UICONTROL Site Operations Dashboard]**

이 대시보드는 웹 사이트 작업과 관련된 주요 지표의 개요를 제공합니다. 이 대시보드의 보고서는 다음과 같습니다.
...
을 [!UICONTROL Dashboard Manager] 사용하여 대시보드를 편집 및 관리하고 DirectAccess에 대해 활성화합니다.
...
reportlet을 만드는 방법을 설명하는 단계입니다. reportlet을 만들면 대시보드에 표시할 수 있습니다.
...
1. (선택 사항) 날짜 범위를 구성합니다.
   ...
   * **[!UICONTROL Rolling]**:시간 범위(일별, 월별 등)에 따라 시간이 경과함에 따라 날짜를 변경합니다. 예를 들어 오늘이 1월 17일인 경우 1월 15일 - 16일에 대한 날짜를 설정할 수 있습니다. 그런 다음 1월 27 **[!UICONTROL Rolling]**&#x200B;일에 reportlet에 1월 25일 - 26일에 대한 데이터가 표시됩니다.
   * **[!UICONTROL Fixed]**:시간이 경과하면 날짜가 앞으로 이동하지 않습니다.

1. (선택 사항) 게시 배포 목록을 무시합니다.

   **[!UICONTROL Publishing List Override]**:이 옵션을 활성화하면 게시 목록에 배포할 때 이 reportlet에서 참조되는 보고서 세트가 항상 사용됩니다. 이 옵션을 비활성화하면 게시 목록에서 식별된 보고서 세트가 이 reportlet의 보고서 세트를 대체합니다.
...
다른 대시보드 및 공유 대시보드에서 컨텐츠를 추가하는 방법을 설명하는 단계입니다. 텍스트 및 이미지와 같은 사용자 지정 및 외부 소스의 컨텐츠를 추가할 수도 있습니다.
...
1. Click **[!UICONTROL Add Content]**, then drag items to the dashboard.
   ...
   * 지표 측정: 지정한 임계값과 관련하여 지표 수치의 위치를 알려주는 측정치를 표시합니다.
      ...
      지표, 그래프 유형, 색상 범위 및 임계값을 선택할 수 있습니다. 지표의 개수가 임계값 보다 큼인 경우 측정은 reportlet의 초과 필드 위에 있는 색을 사용하여 reportlet임을 가리킵니다. 지표의 개수가 임계값 보다 작음인 경우 측정은 미만 필드 위에 있는 색을 사용하여 reportlet임을 가리킵니다. 이러한 필드에 지정하는 값은 페이지 보기 횟수, 달러 금액, 장바구니 보기 횟수 등과 같이 계산 가능한 지표 값입니다. 특수 문자는 사용하지 마십시오.
...
URL을 제공하여 다음 사용자 컨텐츠 reportlet을 만들 수 있습니다. 이미지 또는 기타 리소스 URL이 https://로 시작되지 않는 경우 Internet Explorer는 혼합 컨텐츠 경고를 표시할 수 있습니다. 브라우저의 보안 설정에서 혼합 컨텐츠에 대한 경고를 비활성화할 수 있습니다.
   ...
   **내 대시보드**
   ...
   컨텐츠를 새 대시보드로 이동할 수 있는 업그레이드된 대시보드를 나열합니다.
   ...
   **공유 대시보드**
   ...
   컨텐츠를 새 대시보드로 이동할 수 있는 기존 대시보드를 나열합니다. 기존 대시보드는 이전 버전의 대시보드 서식을 유지하려는 경우 유용합니다.
...
대시보드 또는 reportlet 수준에서 데이터 설정을 변경할 수 있습니다. 예를 들어 보고서 세트를 변경하고, 보고서 세트를 잠그고, 날짜를 변경하고, 세그먼트를 적용하는 등의 작업을 수행할 수 있습니다. 또한 회사의 기밀 유지 정책을 삽입하여 대시보드를 개인화하고 사용자 지정 reportlet에 HTML, XML, Web API 및 CSV 데이터를 포함할 수 있습니다.
...
| 대시보드에 세그먼트 적용 | Experience Cloud 헤더에서 을 클릭하고 [!UICONTROL Show Segments]세그먼트를 선택합니다. |
| reportlet에 세그먼트 적용 | 대시보드에서 레이아웃을 클릭하여 대시보드를 편집합니다.   reportlet에서 보고서 세트 이름을 클릭한 다음 세그먼트 필드에서 값을 선택하고 업데이트를 클릭합니다. |
| 보고서 세트 잠금(reportlet에서 보고서 세트 변경 방지) | reportlet에서 보고서 세트 이름을 클릭한 다음 활성화합니다 [!UICONTROL Lock Report Suite]. 업데이트를 클릭합니다. |
| 보고 날짜 변경 | 대시보드의 경우 달력을 클릭합니다. (대시보드의 모든 reportlet은 변경 사항을 반영합니다.)<br>reportlet에 대해 날짜 링크를 클릭한 다음 달력을 구성합니다. |
...
| 대시보드를 랜딩 페이지로 설정 | 대시보드에서 [!UICONTROL More] > [!UICONTROL Set As Landing Page]을 클릭합니다. |
...
1. Browse to upload the image, then click **[!UICONTROL Save.]**
   ...
   브라우저에서 이미지를 볼 때 최상의 결과를 얻으려면 100px x 30px 이미지를 업로드하십시오. PDF 출력에서 최상의 결과를 얻으려면 417px x x 125px(300dpi) 이미지를 업로드하십시오. 크기가 큰 이미지는 종횡비를 유지하면서 크기로 축소됩니다.

## 대시보드에서 세그먼트 사용 {#concept_ED030C3713D54D03971FB7B209285750}

Adobe Analytics의 대부분의 보고서와 같은 대시보드는 세그먼트를 활용하여 원하는 데이터를 검색할 수 있습니다.
...
세그먼트는 두 가지 수준에 적용할 수 있습니다.전체 대시보드나 특정 reportlet에 있는 URL을 표시합니다.

* **Reportlet 수준**:을 **[!UICONTROL Layout]**&#x200B;클릭한 다음 세그먼트화할 reportlet의 보고서 세트를 클릭합니다. reportlet에서 사용하는 세그먼트를 추가하거나 변경할 수 있는 모달 창이 나타납니다.
* **대시보드 수준**:왼쪽 탐색 창에서 세그먼트 아이콘을 클릭하고 사용할 세그먼트를 선택한 다음 적용을 클릭합니다. 선택한 세그먼트는 모든 reportlet 수준 세그먼트를 대체하고 대체합니다.
