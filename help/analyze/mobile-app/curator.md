...
![성과 기록표의 예](/help/analyze/mobile-app/assets/intro_scorecard.png)
...
1. Adobe Analytics를 열고 작업 영역 탭을 클릭합니다.
...
프로젝트 화면의 구성 요소 탭에서 추가 단추를 클릭하고 Mobile Scorecard를 선택합니다.
...
3. 지표에 관련 차원을 추가하려면 왼쪽 패널에서 차원을 드래그하여 타일에 놓습니다. 예를 들어, 타일에 끌어다 놓아 적절한 차원(이 예에서는 DMA 영역 등)을 고유 방문자 수 지표에 추가할 수 있습니다.추가한 차원은 타일 특정 속성의 분류 섹션 아래에 표시됩니다. 각 타일에 여러 차원을 추가할 수 있습니다.


   *참고:Scorecard 캔버스에 차원을 놓아 모든 타일에 차원을 추가할 수도 있습니다. *

   [스코어카드 빌더]에서 타일을 클릭하면 오른쪽 레일에 해당 타일과 연결된 속성 및 특성이 표시됩니다. 이 레일에서 타일에 대한 새 제목을 제공하고, 또는 왼쪽 레일에서 구성 요소를 드래그하여 놓는 대신 구성 요소를 지정하여 타일을 구성할 수 있습니다.
   ...
   또한 타일을 클릭하면 동적 팝업이 앱의 경영진 사용자에게 분류 보기가 표시되는 방식을 표시합니다. 타일에 적용된 차원이 없는 경우, 분류 차원은 기본 날짜 범위에 따라 시간 또는 일이 됩니다.




   타일을 클릭하면 해당 타일에 대한 분류 보기가 스코어카드 측면에 표시됩니다.
...
5. 마찬가지로 전체 스코어카드에 적용되는 구성 요소를 제거하려면 타일 바깥의 아무 곳이나 클릭한 다음 모바일 고객 세그먼트에 대해 아래에 표시된 것처럼 구성 요소 위로 마우스를 가져가면 나타나는 x를 클릭하여 제거합니다.

6. 스코어카드 등록 정보에서 다음을 선택적으로 지정할 수도 있습니다.
...
1. 공유 메뉴를 클릭하고 스코어카드 공유를 선택합니다.

2. 공유 양식에서 다음 방법을 사용하여 필드를 완성합니다.

   * 스코어카드의 이름 제공
   * 스코어카드에 대한 설명 제공
   * 관련 태그 추가
   * 스코어카드의 수신자 지정
   * 포함된 구성 요소를 수신자와 공유하는 옵션을 선택하여 경영진 사용자가 스코어카드의 모든 구성 요소에 액세스할 수 있도록 합니다.
...
*참고:새 구성 요소를 추가하여 Scorecard를 업데이트하는 경우, 성과 기록표를 다시 공유(및 수신자와 포함된 구성 요소 자동 공유 옵션 선택)하여 경영진 사용자가 이러한 변경 사항에 액세스할 수 있는지 확인할 수 있습니다.*
...
경영진 사용자가 앱에서 스코어카드에 액세스할 수 있도록 지원하려면 다음을 확인하십시오.
    ...
    * 장치에서 모바일 OS의 최소 요구 사항은 iOS 버전 10 이상 또는 Android 버전 4.4(KitKat) 이상
    * Adobe Analytics*에 유효한
    로그인되어 있습니다. 모바일 스코어카드를 올바르게 만들고 이 성과 기록표와 공유합니다.
    * 분석 작업 공간 및 스코어카드가 기반으로
    하는 보고서 세트에 액세스할 수 있습니다.* 스코어카드에 포함된 구성 요소에 액세스할 수 있습니다. 참고:스코어카드를 수신자와 공유할 때 옵션을 선택할 수 있습니다.


### 경영진 사용자가 앱 사용

1. 행정 직원이 모바일 디바이스 운영 체제에 따라 앱을 다운로드하여 설치할 수 있도록 도와주십시오.
...
* 해당되는 경우 이 목록을 가장 최근에 수정한 날짜별로 정렬하도록 도와줍니다.
* 성과 기록표의 이름을 눌러 표시합니다.


1. 탭하여 회사를 선택합니다.
2. 스코어카드 목록에서 스코어카드를 누릅니다.

   참고:경영진 사용자가 로그인하여 공유된 항목이 없다는 메시지가 표시되는 경우:

   * 경영진 사용자가 잘못된 Analytics 인스턴스를 선택했을 수 있습니다.
   * 성과 기록표는 경영진 사용자와 공유되지 않았습니다.

3. 공유하는 스코어카드에 타일이 표시되는 방식을 표시합니다.
 ...
 * 스파크라인의 세부기간은 날짜 범위의 길이에 따라 달라집니다.      * 1일은 시간별 트렌드
 표시     * 1일 이상 1년 미만 및 일일 트렌드를
 표시합니다     * 1년 이상 주별 트렌드
     * 백분율 값 변경 공식은 지표 합계(현재 날짜 범위) - 지표 합계(비교 날짜 범위) / 지표 합계(비교 날짜 범위)를 표시합니다.
   *     화면을 아래로 당김하여 스코어카드를 새로 고칠 수 있습니다.
   

4. 타일에 대한 상세 분류 또는 트렌드 보고서가 어떻게 작동하는지 표시하려면 타일을 누릅니다.
...
5. 스코어카드의 날짜 범위를 변경하려면 다음을 수행합니다.
   ...
   *1. 날짜 헤더를 누릅니다. 2.날짜 범위 화면에서 작업할 시간을 누릅니다.*
...
6. 이 앱에 대한 피드백을 남겨 두려면:
   ...
   2. 내 계정 화면에서 피드백 옵션을 누릅니다.
   3. 피드백을 남길 수 있는 옵션을 보려면 을 누릅니다.



*오른쪽 상단에 있는 사용자 아이콘을 누릅니다. 2. 피드백 유형을 누릅니다. 3.적용 가능한 피드백 옵션을 누릅니다.*
...
옵션을 누르고 상단 필드에 이메일 주소를 입력하고 아래 필드에 질문을 입력합니다. 스크린샷은 메시지에 자동으로 첨부되지만 첨부 이미지의 X를 눌러 원하는 경우 삭제할 수 있습니다. 화면 녹화, 스크린샷 추가 또는 파일 첨부 옵션을 사용할 수도 있습니다. 질문을 보내려면 양식의 오른쪽 상단에 있는 용지 평면 아이콘을 누릅니다.
