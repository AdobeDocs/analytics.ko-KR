...
1. [!UICONTROL 표시] 옵션을 구성합니다. 
   ...
   **행 및 열 머리글 표시:** 행 및 열을 추가하여 이 항목을 표시합니다.
   ...
   버전 3.11에서는 각 항목에 대해 머리글을 표시할 수도 있습니다. 버전 4에는 세 항목 모두 표시되거나 아무 것도 표시되지 않습니다. 버전 3.11에서 요청을 작성하고 버전 4.x에서 열면 헤더가 없는 항목에 대해 셀 하나씩 범위를 업데이트하라는 Report Builder 메시지가 2단계에서 표시됩니다.

   **머리글을 Excel 자동 필터로 변경:** 행 및 열 머리글이 표시되는 경우에만 사용할 수 있습니다. 이 설정은 Excel 자동 필터를 만들어 이 요청에 대해 반환되는 데이터 Report Builder에 추가합니다.
