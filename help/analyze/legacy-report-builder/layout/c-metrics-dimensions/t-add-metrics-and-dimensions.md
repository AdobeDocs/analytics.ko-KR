...
1. [!UICONTROL 요청 마법사: 2단계] 양식에서 지표를 두 번 클릭하거나 지표를 원하는 위치로 끕니다.
   ...
   지표를 추가하면 요청 내에서 지표를 여러 번 표시할 수 있으므로 [!UICONTROL 지표] 탭에서 지표가 제거되지 않습니다. 예를 들어 각 값 외에 지표 소계를 표시할 수 있습니다. 그러나 차원을 추가하거나 제거할 때마다 사용 가능한 지표 목록이 변경됩니다.

   [!UICONTROL 지표] 레이아웃 섹션에 지표만 추가할 수 있습니다. 지표가 [!UICONTROL 열 레이블] 레이아웃에 [!UICONTROL 지표 헤더]&#x200B;(으)로 추가됩니다. [!UICONTROL 지표 헤더]을(를) [!UICONTROL 열 레이아웃]에서 [!UICONTROL 행 레이아웃]&#x200B;(으)로 이동하면 지표로 표시되고 분류로 사용됩니다.

   검색 막대는 지표 목록 바로 위의 지표 탭에 표시됩니다.
...
* 전체 단어 검색 및 기타 특수 검색 플래그(다음으로 시작, AND로 종료, OR 등) 은(는) 지원되지 않습니다.
...
* 다른 하위 양식(모달 또는 모델 없음)을 호출하면 종료 시 요청 마법사 2단계로 돌아갑니다. 이러한 형식의 예는 다음과 같습니다.
...
1. 차원을 추가합니다. 지표를 추가하는 것과 동일한 방식으로 차원을 추가합니다. 위의 1단계와 2단계를 참조하십시오.
   ...
   [!UICONTROL 날짜] 차원이 자동으로 추가됩니다. 사용 가능한 날짜 차원은 [!UICONTROL 요청 마법사: 1단계]에서 선택한 세부 기간에 따라 변경됩니다. 유효한 값은 다음과 같습니다.
...
다음 예에서는 [!UICONTROL 페이지] 지표와 관련된 차원입니다. [!UICONTROL 참조 도메인] 차원은 [!UICONTROL 페이지]와(과) [!UICONTROL 참조 도메인] 사이에 분류 보고서를 만듭니다. [!UICONTROL Dimension] 탭에 분류 보고서에 추가할 수 있는 차원만 업데이트되었습니다.
