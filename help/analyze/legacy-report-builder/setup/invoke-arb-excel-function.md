...
예를 들어 다른 소스에서 Excel로 가져온 데이터를 기반으로 입력 필터로 Report Builder 요청을 자동으로 새로 고치려면 RefreshRequestsInCellsRange(..) 문자열을 사용합니다. 함수. 모든 호출은 비동기적으로 수행되며 즉시 반환되고 완전히 실행될 때까지 기다리지 않습니다.
...
| AsyncRefreshRangeAltTextParam() | 문자열 | Ms 양식 컨트롤의 대체 텍스트를 통해 전달되는 지정된 셀 범위에 있는 모든 Report Builder 요청을 새로 고칩니다. |
...
| AsyncRefreshWorksheetAltTextParam(); | 문자열 | Ms Form Control의 대체 텍스트를 통해 전달된 특정 워크시트 이름에 있는 모든 Report Builder 요청을 새로 고칩니다. |
| tring GetLastRunStatus() | 문자열 | 마지막 실행 상태를 설명하는 문자열을 반환합니다. |
...
1. 매크로 매개 변수를 설정합니다.
1. 컨트롤을 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 매크로 할당]**&#x200B;을 선택합니다.
...
1. **[!UICONTROL 대체 텍스트]**&#x200B;에서 새로 고칠 셀 범위를 입력하십시오.
