...
거래 ID 데이터 소스는 다른 히트와 같이 처리됩니다. 거래 ID 데이터 소스는 일반적으로 기존 보고에 사용되는 특수 처리를 사용하지 않습니다. 즉, 보고서 처리 시간을 사용하는 경우 거래 ID 히트의 타임스탬프 근처에 발생한 히트에서 전파된 eVar 값이 거래 ID 히트에 생깁니다. 원래 트랜잭션 시간 근처에 발생한 히트에서 값이 전파되지 않습니다.

가능한 경우 속성 IQ는 지속된 값이 아니라 데이터 소스의 이벤트 내에서 전송된 MID 열 값에 의존합니다. 속성 모델은 데이터 소스의 MID 열 값에 즉시 적용됩니다. 예를 들어 &quot;마지막 터치&quot; 속성을 사용하는 경우 모델은 지표의 각 인스턴스에서 시작되며, 모델이 MID 열에서 관찰한 마지막 값에 도달할 때까지 히트에서 순차적으로 뒤로 이동합니다.

불가능한 경우, 속성 IQ는 평가를 위해 데이터 소스의 &#39;이전 레코드&#39;에 있는 MID 값을 사용합니다. AA가 순서가 잘못된 데이터를 지원하지 않는 경우 이 이전 레코드는 타임스탬프별로 순차적으로 정렬되지 않을 수 있습니다.

레코드가 순차적으로 순서가 지정되지 않으므로 지속성을 적용할 때의 예상 값은 제공된 트랜잭션 ID 타임스탬프와 원래 트랜잭션 사이에 존재하는 시간에 영향을 줄 수 있습니다.
...
마케팅 채널이 처음 도입되었을 때에는 첫 번째 터치와 마지막 터치 차원에서만 제공되었습니다. 현재 버전의 속성을 사용할 경우 명확한 첫 번째/마지막 접촉 차원이 더 이상 필요하지 않습니다. Adobe에서 일반 제공 [!UICONTROL 마케팅 채널] 및 [!UICONTROL 마케팅 채널 세부 정보] 차원을 활성화하여 원하는 속성 모델과 함께 사용할 수 있습니다. 이러한 일반 차원은 다음과 동일하게 동작합니다 [!UICONTROL 마지막 터치 채널] 차원이지만 다른 속성 모델과 함께 마케팅 채널을 사용할 때 혼동을 방지하기 위해 레이블이 다르게 지정됩니다.
