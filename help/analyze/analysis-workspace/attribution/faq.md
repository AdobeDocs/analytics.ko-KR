...
마케팅 채널이 처음 도입되었을 때에는 첫 번째 터치와 마지막 터치 차원에서만 제공되었습니다. 현재 버전의 속성을 사용할 경우 명확한 첫 번째/마지막 접촉 차원이 더 이상 필요하지 않습니다. Adobe이 일반 제공 [!UICONTROL 마케팅 채널] 및 [!UICONTROL 마케팅 채널 세부 사항] 원하는 속성 모델과 함께 사용할 수 있는 차원입니다. 이러한 일반 차원은 [!UICONTROL 마지막 터치 채널] 차원이지만 다른 속성 모델과 함께 마케팅 채널을 사용할 때 혼동을 방지하기 위해 다르게 레이블이 지정됩니다.
