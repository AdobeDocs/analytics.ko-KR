---
description: Analysis Workspace에서 프로젝트를 큐레이션하는 방법을 알아봅니다. 큐레이션은 프로젝트를 공유하기 전에 구성 요소에 대한 액세스를 제한합니다.
...
데모 비디오는 ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [프로젝트 조정](https://video.tv.adobe.com/v/31298?captions=kor&quality=12&learn=on){target="_blank"}을 참조하십시오.
...
프로젝트 큐레이션을 제거하고 왼쪽 레일에 있는 전체 구성 요소 세트를 복원하려면 다음 작업을 수행하십시오.
...
>[!NOTE]
>
> 가상 보고서 세트 큐레이션은 항상 프로젝트 큐레이션 전에 적용됩니다. 조정된 프로젝트에 특정 구성 요소가 포함되어 있어도 조정된 가상 보고서 세트에 이러한 구성 요소가 포함되어 있지 않으면 필터링됩니다.
> 
...
|---|---|---|---|
| **조정된 가상 보고서 세트** | 큐레이션되지 않은 모든 가상 보고서 세트 구성 요소 | 이 역할이 소유하거나 함께 공유한 조정되지 않은 가상 보고서 세트 구성 요소 | 이 역할이 소유하거나 함께 공유한 조정되지 않은 가상 보고서 세트 구성 요소 |
