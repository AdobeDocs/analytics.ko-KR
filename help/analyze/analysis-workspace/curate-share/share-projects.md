...
* **[!UICONTROL 사본 편집]** 또는 **[!UICONTROL 읽기 전용]** 역할에 배치된 관리자는 프로젝트를 열 때 제한된 경험을 받게 됩니다. 관리자는 자신의 역할을 다음으로 변경할 수 있습니다. **[!UICONTROL 원본 편집]** 프로젝트를 자신과 공유하고 **편집** 다음 절차에 설명된 대로 역할.
...
1. Adobe Analytics에서 [!UICONTROL **작업 영역**] 탭을 선택한 다음 를 선택합니다 [!UICONTROL **프로젝트**] 왼쪽 레일에서.
...
1. 선택 **[!UICONTROL 공유]**. (프로젝트가 이미 공유된 경우 [!UICONTROL **업데이트**].)
   ...
   선택 **[!UICONTROL 선별 및 공유]** 프로젝트 조정을 자동으로 적용합니다. (프로젝트가 이미 공유된 경우 **[!UICONTROL 조정 및 업데이트]**.) [프로젝트 조정](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/curate-share/curate.html?lang=ko-KR)에 대한 자세한 내용을 살펴보십시오.
...
다음 방법으로 누군가가 나와 프로젝트를 공유하는 경우 [특정 프로젝트 역할 공유](#share-a-specific-project-role)에서 공유 프로젝트에 액세스할 수 있습니다. [Analytics 랜딩 페이지의 프로젝트 탭](/help/analyze/landing.md#navigate-the-projects-tab).

다른 사용자가 링크를 공유하여 프로젝트를 귀하와 공유하는 경우(다음 중 하나에서) [프로젝트 탭 공유](#share-a-link-to-a-project) 또는 를 사용하여 [모든 사람과 공유](#share-a-project-with-anyone-no-login-required) link), 프로젝트에 액세스하려면 공유된 링크를 사용해야 합니다. 예를 들어 링크가 이메일, 내부 웹 사이트 등에 공유되었을 수 있습니다.
