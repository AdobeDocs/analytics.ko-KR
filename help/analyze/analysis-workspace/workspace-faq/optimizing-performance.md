...
| 동시 쿼리 | 조직에서 Adobe에 동시에 요청한 쿼리 수입니다. 각 조직은 최소 5개의 동시 쿼리를 사용할 수 있습니다. | 보고서가 시간이 오래 걸리는 경우 일반적으로 다른 보고서로 대기열에 있기 때문입니다. 즉, 조직에서 특정 보고서 세트에 대해 여러 개의 동시 요청을 실행하려고 합니다. 쿼리는 API 요청, 보고 UI(Analysis Workspace, 보고 및 분석, Report Builder 등), 예약된 프로젝트, 예약된 보고서, 예약된 경고 및 보고 요청을 하는 동시 사용자 중에서 가져올 수 있습니다. | 보고서 세트에 대한 요청과 일정을 온종일 더 균등하게 분배합니다. 또한 요청을 가능하면 피크 시간이 아닌 시간으로 이동합니다. 월요일 아침, 화요일 아침, 그리고 각 달의 1일은 최고의 보고 시간입니다. |
