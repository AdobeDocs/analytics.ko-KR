...
| 동시 쿼리 | 조직에서 Adobe에 동시에 요청한 쿼리 수입니다. | 조직에서 특정 보고서 세트에 대해 너무 많은 동시 요청을 실행하려고 합니다. 쿼리는 API 요청, 보고 UI(Analysis Workspace, 보고 및 분석, Report Builder 등), 예약된 프로젝트, 예약된 보고서, 예약된 경고 및 보고 요청을 하는 동시 사용자 중에서 가져올 수 있습니다. | 보고서 세트에 대한 요청과 일정을 온종일 더 균등하게 분배합니다. 또한 요청을 가능하면 피크 시간이 아닌 시간으로 이동합니다. 월요일 아침, 화요일 아침, 그리고 각 달의 1일은 최고의 보고 시간입니다. |
| 데이터 센터 용량 | Adobe 데이터 센터 내에서 사용자와 다른 고객이 공유하는 보고 용량. | 데이터 센터 내의 조직과 기타 조직이 수행한 동시 쿼리 수입니다. | 공간이 허용되면 Adobe이 최적화됩니다. 조직에서 설정된 용량을 사용할 수 있으며, 시스템이 가벼운 로드 하에 있는 경우 Adobe은 권한이 부여된 허용 범위를 넘어 사용자에게 더 많은 용량을 제공합니다. |
