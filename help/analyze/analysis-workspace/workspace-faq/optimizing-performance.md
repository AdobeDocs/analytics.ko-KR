...
* 순차적 세그먼테이션, 특히 차원 제한(Within/After)이 사용되는 경우
* 세그먼트에 사용된 차원 내의 고유 차원 항목 수(예: 페이지에 10개의 고유 항목이 있는 경우 페이지 = &#39;A&#39;가 페이지 = &#39;A&#39;보다 페이지 = 1,0000개의 고유 항목이 있는 경우)
* 사용된 다른 차원 수(예: 페이지 = &#39;홈&#39; 및 페이지 = &#39;검색 결과&#39;가 eVar 1 = &#39;red&#39; 및 eVar 2 = &#39;blue&#39;보다 빠름)
* 많은 OR 연산자(AND 대신)
* 범위가 다양한 중첩 컨테이너(예: &quot;방문자&quot; 내 &quot;방문&quot; 내의 &quot;히트&quot;)

**로직 복잡성에 대한 권장 사항**

몇 가지 복잡성 요소를 방지할 수 없지만 세그먼트의 복잡성을 줄일 수 있는 기회에 대해 생각해 보십시오. 일반적으로 세그먼트 기준으로 구체화하면 많을수록 좋습니다. 예:

* 컨테이너가 있는 경우 세그먼트 맨 위에 있는 단일 컨테이너를 사용하는 것이 일련의 중첩 컨테이너보다 빠릅니다.
...
날짜 범위(자주색 구성 요소)가 패널 날짜 범위를 재정의합니다. 따라서 다른 날짜 범위를 열(예: 지난 달, 지난 주 및 어제 열)으로 사용하는 경우 패널 날짜 범위가 모든 열 날짜 범위를 확장할 필요가 없습니다. 자유 형식 테이블에서 사용되는 데이터 범위가 패널을 재정의하므로 간단히 어제로 설정할 수 있습니다. Analysis Workspace에서 날짜 범위 작업에 대한 자세한 내용은 [이 비디오](https://www.youtube.com/watch?v=ybmv6EBmhn0)에서 확인하십시오 .
...
프로젝트의 시각화 수를 줄입니다. 분석 작업 공간은 사용자가 추가하는 각 시각적 요소에 대해 백그라운드에서 많은 처리를 수행하고 있으므로 보고서 소비자에게 가장 중요한 시각적 효과를 우선 순위로 지정하고 필요한 경우 지원 시각적 요소를 보다 세부적인 별도의 프로젝트로 분류합니다.

## 시각화의 복잡성(세그먼트, 지표, 필터)

프로젝트에 기본적으로 추가된 시각화 유형(예: 폴아웃 및 자유 형식 테이블)은 프로젝트 성능에 크게 영향을 주지 않습니다. 처리 시간에 추가되는 시각화의 복잡성입니다. 시각화에 복잡성을 추가하는 요인은 다음과 같습니다.

* 위에 언급된 대로 요청된 데이터 범위
* 적용된 세그먼트 수;예를 들어, 자유 형식 테이블의 행으로 사용되는 세그먼트
* 복잡한 세그먼트 사용
* 자유 형식 테이블의 정적 항목 행 또는 열
* 자유 형식 테이블의 행에 적용된 필터
* 포함된 지표 수, 특히 세그먼트를 사용하는 계산된 지표
...
프로젝트 하나에 모든 것을 추가하려고 하지 말고, 이해 당사자의 특정 목적이나 그룹을 지원하는 고유한 프로젝트를 만드십시오. 태그를 사용하여 프로젝트를 주요 테마로 구성하고 관련 프로젝트를 이해 관계자 그룹과 공유할 수 있습니다.

더 많은 프로젝트 구성을 원하는 경우 프로젝트에 [직접 연결하는](https://www.youtube.com/watch?v=6IOEewflG2U) 것이 옵션이라는 것을 기억하십시오. 이해 당사자가 필요한 것을 쉽게 찾을 수 있도록 프로젝트의 내부 색인을 만드십시오.

하나의 작업 영역에서 여러 패널이 필요한 경우 저장하고 공유하기 전에 패널을 축소합니다. 프로젝트가 로드되면 분석 작업 공간은 확장된 패널에 대한 컨텐츠만 로드합니다. 축소된 패널은 사용자가 확장할 때까지 로드되지 않습니다. 이 방법은 다음 두 가지 방법으로 도움을 줍니다.

* 축소된 패널은 프로젝트의 전체 로드 시간에 저장됩니다
* 축소된 패널은 보고서를 이용하는 고객에게 논리적으로 프로젝트를 구성하는 좋은 방법입니다
