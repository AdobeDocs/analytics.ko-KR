...
| --- | --- | --- |
| [!UICONTROL 시스템 오류가 발생했습니다. 도움말 > 지원 티켓 제출에서 고객 지원 요청을 로그하여 오류 코드를 포함하십시오.] | Adobe에서 해결해야 하는 문제가 발생했습니다. | 오류 코드를 고객 지원에 제출합니다. |
| [!UICONTROL 예기치 않은 오류가 발생했습니다.프로젝트를 다시 새로 고쳐 보세요. 문제가 지속되면 추가적인 진단을 위해 이 오류 ID를 Adobe 고객 지원 센터에 제출하십시오.] | Adobe에서 해결해야 하는 문제가 발생했습니다. | 프로젝트를 새로 고쳐 보고 문제가 지속되면 오류 코드를 고객 지원 센터에 제출하십시오. |
