...
[!UICONTROL 빠른 인사이트], [!UICONTROL 비어 있음] 및 [!UICONTROL 자유 형식] 패널 은 분석을 시작하기에 좋은 위치이며, [!UICONTROL Analytics for Target], [!UICONTROL 속성], [!UICONTROL 미디어 Concurrent Viewer] 및 [!UICONTROL 세그먼트 비교] 좀 더 발전된 분석에 힘을 실어주다. 프로젝트에서 `"+"` 버튼을 사용할 수 있으므로 언제든지 빈 패널을 추가할 수 있습니다.
...
패널 필터링을 시작하려면 왼쪽 레일에서 패널 드롭 영역으로 세그먼트를 끌어서 놓습니다. 이 프로세스를 반복하여 패널에 필터를 더 추가합니다. 패널 상단에 필터가 나란히 나타납니다.
...
드롭다운 세그먼트를 사용하면 제어된 방식으로 데이터와 상호 작용할 수 있습니다. 예를 들어, 태블릿, 휴대폰 또는 데스크탑 별로 패널을 분할할 수 있도록 모바일 디바이스 유형에 대한 드롭다운 세그먼트를 추가할 수 있습니다.

드롭다운 세그먼트를 사용하여 여러 프로젝트를 하나로 통합할 수도 있습니다. 예를 들어 다른 국가 세그먼트가 적용된 동일한 프로젝트의 여러 버전이 있는 경우 모든 버전을 단일 프로젝트로 통합하고 국가 드롭다운 세그먼트를 추가할 수 있습니다.

![](assets/dropdown-filter-intro.png)

#### 정적 드롭다운 세그먼트 만들기

* 차원 항목을 사용하는 드롭다운 세그먼트의 경우 왼쪽 레일에서 단일 차원을 선택하여 패널 드롭 영역에 드롭합니다 **잠시 중단`[Shift]`**. 이렇게 하면 해당 차원과 연결된 모든 차원 항목이 있는 드롭다운 세그먼트가 만들어집니다.

  또는 드롭다운 세그먼트에 차원과 연결된 특정 차원 항목만 포함하게 하려면 왼쪽 레일에서 원하는 차원 옆에 있는 오른쪽 화살표 아이콘을 클릭합니다. 이 작업을 수행하면 사용 가능한 모든 차원 항목이 표시됩니다. 다음을 사용하여 이 목록에서 여러 차원 항목 선택 `[Shift + Click]` 또는 `[Ctrl + Click]`을 누른 채로 패널 드롭 영역에 드롭합니다 `[Shift]`.

* 단일 구성 요소 유형을 사용하는 드롭다운 세그먼트의 경우(예: 차원만, 세그먼트만 또는 지표만) 를 사용하여 왼쪽 레일에서 동일한 유형의 여러 항목을 선택합니다 `[Shift + Click]` 또는 `[Ctrl + Click]`을 클릭한 다음 패널 드롭 영역에 드롭합니다 **잠시 중단`[Shift]`**.

  선택한 구성 요소로 단일 드롭다운 세그먼트가 만들어집니다.

* 구성 요소 유형(예: 2개 지표 및 3개 필터)을 혼합하여 사용하는 드롭다운 세그먼트의 경우 다음을 사용하여 여러 구성 요소를 선택합니다 `[Shift + Click]` 또는 `[Ctrl + Click]`. **`[Shift]`**&#x200B;를 누른 상태에서 선택 항목을 패널 놓기 영역에 놓습니다. 모든 구성 요소 유형은 이 컨텍스트에서 별도의 드롭다운 세그먼트로 처리됩니다. 예를 들어, 선택 항목에 지표와 차원 항목을 모두 포함하는 경우 두 개의 별도 드롭다운 세그먼트가 생성됩니다. 한 드롭다운 세그먼트에는 차원 항목이 포함되고 다른 드롭다운 세그먼트에는 지표가 포함됩니다.

  ![모바일 고객 세그먼트 필드가 있는 패널 창에서 정적 드롭다운 세그먼트를 삭제할 수 있습니다. ](assets/create-dropdown.png)

드롭다운 세그먼트를 마우스 오른쪽 버튼으로 클릭하면 다음 옵션이 제공됩니다.

* **[!UICONTROL 삭제 드롭다운]**: 패널에서 드롭다운 세그먼트를 제거합니다.
* **[!UICONTROL 레이블 삭제]**: 드롭다운 세그먼트 위의 텍스트를 제거합니다.
* **[!UICONTROL 레이블 추가]**: 프로젝트에 드롭다운 세그먼트를 추가하면 레이블이 구성 요소 이름으로 자동으로 설정됩니다. 레이블을 삭제한 경우 이 옵션을 사용하여 다시 추가할 수 있습니다.
* **[!UICONTROL 선택 필요]**: 패널에서 세그먼트를 설정해야 합니다.
...
사용자는 다음 방법 중 하나로 드롭다운 메뉴를 사용하여 패널을 필터링할 수 있습니다.

* 드롭다운 메뉴에서 필터를 선택하여 패널에 단일 필터를 적용합니다.

* 드롭다운 메뉴에서 두 개 이상의 필터를 선택하여 패널에 여러 필터를 적용합니다. 패널은 선택한 필터를 포함하도록 필터링됩니다.

  ![여러 필터 선택](assets/dropdown-filter-multiselect.png)

### 동적 드롭다운 세그먼트

동적 드롭다운 세그먼트를 사용하면 패널의 보고 범위 내의 데이터와 다른 드롭다운 세그먼트의 값을 기반으로 사용 가능한 값을 결정할 수 있습니다. 예를 들어 다음을 사용하여 두 개의 동적 드롭다운을 만들 수 있습니다. [국가](/help/components/dimensions/countries.md) 차원 및 [도시](/help/components/dimensions/cities.md) 차원. 다음 목록에서 국가를 선택하면 [!UICONTROL 국가] 드롭다운 목록, [!UICONTROL 도시] 드롭다운 목록은 해당 국가 내의 도시만 표시하도록 동적으로 조정됩니다.
...
나중에 특정 차원 항목을 수집할 것으로 예상되는 경우 차원 항목을 수동으로 선택할 수 있습니다. 동적 드롭다운 세그먼트를 지워 값을 포함하지 않도록 하여 다른 동적 드롭다운 세그먼트에 더 많은 값을 포함할 수도 있습니다. 선택 **[!UICONTROL 모두 재설정]** 을 눌러 해당 패널에 대한 모든 드롭다운 세그먼트에서 선택 항목을 지웁니다.

동적 드롭다운 세그먼트를 만들려면 다음 작업을 수행하십시오.

* **`[Shift]`**&#x200B;를 누른 상태에서 단일 차원을 패널 놓기 영역으로 끌어다 놓습니다.
* 동적 드롭다운 세그먼트는 지표, 세그먼트 또는 날짜 범위에 사용할 수 없습니다.
* 드롭다운 세그먼트를 마우스 오른쪽 버튼으로 클릭하고 다음을 선택합니다. **[!UICONTROL 드롭다운 삭제]** 삭제.
