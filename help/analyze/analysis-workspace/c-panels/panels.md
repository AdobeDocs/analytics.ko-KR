...
| [세그먼트 비교](/help/analyze/analysis-workspace/c-panels/c-segment-comparison/segment-comparison.md) | 자동으로 모든 데이터 포인트에서 두 세그먼트를 빠르게 비교하여 관련 차이점을 찾습니다. |
...
분석 사용 사례에 따라 프로젝트 내에서 하나 또는 [여러 보고서 세트](/help/analyze/analysis-workspace/build-workspace-project/multiple-report-suites.md)를 사용할 수 있습니다.

보고서 세트 목록은 관련성을 기준으로 정렬되며, Adobe은 현재 사용자가 제품군을 얼마나 최근에 그리고 얼마나 자주 사용했는지를 기준으로 정의합니다. 및 조직 내에서 제품군을 사용하는 빈도입니다.

![패널의 보고서 세트 드롭다운 메뉴](assets/panel-report-suite.png)
...
1. 필요한 경우 **[!UICONTROL 고급 설정 표시]**&#x200B;를 선택하여 다음 작업을 수행하십시오.
   ...
   * **[!UICONTROL 패널 캘린더를 기준으로 날짜 범위 구성 요소 만들기]**. 비활성화되면 패널에서 사용되는 날짜 범위 구성 요소는 현재 시간을 기준으로 합니다. 활성화된 경우 패널에 사용되는 날짜 범위 구성 요소는 패널 달력을 기준으로 합니다.
   * **[!UICONTROL 순환 날짜 사용]**. 활성화된 경우 **[!UICONTROL 최근 7일 전체]**&#x200B;와 같은 사전 설정된 날짜 범위가 현재 날짜 및 시간 진행에 따라 동적으로 업데이트됩니다. 비활성화되면 해당 사전 설정이 적용되어 업데이트되지 않습니다.
...
놓은 세그먼트가 아닌 구성 요소(차원, 차원 항목, 지표, 날짜 범위)는 패널을 세그먼트화할 [빠른 세그먼트](#quick-segment)를 정의합니다. 세그먼트가 아닌 구성 요소를 사용하여 [세그먼트 빌더](/help/components/segmentation/segmentation-workflow/seg-quick.md)를 사용하지 않고 빠른 세그먼트를 만드십시오. 이러한 방식으로 만들어진 세그먼트는 자동으로 이벤트 수준 세그먼트로 정의되며 기본적으로 **[!UICONTROL 빠른 세그먼트]** 레이블이 지정됩니다.

또는 ![FilterAdd](/help/assets/icons/FilterAdd.svg)를 사용하여 빠른 세그먼트를 만들 수 있습니다.

빠른 세그먼트를 만들고 관리하는 방법은 [빠른 세그먼트](/help/components/segmentation/segmentation-workflow/seg-quick.md)를 참조하세요.
...
| 빠른 세그먼트를 편집하려면 다음을 수행하십시오. | 빠른 세그먼트 내부를 마우스로 가리킨 다음 ![편집](/help/assets/icons/Edit.svg)을 선택합니다. 자세한 내용은 [빠른 세그먼트](/help/components/segmentation/segmentation-workflow/seg-quick.md)를 참조하세요. |
...
데모 비디오는 ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [동적 드롭다운 메뉴](https://experienceleague.adobe.com/ko/docs/customer-journey-analytics-learn/tutorials/analysis-workspace/tips-and-tricks/dynamic-drop-downs){target="_blank"}를 참조하십시오.
