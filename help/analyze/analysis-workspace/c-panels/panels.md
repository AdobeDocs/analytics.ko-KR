...
* 구성 요소 유형(예: 2개 지표 및 3개 필터)을 혼합하여 사용하는 드롭다운 세그먼트의 경우 다음을 사용하여 여러 구성 요소를 선택합니다 `[Shift + Click]` 또는 `[Ctrl + Click]`. **`[Shift]`** 키를 누른 상태에서 선택 항목을 패널 드롭 영역에 놓습니다. 이 컨텍스트에서 모든 구성 요소 유형은 별도의 드롭다운 세그먼트로 처리됩니다. 예를 들어, 선택 항목에 지표와 차원 항목을 모두 포함하는 경우 두 개의 별도 드롭다운 세그먼트가 생성됩니다. 한 드롭다운 세그먼트에는 차원 항목이 포함되고 다른 드롭다운 세그먼트에는 지표가 포함됩니다.
