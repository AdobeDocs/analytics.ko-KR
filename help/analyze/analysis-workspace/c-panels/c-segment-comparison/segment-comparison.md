...
선택된 각 세그먼트의 상대적 크기와 서로 겹치는 정도를 벤 다이어그램을 사용하여 나타냅니다. 시각적 개체 위로 마우스를 가져가면 겹치거나 겹치지 않는 각 섹션에 방문자 수를 확인할 수 있습니다. 또한 오버랩을 마우스 오른쪽 버튼으로 클릭하여 추가적인 분석을 위해 새로운 세그먼트를 만들 수 있습니다. 두 세그먼트가 상호 배타적인 경우에는 두 원 간에 겹치는 부분이 없습니다(일반적으로 히트 컨테이너를 사용하여 세그먼트로 표시됨).
