...
* **속성 비교 테이블**: 막대 차트와 동일한 데이터를 테이블로 표시합니다. 이 표에서 다른 열 또는 행을 선택하면 막대 차트와 패널의 다른 여러 가지 시각화가 필터링됩니다. 이 테이블은 작업 공간의 다른 자유 형식 테이블과 유사하며, 지표, 세그먼트 또는 분류와 같은 구성 요소를 추가할 수 있습니다.
...
* **트렌드 성능**:기본적으로 인접한 자유 형식 테이블에 나열된 첫 번째 차원에 대한 속성 모델별 전환 성과 트렌드를 보여줍니다. 자유 형식 테이블에서 다른 차원 행을 선택하여 선택한 차원에 대한 트렌드(예: 소셜 캠페인 및 유료 검색에 대한 각 속성 모델에 대한 총 매출액)를 표시할 수 있습니다. 또는 자유 형식 테이블의 모든 지표 및 속성 유형 조합에 대한 열에서 셀을 선택하여 지정된 속성 모델에 대한 트렌드 성과를 차원별 값(예: 마지막 터치 및 첫 번째 터치 속성을 사용한 마케팅 채널별 총 매출액)으로 볼 수 있습니다.
