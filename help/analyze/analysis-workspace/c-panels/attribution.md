...
* **Dimension 겹침 차트**: 상위 3개 차원 항목과 이들이 전환에 공동 참여하는 빈도를 보여주는 벤 다이어그램. 예를 들어 버블 오버랩의 크기는 방문자가 두 차원 항목에 노출되었을 때 얼마나 자주 전환되었는지를 나타냅니다. 인접 자유 형식 테이블에서 다른 행을 선택하면 선택 사항을 반영하도록 시각화가 업데이트됩니다.
