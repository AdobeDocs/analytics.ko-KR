...
* **** 사용자 지정 조회 창:보고 날짜 범위 이상으로 최대 90일까지 속성 창을 확장할 수 있습니다. 사용자 지정 조회 창은 보고 기간의 각 변환에서 평가됩니다. 예를 들어, 2월 20일에 발생하는 전환의 경우 10일의 조회 창은 속성 모델에서 2월 10일부터 20일까지 모든 차원 터치포인트를 평가합니다.
...
* 7일 동안 **첫 번째 터치** 및 **사용자 지정 룩백 창을**&#x200B;사용하는 속성은 최근 두 번의 방문만 봅니다. 이 보고 창에서는 소셜 미디어 링크가 먼저 사용되었으므로 $50의 구매에 대해 100% 크레딧을 받습니다.
...
* 7일의 **선형** 및 **사용자 지정 룩백 창을** 사용하면 크레딧이 소셜, 이메일 및 디스플레이로 구분됩니다. 이러한 각 채널은 이 구매에 대해 $12.50의 크레딧을 받습니다. 유료 검색은 지정된 조회 창 내에서 발생하지 않으므로 제외됩니다.
...
* **시간 가치 하락**&#x200B;과 **방문자 전환 확인 기간**&#x200B;을 사용하는 경우 크레딧은 유료 검색, 소셜, 이메일 및 디스플레이 간에 나눠집니다. 다음과 같이 기본 7일 반감기를 사용합니다.
   * 디스플레이 터치 포인트와 전환 간 0일 간격. `2`<sup>`(-0/7)`</sup> `= 1`
   * 이메일 터치 포인트와 전환 간 0일 간격. `2`<sup>`(-0/7)`</sup> `= 1`
   * 소셜 터치 포인트와 전환 간 6일 간격. `2`<sup>`(-6/7)`</sup> `= 0.552`
   * 유료 검색 터치 포인트와 전환 간 9일 간격. `2`<sup>`(-9/7)`</sup> `= 0.41`
