...
| **[!UICONTROL 지정되지 않음 (없음) 포함]** 설정은 합계에 어떤 영향을 줍니까? | 지정되지 않음(없음) 포함 설정이 선택 취소되면 없음/지정되지 않음 행이 테이블, 테이블 합계에서 제거되고, [&#39;합계&#39; 지표 유형](/help/components/calculated-metrics/workflow/c-build-metrics/m-metric-type-alloc.md)을 사용하는 임의의 계산된 지표로 전달됩니다. |
