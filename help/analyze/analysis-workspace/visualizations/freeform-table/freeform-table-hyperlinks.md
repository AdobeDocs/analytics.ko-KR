...
차원 항목에 대한 하이퍼링크를 만들어 Analysis Workspace의 자유 형식 테이블 내에서 해당 항목을 클릭 가능하게 할 수 있습니다.

이 기능은 다음 유형의 차원 항목에 대한 하이퍼링크를 만들 때 특히 유용합니다.

* URL 값이 있는 차원 항목(예: 페이지 URL 차원).

* URL 값이 있는 분류를 포함하는 차원 항목(예: 페이지 URL 차원의 분류가 있는 페이지 이름 차원).
...
1. Analysis Workspace의 자유 형식 테이블에서 다음 중 하나를 수행합니다.
   ...
   * **차원 열의 모든 차원 항목에서 하이퍼링크 제거:**
   ...
   단일 차원 항목을 선택한 경우 단일 차원 항목에서 하이퍼링크가 제거됩니다. 또는 차원 열 머리글에서 차원 이름을 선택한 경우 모든 차원 항목에서 하이퍼링크가 제거됩니다.
