...
Analysis Workspace에서 다음 유형의 열을 기준으로 자유 형식 테이블의 데이터를 정렬할 수 있습니다.

* 모든 지표 열

* 모든 차원 열(문자열 기반 차원 제외)

여러 열을 동시에 정렬할 수도 있습니다.
...
여러 열에 대해 데이터를 정렬할 때 데이터는 각 열에 할당한 우선순위에 따라 정렬됩니다. 정렬 아이콘 ![정렬 우선 순위 아이콘](assets/sort-priority-icon.png) 옆에 우선 순위 번호가 표시됩니다.
...
각 열에 정렬 우선 순위를 할당하면 데이터가 테이블에 표시되는 방식을 정확하게 제어할 수 있습니다. 이 예에서 정보는 먼저 일별로, 그 다음 페이지 보기 횟수별로, 그 다음 방문 횟수별로, 그리고 마지막으로 콘텐츠 속도별로 정렬됩니다.
