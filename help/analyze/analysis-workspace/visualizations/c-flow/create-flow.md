...
1. 프로젝트에 빈 패널을 추가하고 왼쪽 레일에서 시각화 아이콘을 클릭합니다.
   ...
   의 &quot;패널에 시각화 추가&quot; 섹션에 설명된 방법으로 시각화를 추가합니다. [시각화 개요](/help/analyze/analysis-workspace/visualizations/freeform-analysis-visualizations.md).
...
플로우 구성의 요약이 다이어그램의 맨 위에 나타납니다. 다이어그램에서 패스의 두께는 해당 활동에 비례합니다. 활동이 많은 패스는 활동이 적은 패스보다 두껍게 표시됩니다.

데이터를 추가 드릴다운하려면 다음과 같은 몇 가지 옵션이 있습니다.

* 흐름 다이어그램은 대화형입니다. 표시되는 세부 사항을 변경하려면 마우스를 다이어그램 위에 놓습니다.

* 다이어그램에서 노드를 선택하면 해당 노드에 대한 세부 사항이 표시됩니다. 축소하려면 노드에서 를 다시 선택합니다.
...
각 열 위에 마우스를 가져다 대면 필터가 나타납니다. 필터를 선택하면 현재 자유 형식 테이블에 있는 것과 동일한 필터 대화 상자가 표시됩니다. 이 필터는 자유 형식 테이블과 동일하게 작동합니다.
...
* 필터를 제거하려면 필터 아이콘을 선택하여 필터 메뉴를 엽니다. 적용된 필터를 제거한 다음 선택 **[!UICONTROL 저장]**. 흐름은 필터링되지 않은 이전 상태로 돌아가야 합니다.
