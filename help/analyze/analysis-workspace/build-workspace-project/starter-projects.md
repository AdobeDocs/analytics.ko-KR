...
관리자 권한이 있는 사용자는 자신이 만든 프로젝트를 사용자 지정 템플릿으로 변환할 수 있습니다. 방법은 다음과 같습니다.
...
1. Go to **[!UICONTROL Project]** > **[!UICONTROL Save As Template]**.
   ...
   프로젝트가 현재 프로젝트 이름 아래에 저장되고 단어(템플릿)가 괄호 안에 표시됩니다. 관리자는 템플릿을 편집하여 이 이름을 변경할 수 있습니다.
...
| 작업 | 설명 |
|--- |--- |
| 편집 템플릿 | 관리자가 데이터 소스를 변경하거나 구성 요소, 시각화, 날짜 범위 등을 수정하여 템플릿을 편집할 수 있습니다.  사용자 지정 템플릿을 편집하려면<ul><li>Analysis Workspace에서 사용자 지정 템플릿 목록을 표시하고 하나를 선택한 다음 템플릿 편집을 클릭합니다. 또는</li><li>Analytics에서 구성 요소 >  프로젝트로 이동한 다음 템플릿에 대해 필터링합니다. 편집할 템플릿의 이름을 클릭하십시오.</li></ul>**참고:** 상황에 따라 템플릿을 편집한 후에는 다음 두 가지 옵션이 있습니다.저장, 다른 이름으로 저장. 다음은 이러한 차이점입니다.<ul><li>**저장:** 모든 사용자에 대한 사용자 지정 템플릿을 업데이트합니다. 다른 사람이 이 사용자 지정 템플릿에서 프로젝트를 만들면 사용자가 변경한 내용이 표시됩니다.</li><li>**다른 이름으로 저장:** 변경 사항이 있는 사용자 지정 템플릿의 복사본을 만듭니다. (공유 > 프로젝트 공유 메뉴 항목 사용이 비활성화되어 있으면 편집 모드 상태임을 알 수 있습니다.)</li></ul> |
| 템플릿 검색 | 사용자 지정 템플릿 대화 상자에서 템플릿 검색을 클릭합니다. |
| 템플릿 정렬 | 템플릿을 알파벳순, 연관성 및 작성 날짜별로 정렬할 수 있습니다.  사용자 지정 템플릿 대화 상자에서 정렬:을 클릭합니다. |
...
작업 공간을 처음 열면 왼쪽 레일에서 템플릿을 사용할 수 있습니다. 분석 작업 공간 템플릿은 일반적인 사용 사례를 다룹니다. 이 세그먼트는 해당 세로로 그룹화되고 선택한 보고서 세트에 따라 다른 차원, 세그먼트, 지표 및 시각화로 채워집니다.
...
* 미국/캐나다 및 기타 지역의 데이터를 세그먼트화할 수 있습니다. Device Co-op는 현재 북아메리카에서만 사용할 수 있습니다.
* 사람 및 고유 방문자 수 지표를 나란히 비교합니다.
* 사람 지표가 고유 방문자 수의 백분율로 얼마나 작은지 계산하는 계산된 지표인 &quot;압축률&quot;을 참조하십시오.
* 고객이 사용하는 디바이스 유형 합계 비교
* 1인당 평균 디바이스 수를 확인합니다.
* 사람 지표로 세그먼트 스택을 사용하는 방법을 알아봅니다.
* 사용자 환경에서 Experience Cloud ID를 사용하여 사람 지표의 효율성을 향상시키는 방법을 살펴보십시오.
