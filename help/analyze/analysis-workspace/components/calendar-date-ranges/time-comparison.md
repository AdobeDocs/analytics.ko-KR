...
분석하려면 컨텍스트가 필요하며, 해당 컨텍스트를 이전 기간에서 제공하는 경우가 종종 있습니다. 예를 들어, "작년 이맘때보다 얼마나 좋아질까요/나빠질까요?"라는 질문은 비즈니스를 이해하는 데 기본적인 질문입니다. 날짜 비교에는 지정된 기간과 비교하여 백분율 변경을 표시하는 "차이" 열이 자동으로 포함됩니다.
