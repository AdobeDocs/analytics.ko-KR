---
description: 랜딩 페이지가 Workspace 우산 아래에 있는 단일 인터페이스 및 액세스 포인트로 Analysis Workspace과 Reports & Analytics를 모두 통합하는 방법을 설명합니다.
title: Adobe Analytics 랜딩 페이지
role: User, Admin
...
# Adobe Analytics 랜딩 페이지

Adobe Analytics용 랜딩 페이지는 Analysis Workspace과 Reports &amp; Analytics를 모두 Workspace 우산 아래 단일 인터페이스 및 액세스 포인트로 가져옵니다. 프로젝트 관리자 홈 페이지, 업데이트된 보고서 메뉴, 현대화된 보고서 및 학습 섹션을 통해 보다 효과적으로 시작할 수 있습니다.
...
* Analytics 상단 메뉴: 이제 대부분의 보고서가 왼쪽 레일의 [!UICONTROL 보고서] 메뉴에 있습니다.
...
![모두 랜딩](assets/landing-all.png)

>[!NOTE]
>
>다음 설정 중 일부는 세션 전체에서 그리고 세션 간에 유지(기억)됩니다. 예: 현재 사용 중인 탭, 선택된 필터, 선택된 열, 열 정렬 방향. 그러나 검색 결과는 유지되지 않습니다.
...
| 간단히/자세히 표시 | 배너 표시 안 함과 표시 간 전환: ![위쪽 배너](assets/top-banner.png) |
...
| 이름 | 프로젝트의 이름입니다 |
| 고정 아이콘 | 항목을 고정할 수 있으며 이 항목이 항상 목록 맨 위에 나타납니다.** |
...
메뉴 및 하위 메뉴는 다음과 같습니다. 특정 보고서를 찾을 수 없는 경우 “페이지에서 검색”을 수행하여 찾습니다.
...
| **[!UICONTROL 회사 보고서]** | 회사가 사용자의 사용을 위해 만든 보고서입니다. |
...
| Beta 프로그램 UI에서 수행하는 작업이 프로덕션 Workspace 환경으로 이어집니까? | 예, Beta에서 수행된 모든 작업은 이전/현재 Workspace 환경으로 이월됩니다. |
