...
다음 항목 선택 [!UICONTROL **작업**] 탭에서는 각 피드가 만드는 개별 작업을 볼 수 있습니다. [데이터 피드 작업 관리](df-manage-jobs.md)를 참조하십시오.

### 추가

다음 [!UICONTROL 추가] 버튼을 사용하면 새 피드를 만들 수 있습니다. 다음을 참조하십시오 [데이터 피드 만들기](create-feed.md) 추가 정보.
...
* **상태**: 피드의 상태입니다.
   ...
   * 비활성: &#39;일시 중지됨&#39; 또는 &#39;보류 중&#39; 상태에 해당합니다. 채우기 피드(내역 데이터만 처리하는 피드)가 다시 활성화되면 작업 전달이 중지된 시점부터 다시 시작됩니다. 라이브 피드가 다시 활성화되면 작업 전달이 중지된 시점부터 다시 시작됩니다.
...
* **활성화**: 비활성 피드에만 사용할 수 있습니다. 피드 채우기(내역 데이터만 처리하는 피드)는 중지된 위치에서 데이터 처리를 재개하고 필요한 경우 날짜를 다시 채웁니다. 라이브 피드는 현재 시간부터 데이터 처리를 재개합니다.
