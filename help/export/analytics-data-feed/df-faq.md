...
데이터 피드 파일 이름은 보고서 세트 ID와 날짜로 이루어집니다. 동일한 RSID와 날짜에 대해 구성된 두 피드의 파일 이름은 모두 동일합니다. 이러한 피드가 동일한 위치에 전달되면 한 파일이 다른 피드를 덮어씁니다. 파일 덮어쓰기를 방지하기 위해, 동일한 위치에서 기존의 피드를 덮어쓸 가능성이 있는 피드는 만들 수 없습니다.
...
## 데이터가 언제 처리됩니까? {#section_6346328F8D8848A7B81474229481D404}

시간별 또는 일별 데이터를 처리하기 전에 데이터 피드는 시간 (일 또는 시간) 내에 데이터 수집에 입력된 모든 히트가 Data Warehouse에 작성될 때까지 대기합니다. 이후 데이터 피드는 해당 기간에 맞는 타임스탬프를 포함한 데이터를 수집하여 압축하고 FTP를 통해 전송합니다. 시간별 피드의 경우 파일은 해당 시간 이후 15~30분 내에 Data Warehouse로 작성되지만 기간이 정해져 있지는 않습니다. 기간에 해당하는 타임스탬프를 가진 데이터가 없는 경우 다음 기간을 다시 시도하게 됩니다. 현재 데이터 피드 프로세스는 `date_time` 필드를 사용하여 어떤 히트가 시간에 속하는지 결정합니다. 이 필드는 보고서 세트의 시간대를 기반으로 합니다.
...
예: 새 데이터 피드는 2021년 3월 9일에 생성되고 2021년 1월부터 3월 9일까지는 “시간별”로 전달됩니다. 그러나 2021년 3월 2일 이전의 &quot;시간별&quot; 파일은 하나의 &quot;일별&quot; 파일로 결합됩니다. 생성일로부터 7일이 초과되지 않은 데이터에서만 “시간별” 파일을 추출할 수 있습니다. 이 경우 3월 2일부터 3월 9일까지 파일을 추출할 수 있습니다.

## 시간별 데이터 피드에 일광 절약 시간제가 주는 영향은 무엇입니까? {#section_70E867D942054DD09048E027A9474FFD}
...
## Analytics는 FTP 전송 실패를 어떻게 처리합니까? {#section_4BD44E9167F0494FB2B379D2BA132AD8}

FTP 전송이 실패하는 경우(로그인이 거부됨, 연결이 끊김, 할당량 오류 또는 기타 문제로 인해) Adobe은 자동으로 데이터를 연결하고 최대 3개의 개별 데이터로 보냅니다. 그래도 문제가 계속되면 피드가 실패된 것으로 표시되고 이메일 알림이 발송됩니다.

전송이 실패하면 성공할 때까지 작업을 다시 실행할 수 있습니다.

FTP 사이트에 데이터 피드를 표시하는 데 문제가 있는 경우 [작업 문제 해결](jobs-troubleshooting.md)을 참조하십시오.
...
Amazon S3에 대한 일반적인 사용 사례는 AWS (Amazon 웹 서비스) 계정 소유자가 버킷을 만들고, 해당 버킷에 개체를 만들 권한이 있는 사용자를 만든 다음 해당 사용자에 대한 자격 증명을 제공하는 것입니다. 이 경우, 사용자의 개체는 동일한 계정에 속하고, 계정 소유자는 암시적으로 객체에 대한 모든 제어(읽기, 삭제 등)를 갖습니다. 이 프로세스는 FTP 전송 작동 방식과 유사합니다.

또한 사용자는 AWS를 사용하여 다른 사용자 계정에 속하는 버킷에 개체를 만들 수 있습니다. 예를 들어, userA와 userB라는 두 AWS 사용자가 동일한 AWS 계정에 속하지 않고 다른 버킷에서 개체를 만들려고 하는 경우, userA가 &quot;bucketA&quot;라는 버킷을 만드는 경우, 사용자가 버킷을 소유하지 않더라도 userB가 명시적으로 bucketA에서 개체를 만들 수 있도록 하는 버킷 정책을 만들 수 있습니다. 이 정책은 userA 및 userB가 자격 증명을 교환하지 않아도 되므로 유용할 수 있습니다. 대신 userB는 userA에게 계정 번호를 제공하고, userA는 &quot;userB가 bucketA에 개체를 만들 수 있도록 허용&quot;하는 버킷 정책을 만듭니다.

그러나 개체는 상위 버킷의 권한을 상속하지 않습니다. 따라서 userB가 개체를 userA의 버킷에 업로드하는 경우 userB는 해당 개체를 여전히 &quot;소유&quot;하며, 기본적으로 userA가 버킷을 소유하더라도 해당 개체에 대한 권한을 부여받지 않습니다. UserB는 여전히 개체 소유자이므로 userA에게 명시적으로 권한을 부여해야 합니다. 이 권한을 부여하려면 userB가 userB에 의해 &quot;소유&quot;된 개체라도 버킷 소유자(userA)에게 객체에 대한 모든 권한(읽기, 쓰기, 삭제 등)이 부여되도록 지정하는 BucketOwnerFullControl ACL을 사용하여 개체를 업로드해야 합니다.

>[!NOTE]
>
>[!DNL Analytics] 버킷에 새 개체를 전체 제어할 수 있도록 버킷 소유자에게 필요한 정책이 있는지, 또는 버킷 소유자가 데이터를 작성하는 사용자와 다른 계정에 있는 경우에도 영향을 주지 않는지 여부를 확인하지 않습니다. 대신 [!DNL Analytics]은(는) 각 피드 업로드를 통해 BucketOwnerFullControl ACL에 버킷 소유자를 자동으로 추가합니다.
