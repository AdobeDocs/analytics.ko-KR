...
1. [!UICONTROL **추가**]&#x200B;를 선택합니다.
   ...
   페이지가 [!UICONTROL **피드 정보**], [!UICONTROL **대상**] 및 [!UICONTROL **데이터 열 정의**]&#x200B;의 세 가지 기본 범주로 표시됩니다.
1. [!UICONTROL **피드 정보**] 섹션에서 다음 필드를 작성합니다.
   ...
   | [!UICONTROL **시작 및 종료 날짜**] | 시작 날짜는 데이터 피드를 시작할 날짜를 나타냅니다. 이전 데이터에 대한 데이터 피드 처리를 즉시 시작하려면 이 날짜를 데이터가 수집되는 과거의 날짜로 설정하십시오. 시작 및 종료 날짜는 보고서 세트의 시간대를 기반으로 합니다. |
   | [!UICONTROL **연속 피드**] | 이 확인란을 선택하면 종료 날짜가 제거되어 피드가 무기한 실행될 수 있습니다. 한 피드가 이전 데이터 처리를 완료하면 한 피드가 지정된 시간 또는 날 동안 데이터 수집이 완료되기를 기다립니다. 현재 시간 또는 날이 끝나면 지정된 지연 후 처리가 시작됩니다. |
...
1. [!UICONTROL **대상**] 섹션의 [!UICONTROL **유형**] 드롭다운 메뉴에서 데이터를 전송할 대상을 선택합니다.
   ...
   >
   >* 보고서 대상에 클라우드 계정을 사용하는 것이 좋습니다. [기존 FTP 및 SFTP 계정](#legacy-destinations)을 사용할 수 있지만 권장하지 않습니다.
   ...
   >   
   >   * [Adobe Analytics 분류 데이터를 가져오는 중](/help/components/locations/locations-manager.md)(분류 데이터를 가져오도록 구성된 모든 위치를 사용할 수 없음).
   >   
   >   * 위치 관리자의 [구성 요소 > 위치](/help/components/locations/configure-import-accounts.md)에서
   >
   ...
   데이터 피드를 만들 때 다음 대상 유형을 사용하십시오. 구성 지침을 보려면 대상 유형을 확장합니다. (추가 [기존 대상](#legacy-destinations)은 사용할 수도 있지만 권장되지 않습니다.)
   ...
   1. [!UICONTROL **대상**] 섹션의 [!UICONTROL **유형**] 드롭다운 메뉴에서 [!UICONTROL **Amazon S3**]&#x200B;을(를) 선택합니다.
   ...
   1. (조건부) Amazon S3 계정(및 해당 계정에서의 위치)이 이미 Adobe Analytics에 구성된 경우 이 계정을 데이터 피드 대상으로 사용할 수 있습니다.
...
+++
   ...
   1. 아직 인증하지 않았다면 Adobe Analytics에서 인증에 사용할 수 있는 Azure 애플리케이션을 만든 다음 IAM(액세스 제어)에서 액세스 권한을 부여합니다.
      ...
      자세한 내용은 Azure Active Directory 응용 프로그램을 만드는 방법에 대한 [Microsoft Azure 설명서](https://learn.microsoft.com/en-us/azure/active-directory/develop/howto-create-service-principal-portal)를 참조하세요.

   1. Adobe Analytics Admin Console의 [!UICONTROL **대상**] 섹션의 [!UICONTROL **유형**] 드롭다운 메뉴에서 [!UICONTROL **Azure RBAC**]&#x200B;을(를) 선택합니다.
   ...
   1. (조건부) Azure RBAC 계정(및 해당 계정의 위치)이 이미 Adobe Analytics에 구성된 경우 데이터 피드 대상으로 사용할 수 있습니다.
   ...
   1. (조건부) 이전에 Azure RBAC 계정을 추가하지 않은 경우:
      ...
      1. [!UICONTROL **계정 추가**]&#x200B;를 선택하고 다음 정보를 지정합니다.
         ...
         | 필드 | 함수 |
         |---------|----------|
         | [!UICONTROL **계정 이름**] | Azure RBAC 계정의 이름입니다. 이 이름은 [!UICONTROL **계정 선택**] 드롭다운 필드에 표시되며, 사용자가 선택한 모든 이름을 사용할 수 있습니다. |
         | [!UICONTROL **계정 설명**] | Azure RBAC 계정에 대한 설명입니다. 이 설명은 [!UICONTROL **계정 선택**] 드롭다운 필드에 표시되며, 사용자가 선택한 모든 이름일 수 있습니다. |
      ...
      1. [!UICONTROL **위치 추가**]&#x200B;를 선택한 후 다음 정보를 지정하십시오.
         ...
         | 필드 | 함수 |
         |---------|----------|
         | [!UICONTROL **이름**] | 위치의 이름입니다. 이 이름은 [!UICONTROL **위치 선택**] 드롭다운 필드에 표시되며, 사용자가 선택한 모든 이름을 사용할 수 있습니다. |
         | [!UICONTROL **설명**] | 위치에 대한 설명. 이 설명은 [!UICONTROL **위치 선택**] 드롭다운 필드에 표시되며, 사용자가 선택한 모든 이름일 수 있습니다. |
...
+++
   ...
   1. Adobe Analytics 관리 콘솔의 [!UICONTROL **대상**] 섹션에서 [!UICONTROL **Azure SAS**]&#x200B;를 선택합니다.
   ...
   1. (조건부) Azure SAS 계정(및 해당 계정의 위치)이 이미 Adobe Analytics에 구성된 경우, 이를 데이터 피드 대상으로 사용할 수 있습니다.
...
+++
   ...
   1. Adobe Analytics Admin Console의 [!UICONTROL **대상**] 섹션에서 [!UICONTROL **Google Cloud Platform**]&#x200B;을(를) 선택합니다.
   ...
   1. (조건부) Google Cloud Platform 계정(및 해당 계정의 위치)이 Adobe Analytics에 이미 구성된 경우, 이를 데이터 피드 대상으로 사용할 수 있습니다.
      ...
      1. [!UICONTROL **저장**] > [!UICONTROL **저장**]&#x200B;을 선택합니다.
         ...
         이제 대상이 지정한 Google Cloud Platform 위치로 데이터를 보내도록 구성되었습니다.
...
1. 오른쪽 상단에서 [!UICONTROL **저장**]&#x200B;을 선택합니다.
...
>[!IMPORTANT]
>
>이 섹션에 설명된 대상은 기존 대상으로 권장되지 않습니다. 대신 데이터 피드를 만들 때 Amazon S3, Google Cloud Platform, Azure RBAC 또는 Azure SAS 대상 중 하나를 사용합니다. 이러한 각 권장 대상에 대한 자세한 내용은 [데이터 피드 만들기 및 구성](#create-and-configure-a-data-feed)을 참조하십시오.
...
* s3:PutObjectAcl
  ...
  >[!NOTE]
  >
  >Amazon S3 버킷에 대한 각 업로드의 경우 [!DNL Analytics]은(는) 버킷에 필요한 정책이 있는지 여부에 관계없이 버킷 소유자를 BucketOwnerFullControl ACL에 추가합니다. 자세한 내용은 &quot;[Amazon S3 데이터 피드에 대한 BucketOwnerFullControl 설정은 무엇입니까?](df-faq.md#BucketOwnerFullControl)&quot;을(를) 참조하십시오.
