...
| `country` | 에 있는 값을 나타내는 숫자 ID입니다 `country.tsv` 조회. Reports &amp; Analytics의 최상위 도메인 보고서에 사용됩니다. | smallint 부호 없음 |
...
| `geo_city` | IP를 기반으로 하여 히트가 발생한 도시의 이름입니다. [도시](/help/components/dimensions/cities.md) 차원에 사용됩니다. | char (32) |
| `geo_country` | IP를 기반으로 히트가 발생한 국가의 약어입니다. [국가](/help/components/dimensions/countries.md) 차원에 사용됩니다. | char (4) |
| `geo_dma` | IP를 기반으로 하며 히트가 발생한 인구 통계 영역의 숫자 ID입니다. [US DMA](/help/components/dimensions/us-dma.md) 차원에 사용됩니다. | int 부호 없음 |
| `geo_region` | IP를 기반으로 하여 히트가 발생한 주 또는 지역의 이름입니다. [지역](/help/components/dimensions/regions.md) 차원에 사용됩니다. | char (32) |
| `geo_zip` | IP를 기반으로 하여 히트가 발생한 우편 번호입니다. [우편 번호](/help/components/dimensions/zip-code.md) 차원을 채우는 데 도움이 됩니다. 참조: `zip`. | varchar (16) |
...
| `visid_type` | 외부용이 아닙니다. Adobe가 최적화 처리를 위해 내부적으로 사용합니다. 방문자를 식별하는 데 사용된 방법을 나타내는 숫자 ID입니다.<br>0: 사용자 지정 방문자 ID 또는 알 수 없음/적용할 수 없음<br>1: IP 및 사용자 에이전트 폴백 <br>2: HTTP 모바일 가입자 헤더 <br>3: 존 쿠키 값 (`s_vi`) <br>4: 대체 쿠키 값 (`s_fid`) <br>5: ID 서비스 | tinyint 부호 없음 |
