...
| browser | 브라우저의 숫자 ID입니다. browser.tsv 조회 테이블을 참조합니다. | 부호 없음 |
| browser_height | 브라우저 창의 높이(픽셀). | 미부호 없는 |
| browser_width | 브라우저 창의 폭(픽셀). | 미부호 없는 |
...
| click_sourceid | 더 이상 사용되지 않습니다. 클릭한 링크의 페이지에 있는 위치의 숫자 ID입니다. 이전 Clickmap 도구의 일부입니다. | 부호 없음 |
...
| geo_dma | IP를 기반으로 한, 히트가 발생한 인구 통계학적 영역의 숫자 ID입니다. Adobe에서는 IP 주소를 인구 통계학적 영역에 일치시키기 위해 Digital Envoy와 파트너 관계를 맺습니다. | 부호 없음 |
| geo_region | IP를 기반으로 한, 히트가 발생한 시/도 또는 지역의 이름입니다. Adobe에서는 IP 주소를 시/도 또는 지역에 일치시키기 위해 Digital Envoy와 파트너 관계를 맺습니다. | char(32) |
| geo_zip | IP를 기준으로 히트가 생성된 우편번호입니다. Adobe에서는 IP 주소를 우편 번호에 일치시키기 위해 Digital Envoy와 파트너 관계를 맺습니다. | varchar(16) |
| hier1 - hier5 | 계층 변수에서 사용됩니다. 구분된 값 목록을 포함합니다. 구분 기호는 보고서 세트 설정에서 선택합니다. | varchar(255) |
| hit_source | 히트가 발생한 소스를 나타냅니다. <br>1:타임스탬프 <br>2 없이 표준 이미지 요청:타임스탬프 <br>3이 있는 표준 이미지 요청:타임스탬프 <br>4를 사용한 라이브 데이터 소스 업로드:사용되지 않음 <br>5:일반 데이터 소스 업로드 <br>6:전체 처리 데이터 소스 업로드 <br>7:TransactionID 데이터 소스 업로드 <br>8:더 이상 사용되지 않음;Adobe Advertising Cloud 데이터 소스 <br>9의 이전 버전:더 이상 사용되지 않음;Adobe Social 요약 지표 | tinyint 부호 없음 |
| hit_time_gmt | 히트 Adobe 데이터 수집 서버의 타임스탬프가 Unix 시간을 기준으로 히트를 받았습니다. | int |
| hitid_high | 히트를 고유하게 식별하기 위해 hitid_low와 함께 사용됩니다. | 부호가 없는 |
| hitid_low | 히트를 고유하게 식별하기 위해 hitid_high와 함께 사용됩니다. | 부호가 없는 |
...
| java_enabled | Java가 사용되는지 여부를 나타내는 플래그입니다. <br>Y:활성화 <br>N:사용 안 <br>함 U:알 수 없음 | char(1) |
| javascript | j_jscript를 기반으로 하는 JavaScript 버전의 조회 ID입니다. 조회 테이블을 사용합니다. | tinyint 부호 없음 |
| language | 언어의 숫자 ID입니다. languages.tsv 조회 테이블을 사용합니다. | 미부호 없는 |
| last_hit_time_gmt | 이전 히트의 타임스탬프(Unix 시간)입니다. 마지막 방문 이후의 일수 차원을 계산하는 데 사용됩니다. | int |
| last_purchase_num | 고객 충성도 차원에 사용되는 변수입니다. 방문자가 수행한 이전 구매 횟수를 나타냅니다. <br>0:이전 구매 없음(고객이 아님) <br>1:1 이전 구매(신규 고객) <br>2:2회 이전 구매(재방문 고객) <br>3:3개 이상의 이전 구매(단골 고객) | 부호 없음 |
...
| mobileappid | 모바일 앱 ID입니다. 애플리케이션 이름과 버전을 다음 형식으로 저장:[AppName] 번들 [버전] | varchar(255) |
| mobileappperformanceceappid | Apteligent 데이터 커넥터에서 사용됩니다. Apteligent에서 사용되는 앱 ID입니다. | varchar(255) |
| mobileappperformancecreshid | Apteligent 데이터 커넥터에서 사용됩니다. Apteligent에서 사용되는 충돌 ID입니다. | varchar(255) |
| mobileappstorobjectid | Appfigures 데이터 커넥터에서 사용됩니다. 앱스토어 개체 ID | varchar(255) |
| mobilebeaconmajor | 모바일 서비스 비콘 주 | varchar(100) |
| mobilebeaconminor | 모바일 서비스 비콘 마이너 | varchar(100) |
...
| mobiledeplinkid | 컨텍스트 데이터 변수 a.<span>deeplink</span>.id에서 수집됩니다. 모바일 획득 링크의 식별자로 획득 보고서에 사용됩니다. | varchar(255) |
...
| mobilelaunchessincelastupgrade | 컨텍스트 데이터 변수 a.LaunchesSinceUpgrade에서 수집됩니다. 마지막 업그레이드 이후 시작 수를 보고합니다. | varchar(255) |
...
| mobilemessagepushoptin | 컨텍스트 데이터 변수 a.push.optin에서 수집됩니다. 사용자가 푸시 메시지를 열기 위해 로그인할 때 "true"로 설정합니다.그렇지 않으면 값이 "false"입니다. | varchar(255) |
| mobilemessagepushpayloadid | 컨텍스트 데이터 변수 a.push.payloadid에서 수집됩니다. 푸시 메시지에 페이로드 식별자로 사용됩니다. | varchar(255) |
| mobileosenvironment | 컨텍스트 데이터 변수 a.OSEnenvironment에서 수집됩니다. Android 또는 iOS와 같은 OS 환경을 제공합니다. | varchar(255) |
...
| mobileplacecategory | 컨텍스트 데이터 변수 a.loc.category에서 수집됩니다. 특정 장소의 카테고리를 설명합니다. | varchar(255) |
| mobileplaceid | 컨텍스트 데이터 변수 a.<span>loc</span>.id에서 수집됩니다. 지정된 관심 영역에 대한 식별자입니다. | varchar(255) |
| mobilerlaunchcampaigcontent | Mobile Services 시작 컨텐츠 | varchar(255) |
| mobilerlaunchcampaignmedium | 모바일 서비스 시작 미디어 | varchar(255) |
| mobilerlaunchcampaisource | Mobile Services 시작 소스 | varchar(255) |
| mobilerlaunchcampaignterm | Mobile Services 출시 기간 | varchar(255) |
| mobilerlaunchcampaittrackingcode | 컨텍스트 데이터 변수 a.launch.campaign.trackingcode에서 수집됩니다. 시작 캠페인에 대한 추적 코드로 획득에 사용됩니다. | varchar(255) |
...
| os | 방문자의 운영 체제를 나타내는 숫자 ID입니다. user_agent 열을 기반으로 합니다. os 조회를 사용합니다. | 부호 없음 |
| p_plugins | 더 이상 사용되지 않습니다. 브라우저에 사용할 수 있는 플러그인 목록. JavaScript 함수 navigator.plugins()를 사용합니다. | text |
| page_event | 이미지 요청(표준 히트, 다운로드 링크, 사용자 지정 링크, 종료 링크)에서 전송된 히트 유형입니다. See [Page event lookup](datafeeds-page-event.md). | tinyint 부호 없음 |
...
| 레퍼러 | 이전 페이지의 페이지 URL입니다. | varchar(255) |
...
| truncated_hit | 이미지 요청이 잘렸음을 나타내는 플래그입니다. 부분 히트가 수신되었음을 나타냅니다. <br>Y:히트가 잘렸습니다.부분 히트 수신 <br>N:히트가 잘리지 않았습니다.수신된 전체 히트 | char(1) |
...
| user_hash | 사용하지 않습니다. 보고서 세트 ID의 해시. 대신 사용자 이름을 사용하십시오. | 부호 없음 |
| user_server | 서버 차원에 사용되는 변수입니다. | varchar(100) |
| userid | 사용하지 않습니다. 보고서 세트 ID의 숫자 ID입니다. 대신 사용자 이름을 사용하십시오. | 부호 없음 |
...
| videoadinpod | 비디오 광고 창 위치 | varchar(255) |
...
| videoader | 비디오 광고주 | varchar(255) |
| videoaudioalbum | 비디오 오디오 앨범 | varchar(255) |
| 비디오 오디오 아티스트 | 비디오 오디오 아티스트 | varchar(255) |
| videoaudioauthor | 비디오 오디오 작성자 | varchar(255) |
| videoaudiolabel | 비디오 오디오 레이블 | varchar(255) |
| videoaudioBlisher | 비디오 오디오 게시자 | varchar(255) |
| videoaudio-station | 비디오 오디오 스테이션 | varchar(255) |
| videocampaign | 비디오 캠페인 | varchar(255) |
| videochannel | 비디오 채널 | varchar(255) |
| videochapter | 비디오 장 이름 | varchar(255) |
...
| videoshow | 비디오 쇼 | varchar(255) |
| videoshowtype | 비디오 표시 유형 | varchar(255) |
| videostreamtype | 비디오 스트림 유형 | varchar(255) |
| visid_high | 방문을 고유하게 식별하기 위해 visid_low와 함께 사용됩니다. | 부호가 없는 |
| visid_low | 방문을 고유하게 식별하기 위해 visid_high와 함께 사용됩니다. | 부호가 없는 |
...
| visid_type | 방문자를 식별하는 데 사용된 방법을 나타내는 숫자 ID입니다. <br>0:사용자 지정 방문자 ID <br>1:IP 및 사용자 에이전트 폴백 <br>2:HTTP 모바일 구독자 헤더 <br>3:기존 쿠키 값(s_vi) <br>4:대체 쿠키 값(s_fid) <br>5:ID 서비스 | tinyint 부호 없음 |
| visit_keywords | 검색 키워드 차원에 사용되는 변수입니다. 이 열은 Adobe에서 사용하는 백엔드 로직을 수용하기 위해 비표준 문자 제한을 사용합니다. | varchar(244) |
| visit_num | 방문 번호 차원에 사용되는 변수입니다. 1에서 시작하여 새 방문이 방문자별로 시작될 때마다 증가합니다. | 부호 없음 |
| visit_page_num | 히트 깊이 차원에 사용되는 변수입니다. 사용자가 생성하는 각 히트에 대해 1씩 증가합니다. 각 방문을 재설정합니다. | 부호 없음 |
...
* mobileacquisionclicks
* mobileactioninapptime
* mobileactiontotaltime
* mobileappperformanceAfftedusers
* mobileappperformanceappid<span>.</span>app-perf-app-name
* mobileappperformanceappid<span>.</span>app-perf-platform
* mobileappperformanceAshes
* mobileappperformancececrashid<span>.</span>app-perf-crash-name
* mobileappperformanceceloads
* mobileappstoreavgrating
* mobileappstoredloads
* mobileappstorintexvenue
* mobileappstorinappropriate
* mobileappstorobjectid<span>.</span>app-store-user
* mobileappstorobjectid<span>.</span>application-name
* mobileappstorobjectid<span>.</span>application-version
* mobileappstorobjectid<span>.</span>appstore-name
* mobileappstorobjectid<span>.</span>category-name
* mobileappstorobjectid<span>.</span>국가 이름
* mobileappstorobjectid<span>.</span>장치 제조업체
* mobileappstorobjectid<span>.</span>device-name
* mobileappstorobjectid<span>.</span>in-app-name
* mobileappstorobjectid<span>.</span>platform-name-version
* mobileappstorobjectid<span>.</span>rank-category-type
* mobileappstorobjectid<span>.</span>region-name
* mobileappstorobjectid<span>.</span>검토 주석
* mobileappstorobjectid<span>.</span>리뷰 제목
* mobileappstoroneoffrevenue
* mobileappstoroneofrohit
* mobileappstore purchases
* mobileappstore
* mobileappstorankdivisor
* mobileappstoreving
* mobileappstoretingdivisor
* mobilevergprevsessionlength
* mobilecrashes
* mobilecrashrate
* mobiledailyengagedusers
* mobiledeplinkid<span>.</span>이름
* mobileinstalls
* mobilelaunches
* mobileltvtotal
* mobilemessagecliks
