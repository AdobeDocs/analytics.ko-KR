...
| `mc_audiences` | 방문자가 속한 Audience Manager 세그먼트 ID 목록입니다. 다음 `post_mc_audiences` 열이 구분 기호를 `--**--`. | 텍스트 |
...
| `mvvar1` - `mvvar3` | 목록 변수 값입니다. 구현에 따라 구분된 사용자 지정 값 목록을 포함합니다. 다음 `post_mvvar1` - `post_mvvar3` 열은 원래 구분 기호를 `--**--`. | 텍스트 |
