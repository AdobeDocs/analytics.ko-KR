...
| **`product_list`** | 페이지 변수 [`products`](/help/implement/vars/page-vars/products.md) . 범주](/help/components/dimensions/category.md)[, 제품[](/help/components/dimensions/product.md), 판매량](/help/components/metrics/units.md) 및 매출](/help/components/metrics/revenue.md)을 비롯한 [여러 차원과 지표를 채우는 [데 도움이 됩니다. | 텍스트 |
...
| **`yearly_visitor`** | 히트가 새 연별 방문자인지 판별하는 플래그. | tinyint 부호 없음 |
