...
성공적으로 가져오면 내보내기에서 즉시 적절한 변경 사항이 표시되지만 Analytics의 데이터 변경 사항은 브라우저 가져오기를 사용할 경우 최대 4시간, FTP 가져오기를 사용할 경우 최대 24시간이 걸릴 수 있습니다.
...
분류 또는 Data Source 파일(`.tab` 또는 `.txt`)을 업로드할 때 가져오는 데이터 파일과 이름이 같지만 가 인 빈 파일도 업로드해야 합니다.`.fin` 확장명. 이 `.fin` 파일은 완료한 파일입니다. 이 파일의 목적은 시스템에 데이터 파일이 FTP 계정에 완전히 업로드되었음을 알리는 것입니다. `.fin` 파일을 사용하면 사용자가 가져오기를 완료했음을 Adobe에서 인식할 수 있습니다.
...
`.fin` 파일 없이 데이터 소스 또는 분류 파일을 업로드하는 경우 Adobe가 처리를 위해 해당 파일을 대기열에 추가하지 않습니다. 이 파일은 FTP에 남아 있으며 CX Enterprise의 데이터에 적용되지 않습니다. Analytics의 [!UICONTROL FTP 계정 만들기] 창에서 [!UICONTROL 알림 받는 사람]&#x200B;(으)로 이메일 주소를 입력한 경우에만 이에 대한 알림을 받습니다. 이 필드에 이메일 주소를 입력하지 않으면 알림이 전송되지 않습니다.
