...
* **데이터 피드:** 원시 데이터의 시간별 또는 일별 내보내기를 받습니다. 모든 행은 개별 조회이며 모든 열은 변수입니다. 데이터 피드는 일반적으로 [클라우드 대상](/help/export/analytics-data-feed/create-feed.md)(으)로 전송됩니다.
* **Data Warehouse:** 요청 마법사를 사용하여 스프레드시트의 데이터 출력을 검색합니다. Data Warehouse은 다른 처리 아키텍처를 사용하여 임의 개수의 행과 임의 개수의 고유 값을 허용합니다. Data Warehouse 요청은 일반적으로 [클라우드 대상](/help/export/data-warehouse/create-request/dw-request-report-destinations.md)(으)로 전송됩니다
* **FTP 및 SFTP:** FTP 및 SFTP가 지원되지만 더 이상 권장되지 않습니다. 대신 데이터를 클라우드 대상으로 내보냅니다.
