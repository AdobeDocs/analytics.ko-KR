...
본 안내서에서는 Adobe Analytics에서 데이터를 가져오는 방법을 설명합니다. 이러한 서비스에는 다음이 포함됩니다.

* **데이터 피드:** 원시 데이터의 시간별 또는 일별 내보내기를 받습니다. 모든 행은 개별 히트이며 모든 열은 변수입니다. 데이터 피드는 일반적으로 [클라우드 대상](/help/export/analytics-data-feed/create-feed.md)으로 전송됩니다. 이전 FTP 및 SFTP 계정은 사용할 수 있지만 권장되지 않습니다.
