...
>[!IMPORTANT]
>
>The `<Advertising Analytics ID>` value (in **bold** below) is generic and **must be replaced with your specific account ID string**. 계정 설정 화면의 "추적" 섹션 아래에서 특정 계정 ID 문자열을 가져올 수 있습니다.
...
Bing 계정에 추가해야 하는 문자열이 아래에 표시되어 있습니다. 계정 전체에서 사용되는 모든 최종 URL 접미사에 문자열을 추가해야 합니다.
