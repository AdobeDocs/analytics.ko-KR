...
따라서 을(를) 선택할 때 승인 확인란을 선택해야 합니다. **[!UICONTROL 자동]** 계정 설정을 저장하기 전에

을 사용하여 검색 엔진 계정을 구성하려면 다음을 수행하십시오. **[!UICONTROL 자동]** 를 입력하면 다음 작업을 수행할 책임이 있습니다.

* 다음 `s_kwcid` 매개 변수 및 값이 추가되는 계정의 계정 추적 템플릿 또는 랜딩 페이지 URL에 추가됩니다. 이 매개 변수와 값은 URL 끝에 삽입됩니다. 웹 서버에서 특정 항목을 필요로 하는 경우 추가 작업이 필요할 수 있습니다 `key=value` URL 끝에 연결합니다. 또는 새로운 항목을 지원하는 업데이트 `key=value` 를 URL에 연결합니다. 추가된 URL 매개 변수가 최종 랜딩 페이지에 올바르게 유지되는지는 사용자가 확인해야 합니다.
* 또한 `s_kwcid` 값의 일부로 랜딩 URL에 키워드를 삽입할 수 있습니다. 특수 문자 또는 기호를 포함하는 경우 웹 서버에서 이러한 문자를 지원할 수 있는지 확인하십시오. 예를 들어 일반적인 특수 문자는 입니다. `+`: &quot;Broad Match Modified&quot; 키워드에 사용됩니다.
...
>[!IMPORTANT]
>
>*`<Advertising Analytics ID>`* 값 (아래에 **굵게** 표시됨)은 제네릭이므로, **특정 계정 ID 문자열로 대체해야** 합니다. 계정 화면 아래의 계정 화면에서 특정 계정 ID 문자열을 가져올 수 있습니다 [!UICONTROL 추적] 섹션.
...
![빙](/help/integrate/c-advertising-analytics/c-adanalytics-workflow/assets/bing-account.png)
