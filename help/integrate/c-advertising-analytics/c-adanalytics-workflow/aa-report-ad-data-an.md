...
>[!NOTE]
>
>검색 엔진 데이터가 Analytics 보고서에 채우기를 시작할 때까지 최소 24시간 대기해야 합니다. Adobe Advertising 데이터는 시간별 세부기간을 지원하지 않으므로 Analytics 보고에서는 시간별 세부기간에 대한 데이터를 반환하지 않습니다.
...
| 테이블/시각화 | 설명 |
|--- |--- |
| 광고 트렌드 | AMO 노출 횟수, AMO 클릭 수 및 AMO 비용에 대한 일일 트렌드 개요. |
...
| 캠페인 | 캠페인 비용의 막대 차트. |
| 캠페인 총계 | 연결된 지표로 분류된 상위 캠페인의 자유 형식 테이블. |
| 그룹 | 비용의 트리 맵. |
| 그룹 총계 | 관련 지표로 분류된 상위 광고 그룹의 자유 형식 테이블. |
| 광고 | 노출, 클릭 수 및 비용의 가로 막대 차트. |
| 광고 총계 | 관련 지표로 분류된 상위 광고의 자유 형식 테이블. |
| 키워드 | 모든 키워드/일치 유형 조합에 대한 노출 횟수, 클릭 수 및 비용의 분산형 차트. |
| 키워드 총계 | 관련 지표로 분류된 최상위 키워드/일치 유형 조합의 자유 형식 테이블입니다. |
